...
local C1_32 = 0xcc9e2d51
local C2_32 = 0x1b873593
...
function M.x86_32(key, seed)
  ...
  for i = 0, nblocks - 1 do
    ...
    h1 = bxor(h1, k1)
    h1 = rotl32(h1, 13)
    h1 = tobit(mul32(h1, 5) + tobit(0xe6546b64))
  end
  ...
  if rem > 0 then
    local k1 = 0
    if rem >= 3 then k1 = bxor(k1, lshift(byte(key, tail_start + 2), 16)) end
    if rem >= 2 then k1 = bxor(k1, lshift(byte(key, tail_start + 1),  8)) end
    k1 = bxor(k1, byte(key, tail_start))
    k1 = mul32(k1, C1_32)
    k1 = rotl32(k1, 15)
    k1 = mul32(k1, C2_32)
    h1 = bxor(h1, k1)
...
local C1_128x86 = 0x239b961b
local C2_128x86 = 0xab0e9789
local C3_128x86 = 0x38b34ae5
...
function M.x86_128(key, seed)
  ...
  for i = 0, nblocks - 1 do
    ...
    h1 = rotl32(h1, 19); h1 = tobit(mul32(tobit(h1 + h2), 5) + tobit(0x561ccd1b))

    k2 = mul32(k2, C2_128x86); k2 = rotl32(k2, 16); k2 = mul32(k2, C3_128x86); h2 = bxor(h2, k2)
    h2 = rotl32(h2, 17); h2 = tobit(mul32(tobit(h2 + h3), 5) + tobit(0x0bcaa747))

    k3 = mul32(k3, C3_128x86); k3 = rotl32(k3, 17); k3 = mul32(k3, C4_128x86); h3 = bxor(h3, k3)
    h3 = rotl32(h3, 15); h3 = tobit(mul32(tobit(h3 + h4), 5) + tobit(0x96cd1c35))

    k4 = mul32(k4, C4_128x86); k4 = rotl32(k4, 18); k4 = mul32(k4, C1_128x86); h4 = bxor(h4, k4)
    h4 = rotl32(h4, 13); h4 = tobit(mul32(tobit(h4 + h1), 5) + tobit(0x32ac3b17))
  end
  ...
  if rem >= 15 then k4 = bxor(k4, lshift(byte(key, tail_start + 14), 16)) end
  if rem >= 14 then k4 = bxor(k4, lshift(byte(key, tail_start + 13),  8)) end
  if rem >= 13 then k4 = bxor(k4, byte(key, tail_start + 12)) end
  if rem >= 13 then
    k4 = mul32(k4, C4_128x86); k4 = rotl32(k4, 18); k4 = mul32(k4, C1_128x86); h4 = bxor(h4, k4)
  end

  if rem >= 12 then k3 = bxor(k3, lshift(byte(key, tail_start + 11), 24)) end
  if rem >= 11 then k3 = bxor(k3, lshift(byte(key, tail_start + 10), 16)) end
  if rem >= 10 then k3 = bxor(k3, lshift(byte(key, tail_start +  9),  8)) end
  if rem >=  9 then k3 = bxor(k3, byte(key, tail_start + 8)) end
  if rem >=  9 then
    k3 = mul32(k3, C3_128x86); k3 = rotl32(k3, 17); k3 = mul32(k3, C4_128x86); h3 = bxor(h3, k3)
  end

  if rem >=  8 then k2 = bxor(k2, lshift(byte(key, tail_start +  7), 24)) end
  if rem >=  7 then k2 = bxor(k2, lshift(byte(key, tail_start +  6), 16)) end
  if rem >=  6 then k2 = bxor(k2, lshift(byte(key, tail_start +  5),  8)) end
  if rem >=  5 then k2 = bxor(k2, byte(key, tail_start + 4)) end
  if rem >=  5 then
    k2 = mul32(k2, C2_128x86); k2 = rotl32(k2, 16); k2 = mul32(k2, C3_128x86); h2 = bxor(h2, k2)
  end

  if rem >=  4 then k1 = bxor(k1, lshift(byte(key, tail_start +  3), 24)) end
  if rem >=  3 then k1 = bxor(k1, lshift(byte(key, tail_start +  2), 16)) end
  if rem >=  2 then k1 = bxor(k1, lshift(byte(key, tail_start +  1),  8)) end
  if rem >=  1 then k1 = bxor(k1, byte(key, tail_start)) end
  if rem >=  1 then
    k1 = mul32(k1, C1_128x86); k1 = rotl32(k1, 15); k1 = mul32(k1, C2_128x86); h1 = bxor(h1, k1)
  end
...
function M.x86_128_hex(key, seed)
  ...
  if not h1 then return nil, h2 end
...
local function hex64(v)
  local hi = tonumber(rshift(v, 32))
  local lo = tonumber(band(v, u64(0xffffffff)))
