...
mod.code_action_kind = {
  ...
  source = "source",
  source_organize_imports = "source.organize_imports",
  source_fix_all = "source.fix_all",
}
...
--[[@param s string]]
--[[@return lsp_message? message, table<string, string>|string headers_or_error]]
mod.string_to_lsp_message = function(s)
  -- Parse headers and body
  local headers_str, body = s:match("(.-)\r\n\r\n(.*)")
  if not headers_str or not body then return nil, "invalid LSP message format" end
  local headers = {}
  for a, b in headers_str:gmatch("(.-):%s*(.-)\r\n") do
    headers[a:lower()] = b
  end
  -- Optionally, check Content-Length header
  if headers["content-length"] then
    local len = tonumber(headers["content-length"])
    if len and #body > len then
      body = body:sub(1, len)
    end
  end
  local ok, msg = pcall(lunajson.decode, body)
