...
--:: SLNode = { key: any, value: any, forward: { [integer]: any }, span: { [integer]: integer }, level: integer }
...
function SL:insert(key, value)
  local self_ = self --[[:! Skiplist]]
  local cmp = self_._cmp
  local update, rank = find_update(self_, key)

  -- Check if key already exists at level 1
  local next1 = update[1].forward[1] --[[:! SLNode]]
  if next1 ~= self_._tail and next1.key == key then
    next1.value = value
    return self
  end

  local lvl = random_level()
  if lvl > self_._level then
    -- Extend update and rank arrays for new levels
    for i = self_._level + 1, lvl do
      update[i] = self_._header
      rank[i] = 0
    end
    self_._level = lvl
  end
...
function SL:delete(key)
  local self_ = self --[[:! Skiplist]]
  local update, _ = find_update(self_, key)
  local x = update[1].forward[1] --[[:! SLNode]]

  if x == self_._tail or x.key ~= key then
    return false
  end

  for i = 1, self_._level do
  ...
  for i = x.level + 1, self_._level do
  ...
  while self_._level > 1 and (self_._header.forward[self_._level] --[[:! SLNode]]) == self_._tail do
    self_._level = self_._level - 1
  end
...
function SL:get(key)
  local self_ = self --[[:! Skiplist]]
  local cmp = self_._cmp
  local x = self_._header
  for i = self_._level, 1, -1 do
    local xfi = x.forward[i] --[[:! SLNode]]
    while xfi ~= self_._tail and cmp(xfi.key, key) do
      x = xfi
      xfi = x.forward[i] --[[:! SLNode]]
    end
  end
  x = x.forward[1] --[[:! SLNode]]
  if x ~= self_._tail and x.key == key then
    return x.value
  end
  return nil
end
...
function SL:has(key)
  ...
  x = x.forward[1] --[[:! SLNode]]
  return x ~= self_._tail and x.key == key
end

-- Minimum key (nil if empty).
function SL:min()
  local self_ = self --[[:! Skiplist]]
  local x = self_._header.forward[1] --[[:! SLNode]]
  if x == self_._tail then return nil end
  return x.key
end
...
function SL:max()
  ...
  for i = self_._level, 1, -1 do
    local xfi = x.forward[i] --[[:! SLNode]]
    while xfi ~= self_._tail do
      x = xfi
      xfi = x.forward[i] --[[:! SLNode]]
    end
  end
  if x == self_._header then return nil end
  return x.key
end
...
function SL:range(lo, hi)
  ...
  for i = self_._level, 1, -1 do
    local xfi = x.forward[i] --[[:! SLNode]]
    while xfi ~= self_._tail and cmp(xfi.key, lo) do
      x = xfi
      xfi = x.forward[i] --[[:! SLNode]]
    end
  end
...
function SL:at_rank(pos)
  local self_ = self --[[:! Skiplist]]
  if pos < 1 or pos > self_._size then return nil end
  local x = self_._header
  local traversed = 0
  for i = self_._level, 1, -1 do
    local xfi = x.forward[i] --[[:! SLNode]]
    while xfi ~= self_._tail and traversed + x.span[i] < pos do
...
function SL:succ(key)
  ...
  x = x.forward[1] --[[:! SLNode]]
  while x ~= self_._tail and x.key == key do
    x = x.forward[1] --[[:! SLNode]]
  end
  if x == self_._tail then return nil end
  return { key = x.key, value = x.value }
...
function SL:floor(key)
  ...
  local candidate = x.forward[1] --[[:! SLNode]]
  if candidate ~= self_._tail and candidate.key == key then
