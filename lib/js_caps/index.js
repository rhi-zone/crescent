...
// fetch_api factory (config-bound at host-side instantiation; see
// below):
//
...
//
// The remaining caps (kv_*, navigate, dialog, toast) involve design
...
//
// Some caps -- `fetch_api` today, and future per-pack-configured caps
// like `kv_*` (max_value_bytes) and `navigate` (allowed_navigations) --
...
// supplied allowlist would defeat the manifest grant; see
// docs/browser_caps.md §4.1.1). These are exported as factories
// (`makeFetchApi`, etc.) the host page calls per manifest entry. The
// host page is expected to construct the cap-impls Map at mount time:
//
