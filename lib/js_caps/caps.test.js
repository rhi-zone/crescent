...
// The dayZeroCaps aggregator is verified to contain exactly the 8
...
test("set_timeout: resolves with performance.now after delay", async () => {
  const before = globalThis.performance.now();
  const fired = await set_timeout(5);
  const after = globalThis.performance.now();
  ok(typeof fired === "number", "resolves with number");
  ok(fired >= before, "fired >= before");
  ok(fired <= after + 1, "fired <= after (allow 1ms jitter)");
});

test("set_timeout: negative delay clamps to 0", async () => {
  const fired = await set_timeout(-1000);
  ok(typeof fired === "number", "still resolves");
});

test("set_timeout: NaN rejected", () => {
  const e = throws(() => set_timeout(NaN), "NaN");
  ok(e instanceof TypeError, "TypeError instance");
});

test("set_timeout: Infinity rejected", () => {
  const e = throws(() => set_timeout(Infinity), "Infinity");
  ok(e instanceof TypeError, "TypeError instance");
});

test("set_timeout: string rejected", () => {
  const e = throws(() => set_timeout("5"), "string");
  ok(e instanceof TypeError, "TypeError instance");
});
...
test("clear_timeout: returns undefined", () => {
  // Silence the deprecation warning emitted once per process.
  const origWarn = globalThis.console.warn;
  globalThis.console.warn = () => {};
  try {
    eq(clear_timeout("any-handle"), undefined, "returns undefined");
    eq(clear_timeout(42), undefined, "also undefined for number");
  } finally {
...
test("dayZeroCaps: does not contain the deferred 10 caps", () => {
  const deferred = [
