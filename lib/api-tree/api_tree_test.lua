-- lib/api-tree/api_tree_test.lua
-- Tests for lib/api-tree's op/api/merge_meta (init.lua).
...
T.describe("lib.api-tree", function()
  ...
  T.describe("merge_meta", function()
    ...
    T.it("no args returns empty table", function()
      assert_deep_eq(api_tree.merge_meta(), {})
    end)

    T.it("nil args are skipped, don't blank out earlier bags", function()
      local out = api_tree.merge_meta({ a = 1 }, nil, { b = 2 })
      assert_deep_eq(out, { a = 1, b = 2 })
    end)

    T.it("scalars: later wins", function()
      local out = api_tree.merge_meta({ verb = "GET" }, { verb = "POST" })
      T.eq(out.verb, "POST")
    end)

    T.it("arrays concatenate in order", function()
      local out = api_tree.merge_meta({ directives = { "a", "b" } }, { directives = { "c" } })
      assert_deep_eq(out.directives, { "a", "b", "c" })
    end)

    T.it("plain tables merge recursively", function()
      local out = api_tree.merge_meta({ tags = { readOnly = true } }, { tags = { deprecated = true } })
    ...
    T.it("recursive merge: later scalar wins on a shared nested key", function()
      local out = api_tree.merge_meta({ http = { verb = "GET" } }, { http = { verb = "POST", path = "/x" } })
  ...
  T.describe("op", function()
    ...
    T.it("op(fn, meta) uses the single bag as-is (no merge)", function()
      ...
      local node = api_tree.op(fn, meta)
    ...
    T.it("op(fn, m1, m2) deep-merges left to right, later wins", function()
      --: (unknown) -> unknown
      local fn = function(x) return x end
      local node = api_tree.op(fn, { tags = { readOnly = true } }, { tags = { deprecated = true }, description = "d" })
    ...
    T.it("op(fn, m1, m2, m3) folds all three left to right", function()
      --: (unknown) -> unknown
      local fn = function(x) return x end
      local node = api_tree.op(fn, { v = 1 }, { v = 2 }, { v = 3, w = 1 })
  ...
  T.describe("api", function()
    ...
    T.it("api(children) has empty meta and no fallback key", function()
      local leaf = api_tree.op(function(x) return x end)
      local node = api_tree.api({ getBook = leaf })
    ...
    T.it("api(children, { meta = ... }) sets meta", function()
      local node = api_tree.api({}, { meta = { description = "root" } })
    ...
    T.it("api(children, { fallback = ... }) sets fallback, leaves meta empty by default", function()
      ...
      local node = api_tree.api({}, { fallback = { name = "bookId", subtree = subtree } })
    ...
    T.it("api(children, { meta = ..., fallback = ... }) sets both", function()
      local subtree = api_tree.api({})
      local node = api_tree.api({}, { meta = { description = "x" }, fallback = { name = "id", subtree = subtree } })
    ...
    T.it("both handler and children is valid (uncommon but valid)", function()
      --: (unknown) -> unknown
      local fn = function(x) return x end
      local leaf = api_tree.op(fn)
      local node = api_tree.api({ child = leaf })
