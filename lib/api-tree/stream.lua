-- lib/api-tree/stream.lua — the streaming-handler convention, ported from
...
function M.from(producer)
	...
	local function emit(value)
		if st.gate_resolve ~= nil then
			error("api_tree.stream: emit called while the previous emit is still pending — await the promise emit returns")
		end
		if st.waiter_resolve == nil then
			error("api_tree.stream: emit called with no pending next()")
		end
	...
	local function next_step()
		...
		if st.waiter_resolve ~= nil then
			error("api_tree.stream: next() called while a previous next() is still pending")
		end
