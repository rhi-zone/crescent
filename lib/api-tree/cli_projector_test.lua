-- lib/api-tree/cli_projector_test.lua
-- Tests for lib/api-tree/cli_projector.lua (the CLI projection of a Node tree).
...
local cli     = require("lib.api-tree.cli_projector")
...
local function build_tree()
  return api_tree.api({
    books = api_tree.api({
      list = api_tree.op(function(_) return { "a", "b" } end,
        { description = "List books", tags = { readOnly = true } }),
      create = api_tree.op(echo, { description = "Create a book" }),
      delete = api_tree.op(function(_) return { deleted = true } end,
        { tags = { destructive = true } }),
    }, {
      meta = { description = "Book commands" },
      fallback = {
        name = "bookId",
        subtree = api_tree.api({
          read = api_tree.op(echo, {}),
        }, {}),
      },
    }),
    ping = api_tree.op(function(_) return "pong" end,
      { cli = { alias = "p" }, tags = { deprecated = true } }),
    hidden = api_tree.op(function(_) return 1 end, { cli = { hidden = true } }),
  }, {})
end

T.describe("lib.api-tree.cli_projector", function()
  ...
  T.describe("flag parsing", function()
    ...
    T.it("a valueless flag is true and a repeated flag collects", function()
      local tree = api_tree.api({ echo = api_tree.op(echo, {}) }, {})
  ...
  T.describe("Result unwrapping", function()
    ...
    T.it("unwraps an ok Result to its value", function()
      local tree = api_tree.api({ get = api_tree.op(function(_) return result.ok({ id = 1 }) end, {}) }, {})
    ...
    T.it("an err Result becomes a cli_error with the default message", function()
      local tree = api_tree.api({
        get = api_tree.op(function(_) return result.err({ kind = "notFound" }) end, {}),
      }, {})
      local r = run(tree, { "get" }, nil)
      local message, exit_code = cli_error_parts(r.reason)
    ...
    T.it("an unmatched kind falls back to the default encoding", function()
      local tree = api_tree.api({
        get = api_tree.op(function(_) return result.err({ kind = "conflict" }) end, {}),
      }, {})
    ...
    T.it("detection.result = false leaves the Result shape intact", function()
      local tree = api_tree.api({ get = api_tree.op(function(_) return result.ok(1) end, {}) }, {})
  ...
  T.describe("thrown handler errors", function()
    ...
    T.it("are collapsed to an internal error, never surfaced verbatim", function()
      local tree = api_tree.api({
        boom = api_tree.op(function(_) error("secret path /etc/passwd") end, {}),
      }, {})
  ...
  T.describe("JSONL output", function()
    ...
    T.it("a streaming-tagged array prints one item per line", function()
      local tree = api_tree.api({
        list = api_tree.op(function(_) return { 1, 2, 3 } end, { tags = { streaming = true } }),
      }, {})
    ...
    T.it("--jsonl turns on the same output mode", function()
      local tree = api_tree.api({ list = api_tree.op(function(_) return { 1, 2 } end, {}) }, {})
      local r = run(tree, { "list", "--jsonl" }, nil)
      T.eq(r.out, "1\n2\n")
    end)

    T.it("nil prints as null", function()
      local tree = api_tree.api({ nothing = api_tree.op(function(_) return nil end, {}) }, {})
  ...
  T.describe("pagination", function()
    ...
    local function offset_tree()
      return api_tree.api({
        list = api_tree.op(function(input)
  ...
  T.describe("input sources", function()
    ...
    T.it("meta.cli.sourceMap can pull a field from the env cap", function()
      local tree = api_tree.api({
        whoami = api_tree.op(echo, { cli = { sourceMap = { apiKey = { store = "env", key = "API_KEY" } } } }),
      }, {})
  ...
  T.describe("deterministic output", function()
    ...
    T.it("object keys are sorted, so output does not vary run to run", function()
      local tree = api_tree.api({
        get = api_tree.op(function(_) return { zeta = 1, alpha = 2, mid = 3 } end, {}),
      }, {})
