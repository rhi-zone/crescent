-- lib/api-tree/jsonrpc_client_test.lua
-- Tests for lib/api-tree/jsonrpc_client.lua (the tree-mirroring client).
...
local client  = require("lib.api-tree.jsonrpc_client")
...
T.describe("lib.api-tree.jsonrpc_client", function()
  ...
  T.describe("proxy shape", function()
    ...
    T.it("mirrors branches as nested tables and leaves as callables", function()
      local tree = api_tree.api({
        users = api_tree.api({ list = api_tree.op(inert) }, nil),
      }, nil)
  ...
  T.describe("name derivation", function()
    ...
    T.it("agrees with the server's dispatch table", function()
      ...
      local handlers = require("lib.api-tree.jsonrpc_project").project_methods(tree, nil).handlers
  ...
  T.describe("fallback slug capture", function()
    ...
    T.it("lets an explicit field win over a captured slug", function()
      local log = {} --: CallLog
      local tree = api_tree.api({}, {
        fallback = { name = "id", subtree = api_tree.api({ get = api_tree.op(inert) }, nil) },
      })
      local api = client.client_from_tree(tree, recording_call(log))
    ...
    T.it("accumulates nested slugs", function()
      ...
      local inner = api_tree.api({}, {
        fallback = { name = "chapterId", subtree = api_tree.api({ get = api_tree.op(inert) }, nil) },
      })
      local tree = api_tree.api({
        books = api_tree.api({}, { fallback = { name = "bookId", subtree = inner } }),
      }, nil)
      local api = client.client_from_tree(tree, recording_call(log))
