-- lib/api-tree/jsonrpc_client.lua — the runtime JSON-RPC client, ported from
...
local function merge_slugs(slugs, input)
	...
	if type(input) ~= "table" then
		error("api_tree.jsonrpc_client: params must be a record — JSON-RPC params are by-name only")
	end
...
function M.call_from_post(post, opts)
	...
	return async.async(function(method, params)
		...
		if body == nil then
			error("api_tree.jsonrpc_client: params are not JSON-encodable")
		end

		local raw, post_err = post(body)
		if raw == nil then
			error(post_err or "api_tree.jsonrpc_client: transport returned no response")
		end

		local decoded = json.decode(raw)
		if type(decoded) ~= "table" then
			error("api_tree.jsonrpc_client: response is not a JSON-RPC Response object")
		end
