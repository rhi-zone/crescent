-- lib/api-tree/http_adapter.lua — the boundary between the HTTP projector's
...
--     lib/api-tree/stream.lua Stream that has run NOTHING up to this instant —
		...
		local outcome = async.await(stream.drive(producer, {
			...
			chunk = function(chunk)
				if type(chunk) ~= "string" then
					error("api_tree.http_adapter: a streaming response producer emitted a "
...
function M.materialize(rv, res, sock)
	if not is_response_value(rv) then
		error("api_tree.http_adapter: materialize expects an http_value response value")
	end
	write_head(res, rv)
	local body = rv.body
	...
	if not is_stream_producer(producer) then
		error("api_tree.http_adapter: a streaming response value carries no Stream producer")
	end
