-- lib/api-tree/jsonrpc_project.lua — JSON-RPC 2.0 method projection, ported
-- from fractal's packages/json-rpc-api-projector/src/project.ts.
--
-- Walks a Node tree (lib/api-tree/init.lua) and produces a flat list of method
...
-- lib/type-ir/json_rpc.lua, optionally narrowed by `meta.jsonrpc.errorDataSchema`.
--
...
-- reason, as `ordered_keys` in lib/type-ir/init.lua; the SET of methods produced is
...
local codes    = require("lib.type-ir.json_rpc")
...
local function build_method(child, name, description_fallback, schemas, handlers)
	...
	if handler == nil then
		error("api_tree.jsonrpc_project: build_method called on a node with no handler")
	end
