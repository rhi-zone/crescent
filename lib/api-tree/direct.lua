-- lib/api-tree/direct.lua — the zero-protocol-overhead projection, ported from
...
local function is_plain_table(v)
	...
	return not api_tree.is_array(v)
end
...
local function make_caller(node, slugs)
	...
	if handler == nil then
		error("api_tree.direct: make_caller called on a node with no handler")
	end
