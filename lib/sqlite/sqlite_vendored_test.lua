-- lib/sqlite/sqlite_vendored_test.lua
-- CI assertion: on platforms where dep/ ships a vendored libsqlite3,
-- the vendored tier must be the one that loads. This catches dep/
-- regressions without removing the runtime fallback to system libs
-- (which is intentional — see CLAUDE.md "Don't degrade runtime to
-- surface CI gaps").

if not package.path:find("./?/init.lua", 1, true) then
	package.path = "./?/init.lua;" .. package.path
end

local ffi = require("ffi")
local T = require("lib.test.assert")

-- Mirror lib/sqlite/init.lua's vendored_name() exactly.
local function expected_vendored_path()
	local os_, arch = ffi.os, ffi.arch
	if os_ == "Linux" then
		return arch == "arm64" and "dep/sqlite3/libsqlite3-linux-aarch64.so"
		                       or  "dep/sqlite3/libsqlite3-linux-x86_64.so"
	elseif os_ == "OSX" then
		return arch == "arm64" and "dep/sqlite3/libsqlite3-macos-arm64.dylib" or nil
	elseif os_ == "Windows" then
		return arch == "x64" and "dep/sqlite3/sqlite3-x64.dll" or "dep/sqlite3/sqlite3-x86.dll"
	end
	return nil
end

T.describe("sqlite: vendored tier", function()
	local expected = expected_vendored_path()
	if not expected then
		T.it("platform has no vendored artifact (skipped)", function()
			T.ok(true, "no dep/ artifact for " .. tostring(ffi.os) .. "/" .. tostring(ffi.arch))
		end)
		return
	end

	local ok, sqlite = pcall(require, "lib.sqlite")
	if not ok then
		T.it("sqlite loads on a platform with a vendored artifact", function()
			error("lib.sqlite failed to load: " .. tostring(sqlite))
		end)
		return
	end

	T.it("_loaded_from equals the vendored dep/ path", function()
		T.eq(sqlite._loaded_from, expected)
	end)

	T.it("_tier is system-sqlite", function()
		T.eq(sqlite._tier, "system-sqlite")
	end)
end)
