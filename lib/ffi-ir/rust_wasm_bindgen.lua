-- lib/ffi-ir/rust_wasm_bindgen.lua — the ffi-ir wasm-bindgen
...
-- `lib/type-ir/rust_wasm_bindgen.lua` (the data-shape-only projector:
...
-- comparison inside `lib/type-ir/rust_wasm_bindgen.lua`'s `vec_element_type`,
...
-- follows (see `lib/type-ir/gleam_native.lua`'s and `lib/type-ir/rescript_native.lua`'s
-- headers): the CONSUMER opts into the refined-kind lattice. This file's test
-- requires it, exactly as `lib/type-ir/rust_wasm_bindgen_test.lua` does.
...
local wb = require("lib.type-ir.rust_wasm_bindgen")
...
-- `lib/type-ir/rust_wasm_bindgen.lua`; the local copy exists because that one is
...
--
-- Deliberately NOT `type_ref_codegen.is_a`, which walks lib/type-ir/init.lua's
-- registry: ffi-ir keeps its own, separate kind lattice (see init.lua's
...
-- (see `with_ownership`'s own doc comment in init.lua), not on arbitrary
-- nested fields.
...
local function unsupported_ownership_error(ref, where)
	...
	-- `ffi_ir.ownership_of` does internally and as `wit.lua`'s
...
-- kind — per its own doc comment in init.lua ("exposes behavior ONLY through
...
function M.to_wasm_bindgen_ffi(ref, name)
	...
	-- init.lua seeds the registry with.
...
-- as in `lib/type-ir/rust_wasm_bindgen.lua` — it is that function, not a wrapper.
