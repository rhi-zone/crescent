-- lib/ffi-ir/init.lua — the FFI-IR data model, ported from fractal's
...
-- model in `lib/type-ir/init.lua` rather than folded into it. Every
...
--      in lib/type-ir/init.lua — it is NOT a wrapping type constructor the way WIT's
...
--      `FfiRef`, mirroring lib/type-ir/init.lua's `TypeShape`/`TypeRef` pattern (open
...
-- nothing to do with lib/type-ir/init.lua's. Boundary-semantics kinds (`function`,
...
-- `Meta` below are lib/type-ir/init.lua's, and every parameter/return position in
...
-- lib/type-ir/json_schema.lua does. That works while init.lua is the file being
...
--
--   → lib/ffi-ir/init.lua:263: undefined type `TypeRef`
--     lib/ffi-ir/init.lua:263: type contains 'any' — use 'unknown' ...
--
...
-- Same shape as the `assign` helper in lib/type-ir/json_schema.lua, which is
...
-- payload. lib/type-ir/init.lua's `types` table makes its payload-free kinds shared
...
-- `rust_wasm_bindgen.lua`'s `declared_discipline_kind` — see that
...
-- enforces — the same "conventions over contracts" precedent lib/type-ir/init.lua's
...
-- lib/type-ir/init.lua's `TypeShape` is open: the TS interface is extended by
...
-- as lib/type-ir/init.lua's `TypeRef.meta`. Recognized keys today: `description` (a
...
-- about its signature, the same rename lib/type-ir/init.lua applied to `t()`.)
--: (shape: FfiShape, meta: Meta | nil) -> FfiRef
function M.ffi_ref_from_shape(shape, meta)
	return { shape = shape, meta = meta or {} }
end

-- The boundary shape constructors, mirroring lib/type-ir/init.lua's `types`.
--
...
-- fractal name. Same rename lib/type-ir/init.lua applied for the same reason.
M.boundary = {
...
--
-- This registry is NOT lib/type-ir/init.lua's — see the file header. Registering
...
-- this port will ever have would fix a vocabulary this schema leaves open
