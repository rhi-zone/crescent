-- lib/ffi-ir/csharp_pinvoke.lua — the .NET/P-Invoke consumer-side projector,
...
-- never throws on one (precedent: `lib/type-ir/init.lua`'s `resolve_ref`, which
...
-- those modules, so `lib/type-ir/init.lua`'s lattice carries only its two
...
-- lands ahead of the `kinds/common` port. `csharp_pinvoke_test.lua` pins the
...
-- THIS file's line numbers. `lib/ffi-ir/init.lua` carries the same three
...
-- `lib/type-ir/init.lua`'s own `ordered_keys` sets, for the same reason. The emitted
...
-- required field"), and the same shape of fix as `init.lua`'s `assign`
...
-- is the existing precedent (`lib/type-ir/init.lua`'s `resolve_ref`, `ffi_ir_test.lua`),
-- and `init.lua`'s own `FfiFunctionLike` is the same idea given a name.
