-- lib/ffi-ir/typescript_deno.lua — the Deno FFI consumer projector, ported from
...
-- projector in ruby_ffi.lua is the sibling consumer-side backend whose
...
--     owns/frees the bytes) that neither ffi-ir's schema (init.lua) nor the
...
-- init.lua), mirroring the per-target reasoning each sibling backend gives:
--
...
-- byte order — the same substitution, for the same reason, that lib/type-ir/init.lua's
...
-- structural-field-read precedent as lib/type-ir/init.lua's `resolve_ref`, and the same
-- formulation ruby_ffi.lua's own builders use.
