-- lib/ffi-ir/wit.lua — the WIT (`.wit` source text) projector, ported
...
-- (see init.lua's header), which is exactly what WIT's call-site
...
-- documented `resource_ref()` convention from init.lua. `opaque-handle` and
			...
			if ref.shape.kind ~= "ref" then
				return nil,
					'to_wit: "resource" ownership metadata requires a { kind: "ref" } TypeRef naming the resource (the '
						.. 'resource_ref() convention from init.lua), got shape kind "'
						.. ref.shape.kind
						.. '"'
			end
...
-- fields it reads. Same structural-field-read precedent as lib/type-ir/init.lua's
-- `resolve_ref`.
		...
		if name == nil then
			return nil,
				'to_wit: a standalone "' .. kind .. '" shape requires a name — its own schema carries no name (see '
					.. "init.lua's FfiFunctionShape/FfiMethodShape)"
		end
