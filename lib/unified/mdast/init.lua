...
-- Block-level HTML element names per CommonMark §4.6 type 6.
local html_block_tags = {
  address=1, article=1, aside=1, base=1, basefont=1, blockquote=1, body=1,
  caption=1, center=1, col=1, colgroup=1, dd=1, details=1, dialog=1, dir=1,
  div=1, dl=1, dt=1, fieldset=1, figcaption=1, figure=1, footer=1, form=1,
  frame=1, frameset=1, h1=1, h2=1, h3=1, h4=1, h5=1, h6=1, head=1, header=1,
  hr=1, html=1, iframe=1, legend=1, li=1, link=1, main=1, menu=1, menuitem=1,
  meta=1, nav=1, noframes=1, ol=1, optgroup=1, option=1, p=1, param=1,
  search=1, section=1, summary=1, table=1, tbody=1, td=1, tfoot=1, th=1,
  thead=1, title=1, tr=1, track=1, ul=1,
}

-- Check if a line starts an HTML block per CommonMark §4.6.
-- Returns true if the line triggers an HTML block.
local function is_html_block_start(line)
  local _, col = count_indent(line)
  local rest = str_sub(line, col)
  if str_byte(rest, 1) ~= 60 then return false end  -- must start with '<'
  local rest2 = str_sub(rest, 2)
  -- Type 1: <script, <pre, <style, <textarea (case-insensitive, followed by space/>/\n).
  local tag1 = rest2:match("^([Ss][Cc][Rr][Ii][Pp][Tt][ \t>]?)")
    or rest2:match("^([Pp][Rr][Ee][ \t>]?)")
    or rest2:match("^([Ss][Tt][Yy][Ll][Ee][ \t>]?)")
    or rest2:match("^([Tt][Ee][Xx][Tt][Aa][Rr][Ee][Aa][ \t>]?)")
  if tag1 then return true end
  -- Closing type 1: </script, </pre, </style, </textarea.
  if rest2:match("^/[Ss][Cc][Rr][Ii][Pp][Tt]") or rest2:match("^/[Pp][Rr][Ee]")
     or rest2:match("^/[Ss][Tt][Yy][Ll][Ee]") or rest2:match("^/[Tt][Ee][Xx][Tt][Aa][Rr][Ee][Aa]") then
    return true
  end
  -- Type 2: <!--
  ...
  if rest:match("^<![A-Z]") then return true end
  -- Type 5: <![CDATA[
  if rest:match("^<!%[CDATA%[") then return true end
  -- Type 6: block-level element (open or close tag).
  local tag_name = rest2:match("^([a-zA-Z][a-zA-Z0-9%-]*)[ \t/>]")
    or rest2:match("^([a-zA-Z][a-zA-Z0-9%-]*)$")
    or rest2:match("^/([a-zA-Z][a-zA-Z0-9%-]*)[ \t>]")
    or rest2:match("^/([a-zA-Z][a-zA-Z0-9%-]*)$")
  if tag_name and html_block_tags[tag_name:lower()] then return true end
  return false
end
...
parse_blocks = function(lines, i, j, lazy_set)
  ...
  while i <= j do
    ...
    elseif is_html_block_start(line) then
