...
// Replace the content + swipe state of an existing message element
...
// Find a message element by its data-id
...
messageList.addEventListener("click", async function (e) {
  ...
  const label = msgEl.querySelector(".message__swipe-label");
  const parts = label.textContent.split("/");
  const index = parseInt(parts[0], 10) - 1;
  const total = parseInt(parts[1], 10);

  // Next past the last swipe generates a new one
  if (dir === "next" && index >= total - 1) {
    setBusy(true);
    const data = await request("POST", "/api/swipe/new", { message_id: id });
    setBusy(false);
    if (data) updateMessage(msgEl, data);
  } else {
    setBusy(true);
    const data = await request("POST", "/api/swipe/" + dir, { message_id: id });
    setBusy(false);
    if (data) updateMessage(msgEl, data);
  }
});
