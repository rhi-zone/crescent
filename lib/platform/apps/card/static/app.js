...
if (btnTestConnection) {
  btnTestConnection.addEventListener("click", async function () {
    ...
    try {
      ...
      connectionResult.textContent = "Failed: " + e.message;
      connectionResult.className = "connection-result connection-result--error";
    }
...
const btnWorldInfo = document.getElementById("btn-world-info");
const worldInfoOverlay = document.getElementById("world-info-overlay");
const worldInfoClose = document.getElementById("world-info-close");
const worldInfoAdd = document.getElementById("world-info-add");
const worldInfoImport = document.getElementById("world-info-import");
const worldInfoExport = document.getElementById("world-info-export");
const worldInfoFile = document.getElementById("world-info-file");
const worldInfoList = document.getElementById("world-info-list");

function openWorldInfo() {
  worldInfoOverlay.hidden = false;
  loadWorldInfo();
}

function closeWorldInfo() {
  worldInfoOverlay.hidden = true;
}

btnWorldInfo.addEventListener("click", openWorldInfo);
worldInfoClose.addEventListener("click", closeWorldInfo);
worldInfoOverlay.addEventListener("click", function (e) {
  if (e.target === worldInfoOverlay) closeWorldInfo();
});

async function loadWorldInfo() {
  const data = await request("GET", "/api/world_info");
  if (!data) return;
  renderWorldInfoEntries(data.entries);
}

function renderWorldInfoEntries(entries) {
  worldInfoList.innerHTML = "";
  if (!entries || entries.length === 0) {
    worldInfoList.innerHTML = '<div class="lorebook-empty">No world info entries. Click "+ Add Entry" to create one.</div>';
    return;
  }
  for (const entry of entries) {
    worldInfoList.appendChild(createWorldInfoEntryEl(entry));
  }
}

function createWorldInfoEntryEl(entry) {
  const el = document.createElement("div");
  el.className = "lorebook-entry" + (entry.enabled ? "" : " lorebook-entry--disabled");
  el.dataset.uid = entry.uid;
  ...
  toggle.addEventListener("click", async function (e) {
    e.stopPropagation();
    const newEnabled = !entry.enabled;
    const data = await request("POST", "/api/world_info/update", {
      uid: entry.uid, enabled: newEnabled,
    });
    if (data) {
      entry.enabled = newEnabled;
  ...
  body.querySelector(".lorebook-entry__save").addEventListener("click", async function () {
    ...
    if (data) loadWorldInfo();
  });

  body.querySelector(".lorebook-entry__delete").addEventListener("click", async function () {
    const data = await request("POST", "/api/world_info/delete", { uid: entry.uid });
    if (data) loadWorldInfo();
  });
...
worldInfoAdd.addEventListener("click", async function () {
  const data = await request("POST", "/api/world_info/add", {
    keys: ["new keyword"],
    content: "",
  });
  if (data) loadWorldInfo();
...
worldInfoExport.addEventListener("click", async function () {
  const data = await request("GET", "/api/world_info/export");
  if (!data || !data.entries) return;
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "world_info.json";
  a.click();
  URL.revokeObjectURL(url);
});

worldInfoImport.addEventListener("click", function () {
  worldInfoFile.click();
});

worldInfoFile.addEventListener("change", async function () {
  const file = worldInfoFile.files[0];
  if (!file) return;
  try {
    ...
    const data = await request("POST", "/api/world_info/import", { entries });
    if (data) loadWorldInfo();
...
const regexClose = document.getElementById("regex-close");
const regexAdd = document.getElementById("regex-add");
const regexList = document.getElementById("regex-list");
const regexTestFind = document.getElementById("regex-test-find");
const regexTestReplace = document.getElementById("regex-test-replace");
const regexTestInput = document.getElementById("regex-test-input");
const regexTestBtn = document.getElementById("regex-test-btn");
const regexTestOutput = document.getElementById("regex-test-output");

const SCOPE_LABELS = { ai_output: "AI Output", user_input: "User Input", display: "Display" };

function openRegex() {
  regexOverlay.hidden = false;
  loadRegex();
}

function closeRegex() {
  regexOverlay.hidden = true;
}

btnRegex.addEventListener("click", openRegex);
regexClose.addEventListener("click", closeRegex);
regexOverlay.addEventListener("click", function (e) {
  if (e.target === regexOverlay) closeRegex();
});
...
function createRegexEntryEl(script) {
  ...
  body.querySelector(".regex-entry__save").addEventListener("click", async function () {
    const newName = body.querySelector('[data-field="name"]').value.trim();
    if (!newName) return;
    const data = await request("POST", "/api/regex/save", {
      name: newName,
      find: body.querySelector('[data-field="find"]').value,
      replace: body.querySelector('[data-field="replace"]').value,
      scope: body.querySelector('[data-field="scope"]').value,
      order: parseInt(body.querySelector('[data-field="order"]').value, 10) || 0,
      enabled: script.enabled,
    });
    if (data) loadRegex();
  });

  body.querySelector(".regex-entry__delete").addEventListener("click", async function () {
...
regexTestBtn.addEventListener("click", async function () {
  ...
  const input = regexTestInput.value;
  if (!find || !input) return;
  const data = await request("POST", "/api/regex/test", { find: find, replace: replace, input: input });
  if (data) {
...
const anDepth = document.getElementById("an-depth");
const anPosition = document.getElementById("an-position");
const anSave = document.getElementById("an-save");

anToggle.addEventListener("click", function () {
  anPanel.hidden = !anPanel.hidden;
  if (!anPanel.hidden) loadAuthorsNote();
});
...
btnExport.addEventListener("click", function () {
  // Simple toggle: show format choice or just export text.
...
function closeAnyPanel() {
  ...
  if (!lorebookOverlay.hidden) { closeLorebook(); return true; }
  if (!worldInfoOverlay.hidden) { closeWorldInfo(); return true; }
  if (!cardEditOverlay.hidden) { closeCardEditor(); return true; }
  if (!regexOverlay.hidden) { closeRegex(); return true; }
...
document.addEventListener("keydown", function (e) {
  ...
  if (mod && e.shiftKey) {
    switch (e.key) {
      ...
      case "L": openLorebook(); e.preventDefault(); break;
      case "W": openWorldInfo(); e.preventDefault(); break;
...
const groupEnabledCheckbox = document.getElementById("group-enabled");
const groupTurnOrder = document.getElementById("group-turn-order");
const groupMemberList = document.getElementById("group-member-list");
const groupAddJson = document.getElementById("group-add-json");
  ...
  for (const m of members) {
    ...
    if (!m.is_primary) {
      ...
      removeBtn.addEventListener("click", async function () {
        const data = await request("POST", "/api/group/remove", { name: m.name });
