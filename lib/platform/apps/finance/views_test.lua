...
T.describe("finance.views", function()
  ...
  T.describe("M.build", function()
    T.it("dispatches known view names to their constructor", function()
      local app = new_app()
      T.eq(views.build("dashboard", app, nil, nil).view, "dashboard")
      T.eq(views.build("accounts", app, nil, nil).view, "accounts")
      T.eq(views.build("periods", app, nil, nil).view, "periods")
      T.eq(views.build("entries", app, nil, nil).view, "entries")
      T.eq(views.build("post_entry", app, nil, nil).view, "post_entry")
      T.eq(views.build("reports", app, nil, nil).view, "reports")
    end)

    T.it("falls back to dashboard for an unknown view name", function()
      local app = new_app()
      T.eq(views.build("not_a_real_view", app, nil, nil).view, "dashboard")
    end)

    T.it("passes params through to account_form (edit-mode prefill)", function()
      local app = new_app()
      app.add_account({ id = "cash", name = "Cash", type = "asset" })
      local view = views.build("account_form", app, { id = "cash" }, nil)
      T.eq(view.title, "Edit Account: cash")
    end)

    T.it("carries a form_error through to the rebuilt view", function()
      local app = new_app()
      local view = views.build("post_entry", app, nil, "entry did not balance")
      T.eq(view.error, "entry did not balance")
    end)
  end)
end)
