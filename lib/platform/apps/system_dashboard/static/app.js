(function () {
  ...
  // --- Action: execute or fall back to clipboard ---
  ...
  function onActionClick(e) {
    ...
    postExecute(aliasId, actionIndex).then(function (data) {
      if (data && data.ok) {
        // Real execution succeeded — show output
        var output = data.output || "";
        showOutput(label + " — output", output || "(no output)", false);
      } else if (data && data.error === "exec cap not available") {
        // Exec cap not granted — fall back to clipboard silently
        copyToClipboard(command, btn);
      } else {
        // Other error — show it
        var msg = (data && data.error) || "unknown error";
        showOutput(label + " — error", msg, true);
      }
    }).catch(function () {
