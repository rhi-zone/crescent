...
class Element {
  constructor(tagName) {
    ...
    this.dataset = new Proxy({}, {
      get(_t, k) {
        ...
        const attr = "data-" + camelToDash(k);
        return self._attrs[attr];
      },
      set(_t, k, v) {
        if (typeof k !== "string") return true;
        const attr = "data-" + camelToDash(k);
        self._attrs[attr] = v == null ? "" : String(v);
        return true;
      },
      deleteProperty(_t, k) {
        const attr = "data-" + camelToDash(k);
        delete self._attrs[attr];
        return true;
      },
      has(_t, k) {
        const attr = "data-" + camelToDash(k);
      ...
      getOwnPropertyDescriptor(_t, k) {
        const attr = "data-" + camelToDash(String(k));
        if (!(attr in self._attrs)) return undefined;
        return { enumerable: true, configurable: true, value: self._attrs[attr] };
      },
    });
    this._value = "";
    this._checked = false;
  ...
  /** Insert nodes after this element among its parent's children. */
  after(...nodes) {
    if (!this.parentNode) return;
    const parent = this.parentNode;
    const i = parent.childNodes.indexOf(this);
    // Insert in order after this node.
    let insertAt = i + 1;
    for (const node of nodes) {
      const n = typeof node === "string" ? new TextNode(node) : node;
      if (n.parentNode) n.parentNode.removeChild(n);
      n.parentNode = parent;
      parent.childNodes.splice(insertAt, 0, n);
      if (n.nodeType === 1) {
        const j = parent.children.indexOf(this);
        const ec = parent.children.indexOf(n);
        if (ec < 0) parent.children.splice(j < 0 ? parent.children.length : j + 1, 0, n);
      }
      insertAt++;
    }
  }
  /** Deep/shallow clone — enough for <template>.content.cloneNode(true). */
  cloneNode(deep) {
    const copy = new Element(this.tagName.toLowerCase());
    for (const k in this._attrs) copy.setAttribute(k, this._attrs[k]);
    if (deep) {
      for (const c of this.childNodes) copy.appendChild(c.cloneNode ? c.cloneNode(true) : new TextNode(c.textContent));
    }
    return copy;
  }
  ...
  /** Whether this element is in the live document tree. */
  get isConnected() {
    let n = this;
    while (n) {
      if (n === document.body || n === document._root) return true;
      n = n.parentNode;
    }
    return false;
  }
...
/** DocumentFragment — used for <template>.content. */
...
function parseHTML(html) {
  ...
  while ((m = tagRe.exec(html))) {
    ...
    if (closing) {
      ...
      if (tag === "template") {
        // <template>.content is a DocumentFragment; children parse into it.
        el.content = new DocumentFragment();
        if (!selfClose) {
          // Push a frag-like proxy so subsequent children append to el.content.
          stack.push({
            tagName: "TEMPLATE",
            _templateContent: el.content,
            appendChild(n) { return el.content.appendChild(n); },
          });
          continue;
        }
      }
...
class LocalStorageMock {
  ...
  getItem(k) { const v = this._data[String(k)]; return v === undefined ? null : v; }
...
class Window {
  constructor() {
    ...
    // Minimal localStorage: plain-object backing store, string keys/values.
...
export { Element, TextNode, DocumentFragment, MockEvent, parseHTML };
