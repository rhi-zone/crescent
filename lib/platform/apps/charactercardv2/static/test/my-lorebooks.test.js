...
let mock, errors, trapped, released, createdEntries;
beforeEach(() => {
  loadIndexHtml();
...
function stubCreateEntryEl(entry, handlers) {
  createdEntries.push({ entry, handlers });
  const el = document.createElement("div");
  el.className = "lorebook-entry";
...
function makeDeps() {
  return {
    ...
    createEntryEl: stubCreateEntryEl,
  };
}
...
describe("my-lorebooks.init", () => {
  ...
  test("selecting a book triggers GET entries and uses createEntryEl per entry", async () => {
    mock.respond("/api/user_lorebooks", () => ({
      status: 200,
    ...
    expect(createdEntries[0].entry.uid).toBe(1);
    expect(createdEntries[0].handlers.updateUrl).toBe("/api/user_lorebooks/entry/update");
    expect(createdEntries[0].handlers.deleteUrl).toBe("/api/user_lorebooks/entry/delete");
    expect(createdEntries[0].handlers.extraBody).toEqual({ book_id: "b1" });
  });
