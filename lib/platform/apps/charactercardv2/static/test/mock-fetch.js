...
export function installFetchMock() {
  ...
  globalThis.fetch = async function (path, opts) {
    ...
    return {
      ...
      async arrayBuffer() {
        if (result.arrayBuffer !== undefined) return result.arrayBuffer;
        if (result.text !== undefined) return new TextEncoder().encode(result.text).buffer;
        if (result.json !== undefined) return new TextEncoder().encode(JSON.stringify(result.json)).buffer;
