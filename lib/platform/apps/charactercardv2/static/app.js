...
(async function () {
  ...
  if (card) {
    ...
    // Set download filename on the export anchor.
    if (card.name) {
...
// Card header shortcut — same action, different entry point.
btnCardHeaderEdit.addEventListener("click", openCardEditor);
...
btnNewCard.addEventListener("click", async function () {
  try {
    ...
    try {
      const importRes = await fetch("/api/import-card/upload", {
        method: "POST",
