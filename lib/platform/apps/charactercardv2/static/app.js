...
if (btnCardHeaderBurger) {
  btnCardHeaderBurger.addEventListener("click", function () {
    ...
    if (open && cardHeaderEl) {
      // Find first focusable action — comma selectors aren't supported by
      // all DOM environments uniformly, so walk children of the actions row.
      const actionsRow = cardHeaderEl.querySelector(".card-header__actions");
      if (actionsRow) {
        for (const child of actionsRow.children) {
          const tag = child.tagName;
          if (tag === "BUTTON" || tag === "A") {
            /** @type {HTMLElement} */ (child).focus();
            break;
          }
        }
      }
