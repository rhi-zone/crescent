...
// against /api/user_lorebooks. Distinct from the card-side lorebook tab in
// the card editor (which stays in app.js for now). `createEntryEl` is
// shared with the card-side lorebook and is injected via deps rather than
// duplicated — the card-side will be extracted in a follow-up.
 ...
 *   releaseFocus: () => void,
 *   createEntryEl: (entry: LorebookEntry, handlers: EntryHandlers) => HTMLElement,
 * }} deps
 * @returns {{
 *   open: () => void,
 *   close: () => void,
...
export function init(deps) {
  ...
  const createEntryEl = deps.createEntryEl;
