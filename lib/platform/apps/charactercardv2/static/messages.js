...
export function init(deps) {
  ...
  async function ensureSiblings(messageId) {
    ...
    // Alias every sibling's id to the same entry. The DOM's data-id flips as
    // the user swipes; without aliasing, the second swipe sees a cache miss
    // and refetches.
    for (const s of data.swipes) siblingCache.set(s.id, entry);
    return entry;
  }
  ...
  function addSiblingToCache(messageId, sibling) {
    ...
    siblingCache.set(sibling.id, entry);
  }
