...
function M.dynamic(init_state, widget)
	return function(signal)
		...
		local state_sig_ = (state_sig --[[:! { get: () -> unknown, set: (unknown) -> nil, update: ((unknown) -> unknown) -> nil }]])

		-- Combined signal: {state=S, value=A}
		local combined = (R.computed(function()
			return { state = state_sig_.get(), value = signal.get() }
		end, { state_sig, signal }) --[[:! { get: () -> unknown, subscribe: (unknown) -> nil, focus: (unknown) -> unknown, narrow: (unknown) -> unknown, dispose: () -> nil }]])
