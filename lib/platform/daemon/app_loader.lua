...
function M.make(opts)
	...
	return function(app_id)
		...
		local app_typed = (app --[[:! { manifest: { caps: { [string]: unknown } | nil, default_entry: string | nil, entry: { [string]: unknown } | nil, meta: { description: string | nil, tags: { [integer]: string } | nil } | nil, name: string | nil, version: string | nil } | nil, path: string, chunks: { [integer]: { type: string, data: string } } | nil, entries: { [number]: { data: string, mode: number, mtime: number, name: string, size: number, typeflag: string } }, _dir_mode: boolean | nil }]])
