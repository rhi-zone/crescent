...
T.describe("db_cap", function()
	...
	T.it("opens an in-memory database", function()
		local cap, revoke = db_mod.db_cap(":memory:", { allow_write = true })
	...
	T.it("prepare + exec inserts rows", function()
		local cap = db_mod.db_cap(":memory:", { allow_write = true })
	...
	T.describe("readonly mode (allow_write = false)", function()
		...
		T.it("blocks writes", function()
			...
			T.eq(ok, nil)
			T.ok(err ~= nil, "write should fail when allow_write is false")
			cap.close()
		end)
	end)
...
T.describe("caps.db risk", function()
	T.it("writable db has low severity", function()
		local result = db_mod.risk({ type = "db", allow_write = true })
