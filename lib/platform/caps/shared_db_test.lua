...
local function setup_shared_db()
	...
	local cap = assert(db_mod.db_cap(path))
	cap.exec("INSERT INTO _sessions VALUES ('s1', 'app_a', 'Session A')")
	cap.exec("INSERT INTO _sessions VALUES ('s2', 'app_b', 'Session B')")
	cap.exec("INSERT INTO _sessions VALUES ('s3', 'app_a', 'Another A')")
	cap.exec("INSERT INTO _messages VALUES ('m1', 's1', 'app_a', 'user', 'hello from A')")
	cap.exec("INSERT INTO _messages VALUES ('m2', 's2', 'app_b', 'user', 'hello from B')")
	cap.exec("INSERT INTO _messages VALUES ('m3', 's1', 'app_a', 'assistant', 'reply from A')")
	cap.close()
	return path
end
...
T.describe("shared_db_cap: authorizer", function()
	...
	T.it("blocks direct INSERT on base table", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap.exec("INSERT INTO _sessions VALUES ('sx', 'app_a', 'hacked')")
		T.fail(ok)
		T.ok(type(err) == "string")
		cap.close()
		os.remove(path)
	...
	T.it("blocks DDL on main schema", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap.exec("CREATE TABLE evil (x TEXT)")
		T.fail(ok)
		T.ok(type(err) == "string")
		cap.close()
		os.remove(path)
	...
	T.it("blocks DROP TABLE", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap.exec("DROP TABLE sessions")
		T.fail(ok)
		T.ok(type(err) == "string")
		cap.close()
		os.remove(path)
	...
	T.it("blocks ATTACH", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap.exec("ATTACH DATABASE ':memory:' AS other")
		T.fail(ok)
		T.ok(type(err) == "string")
		cap.close()
		os.remove(path)
...
T.describe("shared_db_cap: writes through views", function()
	T.it("INSERT via view auto-sets app_id", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap.exec("INSERT INTO sessions (id, app_id, title) VALUES ('s4', 'ignored', 'New Session')")
		T.ok(ok, err)
		...
		local all = assert(host.query("SELECT app_id FROM _sessions WHERE id = 's4'"))
		T.eq(#all, 1)
		T.eq(all[1].app_id, "app_a")
		host.close()
		os.remove(path)
	end)
	...
	T.it("DELETE only affects own rows", function()
		local path = setup_shared_db()
		local cap_a = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap_a.exec("DELETE FROM sessions WHERE id = 's1'")
		T.ok(ok, err)
		...
		local host = assert(db_mod.db_cap(path))
		local b_rows = assert(host.query("SELECT id FROM _sessions WHERE app_id = 'app_b'"))
		T.eq(#b_rows, 1)
		T.eq(b_rows[1].id, "s2")
		host.close()
		os.remove(path)
	...
	T.it("cannot delete another app's rows", function()
		...
		cap_a.exec("DELETE FROM sessions WHERE id = 's2'")
		cap_a.close()
		-- s2 still exists.
		local host = assert(db_mod.db_cap(path))
		local rows = assert(host.query("SELECT id FROM _sessions WHERE id = 's2'"))
		T.eq(#rows, 1)
		host.close()
		os.remove(path)
	end)
	...
	T.it("UPDATE only affects own rows", function()
		local path = setup_shared_db()
		local cap_a = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		local ok, err = cap_a.exec("UPDATE sessions SET title = 'Updated' WHERE id = 's1'")
		T.ok(ok, err)
		...
		local host = assert(db_mod.db_cap(path))
		local b = assert(host.query("SELECT title FROM _sessions WHERE id = 's2'"))
		T.eq(b[1].title, "Session B")
		host.close()
		os.remove(path)
	end)
	...
	T.it("cannot update another app's rows", function()
		local path = setup_shared_db()
		local cap_a = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}))
		cap_a.exec("UPDATE sessions SET title = 'Hacked' WHERE id = 's2'")
		cap_a.close()
		-- s2 still has original title.
		local host = assert(db_mod.db_cap(path))
		local rows = assert(host.query("SELECT title FROM _sessions WHERE id = 's2'"))
		T.eq(rows[1].title, "Session B")
		host.close()
		os.remove(path)
	end)
...
T.describe("shared_db_cap: readonly", function()
	T.it("query works in readonly mode", function()
		local path = setup_shared_db()
		local cap, err = sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}, { readonly = true })
		T.ok(cap, err)
	...
	T.it("exec is not exposed in readonly mode", function()
		local path = setup_shared_db()
		local cap = assert(sdb.shared_db_cap(path, "app_a", {"sessions", "messages"}, { readonly = true }))
		T.eq(cap.exec, nil)
		cap.close()
		os.remove(path)
	end)
...
T.describe("shared_db_cap: concurrent caps", function()
	T.it("two apps have isolated views of the same db", function()
		...
		cap_a.exec("INSERT INTO sessions (id, app_id, title) VALUES ('s5', 'app_a', 'A only')")
