...
T.describe("caps.fs", function ()
	...
	local function write_file(path, content)
		local f = io.open(path, "wb")
		if not f then error("write_file: cannot open " .. path) end
		f:write(content)
		f:close()
	end
	...
	T.it("list_recursive returns nested relative paths", function ()
		local dir = make_tmpdir()
		os.execute("mkdir -p " .. dir .. "/sub")
		write_file(dir .. "/top.txt", "")
		write_file(dir .. "/sub/nested.txt", "")
		local cap = fs_cap({ root = dir })
		local paths, err = cap.list_recursive()
		T.ok(paths ~= nil, tostring(err))
		local found_top, found_sub_dir, found_nested = false, false, false
		for _, p in ipairs(paths) do
			if p == "top.txt" then found_top = true end
			if p == "sub" then found_sub_dir = true end
			if p == "sub/nested.txt" then found_nested = true end
		end
		T.ok(found_top, "top-level file listed")
		T.ok(found_sub_dir, "subdirectory itself listed")
		T.ok(found_nested, "nested file listed with relative path")
		os.execute("rm -rf " .. dir)
	end)

	T.it("stat reports size, mtime, and type for a file", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/hello.txt", "hello")
		local cap = fs_cap({ root = dir })
		local info, err = cap.stat("hello.txt")
		T.ok(info ~= nil, tostring(err))
		T.eq(info.size, 5)
		T.eq(info.type, "file")
		T.ok(type(info.mtime) == "number", "mtime is a number")
		os.execute("rm -rf " .. dir)
	end)

	T.it("stat reports type=directory for a directory", function ()
		local dir = make_tmpdir()
		os.execute("mkdir -p " .. dir .. "/sub")
		local cap = fs_cap({ root = dir })
		local info, err = cap.stat("sub")
		T.ok(info ~= nil, tostring(err))
		T.eq(info.type, "directory")
		os.execute("rm -rf " .. dir)
	end)

	T.it("mkdir is blocked without allow_mkdir", function ()
		local dir = make_tmpdir()
		local cap = fs_cap({ root = dir })
		local ok, err = cap.mkdir("newdir")
		T.ok(ok == nil)
		T.ok(err ~= nil and err:find("not granted"))
		os.execute("rm -rf " .. dir)
	end)

	T.it("mkdir works with allow_mkdir", function ()
		local dir = make_tmpdir()
		local cap = fs_cap({ root = dir, allow_mkdir = true })
		local ok, err = cap.mkdir("newdir")
		T.ok(ok == true, tostring(err))
		local info = cap.stat("newdir")
		T.ok(info ~= nil and info.type == "directory")
		os.execute("rm -rf " .. dir)
	end)

	T.it("delete is blocked without allow_delete", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/a.txt", "")
		local cap = fs_cap({ root = dir })
		local ok, err = cap.delete("a.txt")
		T.ok(ok == nil)
		T.ok(err ~= nil and err:find("not granted"))
		os.execute("rm -rf " .. dir)
	end)

	T.it("delete removes a file with allow_delete", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/a.txt", "")
		local cap = fs_cap({ root = dir, allow_delete = true })
		local ok, err = cap.delete("a.txt")
		T.ok(ok == true, tostring(err))
		T.eq(cap.stat("a.txt"), nil)
		os.execute("rm -rf " .. dir)
	end)

	T.it("delete removes an empty directory with allow_delete", function ()
		local dir = make_tmpdir()
		os.execute("mkdir -p " .. dir .. "/sub")
		local cap = fs_cap({ root = dir, allow_delete = true })
		local ok, err = cap.delete("sub")
		T.ok(ok == true, tostring(err))
		T.eq(cap.stat("sub"), nil)
		os.execute("rm -rf " .. dir)
	end)

	T.it("delete refuses a non-empty directory without recursive opt-in", function ()
		local dir = make_tmpdir()
		os.execute("mkdir -p " .. dir .. "/sub")
		write_file(dir .. "/sub/child.txt", "")
		local cap = fs_cap({ root = dir, allow_delete = true })
		local ok, err = cap.delete("sub")
		T.ok(ok == nil)
		T.ok(err ~= nil and err:find("recursive"), tostring(err))
		os.execute("rm -rf " .. dir)
	end)

	T.it("delete removes a non-empty directory with recursive=true", function ()
		local dir = make_tmpdir()
		os.execute("mkdir -p " .. dir .. "/sub/deeper")
		write_file(dir .. "/sub/child.txt", "")
		write_file(dir .. "/sub/deeper/leaf.txt", "")
		local cap = fs_cap({ root = dir, allow_delete = true })
		local ok, err = cap.delete("sub", { recursive = true })
		T.ok(ok == true, tostring(err))
		T.eq(cap.stat("sub"), nil)
		os.execute("rm -rf " .. dir)
	end)

	T.it("rename is blocked without allow_rename", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/a.txt", "")
		local cap = fs_cap({ root = dir })
		local ok, err = cap.rename("a.txt", "b.txt")
		T.ok(ok == nil)
		T.ok(err ~= nil and err:find("not granted"))
		os.execute("rm -rf " .. dir)
	end)

	T.it("rename moves a file within root with allow_rename", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/a.txt", "hi")
		local cap = fs_cap({ root = dir, allow_rename = true })
		local ok, err = cap.rename("a.txt", "b.txt")
		T.ok(ok == true, tostring(err))
		T.eq(cap.stat("a.txt"), nil)
		local info = cap.stat("b.txt")
		T.ok(info ~= nil and info.type == "file")
		os.execute("rm -rf " .. dir)
	end)

	T.it("attenuate narrows read/list off even though parent held them", function ()
		local dir = make_tmpdir()
		write_file(dir .. "/a.txt", "hi")
		local cap = fs_cap({ root = dir })
		local sub, err = cap.attenuate({ root = dir, allow_read = false, allow_list = false })
		T.ok(sub ~= nil, tostring(err))
		local content, rerr = sub.read("a.txt")
		T.ok(content == nil)
		T.ok(rerr ~= nil and rerr:find("not granted"))
		local names, lerr = sub.list()
		T.ok(names == nil)
		T.ok(lerr ~= nil and lerr:find("not granted"))
		os.execute("rm -rf " .. dir)
	end)

	T.it("attenuate cannot grant mkdir/delete/rename not held by parent", function ()
		local dir = make_tmpdir()
		local cap = fs_cap({ root = dir })
		local sub1, err1 = cap.attenuate({ root = dir, allow_mkdir = true })
		T.ok(sub1 == nil)
		T.ok(err1 ~= nil and err1:find("cannot grant mkdir"))
		local sub2, err2 = cap.attenuate({ root = dir, allow_delete = true })
		T.ok(sub2 == nil)
		T.ok(err2 ~= nil and err2:find("cannot grant delete"))
		local sub3, err3 = cap.attenuate({ root = dir, allow_rename = true })
		T.ok(sub3 == nil)
		T.ok(err3 ~= nil and err3:find("cannot grant rename"))
		os.execute("rm -rf " .. dir)
	end)

	T.it("attenuate can narrow mkdir/delete/rename held by parent", function ()
		local dir = make_tmpdir()
		local cap = fs_cap({ root = dir, allow_mkdir = true, allow_delete = true, allow_rename = true })
		local sub, err = cap.attenuate({ root = dir, allow_delete = false })
		T.ok(sub ~= nil, tostring(err))
		T.ok(sub.mkdir("newdir") == true)
		local ok, derr = sub.delete("newdir")
		T.ok(ok == nil)
		T.ok(derr ~= nil and derr:find("not granted"))
		os.execute("rm -rf " .. dir)
	end)
end)
