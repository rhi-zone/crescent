...
function M.new(ann_res, pool, types_mod, defs, kinds)
    local types = ann_res.types
    local lists = ann_res.lists

    -- Map an annotation primitive TAG to a value-model atom, or nil if outside
    -- the supported slice.
    local function atom_for_tag(tag)
    ...
    local lower  -- forward decl

    -- Lower a UNION. We only understand the nilable shape: a union of exactly
    -- one non-nil atom plus `nil` (in any order, any arity as long as the
    -- non-nil part is a single kind). Otherwise → unknown.
    local function lower_union(tid)
    ...
    -- first return slot (multi-return collapse is DEFERRED). A parameter or
    -- return outside the slice lowers to unknown (not an error — the arrow is
    -- still usable as a function kind).
    local function lower_function(tid)
        local t = types:get(tid)
        ...
        for i = 0, pl - 1 do
            ...
            params[i + 1] = lower(pid) or kinds.UNKNOWN
        end
        local rs = types_mod.fn_returns_start(t)
        local rl = types_mod.fn_returns_len(t)
        local ret
        if rl >= 1 then
            ret = lower(lists:get(rs)) or kinds.UNKNOWN
        else
    ...
    -- lower: annotation type_id -> value-model type, or nil if unrepresentable
    -- (nil only used internally; the public entry coerces nil to unknown).
    lower = function(tid)
        if tid == nil or tid < 0 then return nil end
        local t = types:get(tid)
        local tag = t.tag

        local atom = atom_for_tag(tag)
        ...
        if tag == defs.TAG_UNION then
            return lower_union(tid)
        end

        if tag == defs.TAG_FUNCTION then
            return lower_function(tid)
        end

        -- Everything else (named aliases, intersections, generics, match, ...)
        -- is outside the supported slice. Demand a decision elsewhere: unknown.
        return nil
    end
