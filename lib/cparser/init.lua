...
expandMacros = function(options, macros, tokens, ...)
  ...
  local function prependTokens(pti)
    local pos = 1 + #prepend
    for s, n in pti do table.insert(prepend, pos, { s, n }) end
  end
  local ti = function()
    if #prepend > 0 then
    ...
    while def[i] do
      if isBlank(def[i]) then
        ...
        local function updateJandK()
          if j <= i then
            j = i
            repeat j = j + 1 until def[j] == nil or not isBlank(def[j])
          end
          if k <= j then
            k = j
            repeat k = k + 1 until def[k] == nil or not isBlank(def[k])
          end
        end
        updateJandK()
        -- alternatives
        ...
        elseif def[j] == '##' and def[k] and not inDirective then
          ...
          local function addToU(s)
            if nargs[s] then
              for _, v in ipairs(uargs[s]) do u[1 + #u] = v end
            else
              u[1 + #u] = s
            end
          end
...
local function evaluateCppExpression(options, tokenIterator, n, resolver)
  ...
  local binaryOps = {
    ...
    ["/"] = function(a, b)
      xassert(b ~= 0, options, n, "division by zero"); return math.floor(a / b)
    end,
    ["%"] = function(a, b)
      xassert(b ~= 0, options, n, "division by zero"); return a % b
    end,
    ["+"] = function(a, b) return a + b end,
    ["-"] = function(a, b) return a - b end,
...
processDirectives = function(options, macros, lines, ...)
  ...
  local function doMacroLines(lines, stop)
    while true do
      ...
      if #ss > 0 then
        lines[1 + #lines] = ss; lines[1 + #lines] = n
      end
...
local function parseDeclarations(options, globals, tokens, ...)
  ...
  parseStruct = function(symtable, context, abstract_, nn)
    ...
    local ttag, tnam
    if isName(tok) then
      ttag = tok; tnam = kind .. ' ' .. ttag; nn.newtype = true; ti()
    end
    nn.attr = collectAttributes(nn.attr)
