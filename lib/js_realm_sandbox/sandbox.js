 ...
 *
 * This is the JS-LANGUAGE allow-list — the set of names the ECMAScript
 * spec defines on the global object. Anything the host (browser, node,
 * bun, deno) injects beyond the language spec — `document`, `window`,
 * `navigator`, `indexedDB`, `caches`, `WebSocket`, `Worker`, `crypto`,
 * `performance`, `fetch`, `setTimeout`, `console`, etc. — is NOT on
 * this list and is therefore stripped by the structural walk below.
 *
 * Deliberately removed even though defined by ECMAScript:
 *   - `eval`, `Function` — dynamic code construction.
 *   - `Reflect`, `Proxy` — reflection + meta-object protocol.
 *   - `WeakRef`, `FinalizationRegistry` — GC observation.
 *   - `Atomics`, `SharedArrayBuffer` — shared memory + Spectre vectors.
 *   - `Iterator`, `AsyncIterator` (stage 3 helpers) — exposed below
 *     spec deferral; allow-list will be revisited if/when these stabilise.
 *   - `WebAssembly` — host-side surface, can come back as a cap.
 *   - `Intl` — borderline (in the ECMA-402 spec, not ECMA-262). The
 *     surface is large and ships locale data that is host-dependent;
 *     default-strip and re-introduce as a cap if a pack needs it.
 *
 * `console` is host-defined (Web Platform, not ECMAScript) — stripped
 * by default; a future cap can expose a tamed `console.log`.
 */
const GLOBAL_ALLOWLIST = [
  ...
  // URI codec helpers — pure string transforms, no I/O.
...
export function installLockdown(opts) {
  ...
  // --- Step 3: curate globalThis (allow-list walk). ---
  //
  // Walk every own property of globalThis (names AND symbols) and remove
  // anything not on the JS-language allow-list. We cannot replace
  // globalThis with a fresh Object.create(null) — the language gives no
  // mechanism — so we prune in place. The walk is structural: hosts ship
  // new APIs all the time, and an allow-list closes them by absence
  // rather than by enumerating dangerous names. Browser-platform APIs
  // (`document`, `WebSocket`, `indexedDB`, `crypto`, `setTimeout`, …)
  // and host-injection symbols are gone after this step.
  //
  // Removal technique per slot:
  //   1. `delete globalThis[k]` (works for configurable slots).
  //   2. If that fails (non-configurable), neutralise the value to
  //      `undefined` via defineProperty.
  //   3. If even that fails (non-configurable + non-writable + accessor
  //      that can't be redefined), leave it alone and rely on the fact
  //      that the host shipped no path to escape from a sealed slot we
  //      cannot describe.
  ...
  /** @param {object} obj @param {string|symbol} key */
  function stripOrNeutralise(obj, key) {
    let deleted = false;
    try { deleted = delete obj[key]; } catch (_e) { deleted = false; }
    if (deleted) return;
    // Slot is non-configurable; force it to undefined if writable, else
    // try defineProperty with a non-writable undefined descriptor.
    try {
      O_defineProperty(obj, key, {
        value: undefined,
        writable: false,
    ...
    } catch (_e) { /* sealed beyond our reach; leave as-is */ }
  }

  const ownNames = O_getOwnPropertyNames(globalThis);
  for (let i = 0; i < ownNames.length; i++) {
    const k = ownNames[i];
    if (!allow[k]) stripOrNeutralise(globalThis, k);
  }
  // Own symbols on globalThis. Hosts sometimes plant inspection /
  // identity symbols (e.g. node:vm's context tag, bun's internal tags).
  // None of these are on the JS-language allow-list — strip all.
  const ownSyms = O_getOwnPropertySymbols(globalThis);
  for (let i = 0; i < ownSyms.length; i++) {
    stripOrNeutralise(globalThis, ownSyms[i]);
  }
