...
function buildBoundedWrappers(cap) {
  ...
  function boundedRepeat(count) {
    const n = +count;
    if (!Number.isFinite(n) || n < 0) {
      // Let the original handle the invalid-input error path.
      return O_repeat.call(this, count);
    }
  ...
  function boundedPadStart(targetLength, padString) {
    const t = +targetLength;
    if (Number.isFinite(t) && t > cap) {
      rangeFail("String.prototype.padStart", t, cap);
    }
    return O_padStart.call(this, targetLength, padString);
  }
  function boundedPadEnd(targetLength, padString) {
    const t = +targetLength;
    if (Number.isFinite(t) && t > cap) {
      rangeFail("String.prototype.padEnd", t, cap);
    }
    return O_padEnd.call(this, targetLength, padString);
  }
  ...
  function boundedArrayFrom(items, mapfn, thisArg) {
    ...
    if (items != null && typeof items === "object" &&
      ...
      let count = 0;
      const wrappedMap = function (v, i) {
        count = i + 1;
        if (count > cap) rangeFail("Array.from", count, cap);
        return mapfn ? mapfn.call(thisArg, v, i) : v;
      };
      return O_Array_from.call(O_Array, items, wrappedMap, thisArg);
    }
    return O_Array_from.call(O_Array, items, mapfn, thisArg);
  }

  // Array.of — argument count.
  function boundedArrayOf(...args) {
    if (args.length > cap) rangeFail("Array.of", args.length, cap);
    return O_Array_of.apply(O_Array, args);
  }

  // String.fromCharCode / fromCodePoint — argument count.
  function boundedFromCharCode(...args) {
    if (args.length > cap) {
      rangeFail("String.fromCharCode", args.length, cap);
    }
    return O_fromCharCode.apply(O_String, args);
  }
  function boundedFromCodePoint(...args) {
    if (args.length > cap) {
      rangeFail("String.fromCodePoint", args.length, cap);
    }
    return O_fromCodePoint.apply(O_String, args);
  }
  ...
  function boundedJoin(separator) {
    ...
    return O_join.call(this, separator);
  }

  // Array.prototype.flat — bound resulting element count.
  function boundedFlat(depth) {
    const result = O_flat.call(this, depth);
    if (result.length > cap) {
      rangeFail("Array.prototype.flat", result.length, cap);
    }
    return result;
  }
  function boundedFlatMap(mapper, thisArg) {
    const result = O_flatMap.call(this, mapper, thisArg);
    if (result.length > cap) {
  ...
  // JSON wrappers.
  function boundedJSONStringify(value, replacer, space) {
    const out = O_JSON_stringify.call(JSON, value, replacer, space);
    if (typeof out === "string" && out.length > cap) {
      rangeFail("JSON.stringify", out.length, cap);
    }
    return out;
  }
  function boundedJSONParse(text, reviver) {
    if (typeof text === "string" && text.length > cap) {
      rangeFail("JSON.parse", text.length, cap);
    }
    return O_JSON_parse.call(JSON, text, reviver);
  }
...
function buildStringMatchWrappers() {
  function safeMatch(pat) {
    if (typeof pat === "string") checkPattern(pat);
    return O_String_match.call(this, pat);
  }
  function safeMatchAll(pat) {
    if (typeof pat === "string") checkPattern(pat);
    return O_String_matchAll.call(this, pat);
  }
  function safeSearch(pat) {
    if (typeof pat === "string") checkPattern(pat);
    return O_String_search.call(this, pat);
  }
