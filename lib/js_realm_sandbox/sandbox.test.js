...
// Ambient-API survey.
//
// For each name on the survey corpus, assert it evaluates to `undefined`
// in the locked-down realm. The corpus covers Web Platform APIs and
// other host-injected globals that a deny-list bootstrap would miss
// unless enumerated. The allow-list walk over `globalThis` is supposed
// to remove all of these by structural absence — this survey is the
// test that the structural property holds.
//
// Tolerated outcome: "name was never defined on globalThis" is
// equivalent to "name is undefined after lockdown". `typeof X` does not
// throw a ReferenceError for unbound identifiers in any mode, so the
// assertion `typeof X === "undefined"` works whether the host shipped
// the global or not. The point of the survey is: IF the host ships any
// of these, lockdown must strip it.
//
// For nested members like `navigator.storage`, the parent (`navigator`)
// must itself be undefined after lockdown — so testing the parent is
// already a stronger assertion than testing the leaf. We test the leaf
// path defensively in case a future host structures things differently
// (e.g. exposes a stub `navigator` with just `userAgent`).
// =====================================================================

/**
 * Build a test case asserting `typeof <expr> === "undefined"` in the
 * sandbox. `expr` is a JS expression; nested paths like
 * `navigator.storage` need the leading parent guarded so a missing
 * parent doesn't throw a TypeError under property access — we evaluate
 * via a try/catch and treat any thrown error as "name not defined".
 * @param {string} expr
 * @returns {Case}
 */
function ambientCase(expr) {
  // Use a try/catch around `typeof expr` for leaf access; for bare
  // identifiers `typeof` already handles undeclared names. For
  // member-access expressions, a TypeError from "cannot read
  // properties of undefined" is itself proof the parent was stripped
  // — accept it as success.
  return {
    name: "ambient: " + expr + " is undefined",
    source:
      "try { return typeof (" + expr + ") === 'undefined'; }\n" +
      // Any thrown error during evaluation — ReferenceError for a
      // missing parent identifier, TypeError for member access on
      // `undefined` — is itself proof the surface is gone.
      "catch (e) { return e instanceof ReferenceError || e instanceof TypeError; }",
  };
}

/** @type {{ [category: string]: string[] }} */
const AMBIENT_SURVEY = {
  storage: [
    "indexedDB", "localStorage", "sessionStorage", "caches", "cookieStore",
    "navigator.storage",
  ],
  network: [
    "fetch", "XMLHttpRequest", "WebSocket", "EventSource",
    "BroadcastChannel", "RTCPeerConnection", "MessageChannel",
    "MessagePort", "Beacon", "navigator.sendBeacon",
  ],
  workers: [
    "Worker", "SharedWorker", "navigator.serviceWorker",
  ],
  dom: [
    "document", "Element", "Node", "HTMLElement", "HTMLDocument",
    "Document", "window", "parent", "top", "frames", "self",
    "location", "history", "screen", "Image", "Audio", "Video",
    "MediaSource", "customElements", "Range", "MutationObserver",
    "IntersectionObserver", "ResizeObserver", "PerformanceObserver",
  ],
  sensor: [
    "navigator.bluetooth", "navigator.usb", "navigator.serial",
    "navigator.hid", "navigator.geolocation", "navigator.mediaDevices",
    "navigator.clipboard", "Notification", "WakeLock", "Vibration",
    "Gamepad", "Bluetooth", "USB", "Serial", "HID",
  ],
  crypto_encoding: [
    "crypto", "SubtleCrypto", "Crypto", "CryptoKey",
  ...
  misc: [
    "console", "alert", "confirm", "prompt", "print",
    "structuredClone", "reportError",
    "Intl", "Atomics", "SharedArrayBuffer", "WebAssembly",
...
const ambientCategories = Object.keys(AMBIENT_SURVEY);
for (let ci = 0; ci < ambientCategories.length; ci++) {
  const category = ambientCategories[ci];
  const names = AMBIENT_SURVEY[category];
  for (let i = 0; i < names.length; i++) {
    const c = ambientCase(names[i]);
