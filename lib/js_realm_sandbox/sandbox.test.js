...
const CORPUS = [
  ...
  // uses `new RegExp(...)`). We pass a RegExp literal as the first arg
  // so the wrapper skips the string-pattern validator path (the
  // validator's reliance on `Object.create(null)` is a separate
  // preexisting issue: `Object.create` is removed by lockdown, so any
  // string pattern currently bubbles a TypeError from inside the
  // validator. Tracked separately; outside the bind-patch scope.).
