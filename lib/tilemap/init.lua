...
function M.new(width, height, opts)
  if type(width) ~= "number" or width < 1 then
    return nil, "width must be a positive number"
  end
  if type(height) ~= "number" or height < 1 then
    return nil, "height must be a positive number"
  end
  opts = opts or {}
  local default = opts.default_tile or 0
  local tiles = {}
  ...
  return setmetatable({
    ...
    _height = height,
    _tiles = tiles,
    _tile_size = opts.tile_size or 16,
  }, TileMap)
end
...
function TileMap:find(value)
  ...
  for i = 1, w * h do
    if tiles[i] == value then
      local idx = i - 1
      result[#result + 1] = { idx % w, math.floor(idx / w) }
    end
  end
  return result
end
...
function TileMap:astar(sx, sy, gx, gy, opts)
  opts = opts or {}
  local w = self._width
  local passable = opts.passable or function(t) return t ~= 0 end
  local diagonal = opts.diagonal or false
  local hname = opts.heuristic or "manhattan"
  ...
  local dirs = diagonal and dirs8 or dirs4

  while not open:empty() do
    local cur_k = open:pop()
    if closed[cur_k] then goto continue end
    closed[cur_k] = true

    if cur_k == goal_k then
      -- reconstruct path
      local path = {}
      local k = cur_k
      while k do
        local px = k % w
        local py = math.floor(k / w)
    ...
    local cur_x = cur_k % w
    local cur_y = math.floor(cur_k / w)

    for _, d in ipairs(dirs) do
      local nx, ny = cur_x + d[1], cur_y + d[2]
...
function M.random_rooms(width, height, opts)
  ...
  local map = M.new(width, height, { default_tile = 0 })
  local rooms = {}

  for _ = 1, num_rooms do
    local rw = rng:range(min_size, max_size)
    local rh = rng:range(min_size, max_size)
    local rx = rng:range(1, width - rw - 1)
    local ry = rng:range(1, height - rh - 1)
...
function M.deserialize(str)
  local w, h, hex = str:match("^(%d+):(%d+):(.+)$")
  if not w then return nil, "invalid serialized tilemap" end
  w = tonumber(w)
  h = tonumber(h)
  if w < 1 or h < 1 then return nil, "invalid dimensions" end
  local expected = w * h
  if #hex ~= expected * 2 then return nil, "data length mismatch" end
  local map = M.new(w, h)
  local tiles = map._tiles
  for i = 1, expected do
    local byte = tonumber(hex:sub(i * 2 - 1, i * 2), 16)
