...
local function is_lifetime_expired(pool, entry, now)
  ...
  return (now - entry.created_at) > lifetime
end
...
local function destroy_entry(pool, entry)
  if pool._destroy then
    local ok, err = pcall(pool._destroy, entry.conn)
    if not ok then
  ...
  while #idle > 0 do
    ...
    if is_lifetime_expired(self, entry, now) then
      ...
      if self._validate then
        local ok, result = pcall(self._validate, entry.conn)
        valid = ok and result
      end
...
function pool_mt:release(conn)
  ...
  if self._closed then
    ...
    self._conn_meta[conn] = nil
    if meta then
      local entry = { conn = conn, created_at = meta.created_at, idle_since = 0 }
      destroy_entry(self, entry)
    else
      if self._destroy then pcall(self._destroy, conn) end
  ...
  if self._total <= self._max_size then
    local entry = { conn = conn, created_at = created_at, idle_since = now }
...
function pool_mt:warm()
  ...
  while self._total < target and self._total < self._max_size do
    ...
    self._stats.created = self._stats.created + 1
    self._conn_meta[conn_or_err] = { created_at = now }
    local entry = { conn = conn_or_err, created_at = now, idle_since = now }
    self._idle[#self._idle + 1] = entry
  end
end
...
function M.new(opts)
  ...
  self._destroy      = opts.destroy
  self._validate     = opts.validate
  self._min_size     = opts.min_size or 0
  ...
  self._total     = 0   -- total connections (idle + active)
  self._closed    = false
