...
local function pipe_write_all(fd, s)
    local buf   = (ffi.cast("const char *", s) --[[: unknown]]) --[[:! number]]
    local total = #s
    local done  = 0
    while done < total do
...
local function pipe_read_all(fd)
    ...
    local buf    = (ffi.new("uint8_t[4096]") --[[: unknown]]) --[[:! number]]
    local chunks = {}
    while true do
        ...
        elseif n == 0 then
            ...
            chunks[#chunks + 1] = ffi.string(buf --[[:! Ptr<integer>]], n)
        end
    end
...
function M.dump_one(filename, parent_scope, opts)
    ...
    -- Manual insertion sort (avoids table.sort generic V-bleeding).
    for i = 2, #list do
        local key = list[i] --[[:! { name: string, type: string }]]
        local j = i - 1
        while j >= 1 and (list[j] --[[:! { name: string, type: string }]]).name > key.name do
            list[j + 1] = list[j]
            j = j - 1
        end
        list[j + 1] = key
