...
- function-type arms: `() -> %R` — **implemented** (match.lua, 2026-03-29). `() -> %R` with 0 params in the pattern matches ANY function (param count not constrained). R binds to single return type, or tuple for multi-return. `(A, B) -> %R` with explicit params still requires exact param count match.
