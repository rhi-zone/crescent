...
- field-entry distribution pattern: `{ [%K]: %V }` — **was `{ ...[%K]: %V }`, syntax changed** (see docs/capture-sigil-spec.md). `{ [K]: V }` denotes a field entry (single key-value pair); with captures K and V, the pattern distributes over ALL field entries in the input (named fields and indexers), evaluating the result expression once per entry and unioning results. Named fields: K = integer literal for integer-named fields (arrays), string literal for string-named fields. Indexers: K/V are the indexer types. `{ x: integer, y: string } => (K, V)` → `("x", integer) | ("y", string)`. Empty closed table → never (no iterations). Value types are widened. Enables `Keys<T>`, `Values<T>`, `PairsReturn<T>`, `IpairsReturn<T>` as pure library aliases. `IpairsReturn` uses `match K { number => (integer, V), _ => never }` to filter. Does NOT expose optional/readonly flags — flag manipulation requires `$EachField`. `{ ["foo"]: %V }` is the predicate form (matches only tables with a "foo" field). Implementation: ann.lua detects TAG_CAPTURE key → creates TAG_PAT_ALL_FIELDS; concrete key → predicate match. `{ ...[%K]: %V }` syntax **removed**.
...
flag/descriptor access — not replaceable by `{ [%K]: %V }` distribution).

**`{ [%K]: %V }` distribution vs `$EachField` — complementary, not redundant:**
- `{ [%K]: %V }` — **distribution**: iterates field entries, evaluates a result
  expression per entry, unions results. For PairsReturn, Keys, Values.
