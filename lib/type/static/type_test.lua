...
assert.describe("parse: table constructors", function()
    ...
    assert.it("positional fields", function()
        ...
        assert.eq(f0.data[2], defs.TFIELD_POSITIONAL)
    end)
    assert.it("named fields", function()
        ...
        local f0 = r.nodes:get(r.lists:get(tbl.data[0]))
        assert.eq(f0.kind, defs.NODE_TABLE_FIELD)
        assert.eq(f0.data[2], defs.TFIELD_NAMED)
        -- data[0] is the intern id of the field name itself ("x"), not a node.
        assert.eq(intern.get(r.pool, f0.data[0]), "x")
    end)
    assert.it("computed fields", function()
        ...
        local tbl = r.nodes:get(r.lists:get(ret.data[0]))
        local f0 = r.nodes:get(r.lists:get(tbl.data[0]))
        assert.eq(f0.data[2], defs.TFIELD_COMPUTED)
    end)
...
assert.describe("parse: control flow", function()
    ...
    assert.it("if/elseif/else", function()
        ...
        local stmt = first_stmt(r)
        assert.eq(stmt.kind, defs.NODE_IF_STMT)
        -- IF + ELSEIF clauses only (2); else block lives in data[2]/data[3].
        assert.eq(stmt.data[1], 2)
        assert.eq(bit.band(stmt.flags, defs.FLAG_HAS_ELSE), defs.FLAG_HAS_ELSE)
        assert.eq(stmt.data[3], 1)  -- else block: 1 stmt (z())
    end)
    assert.it("if/then with no else clears FLAG_HAS_ELSE", function()
        local r = p("if x then return 1 end")
        local stmt = first_stmt(r)
        assert.eq(stmt.kind, defs.NODE_IF_STMT)
        assert.eq(bit.band(stmt.flags, defs.FLAG_HAS_ELSE), 0)
    end)
...
local y = x
    ...
    -- HM Phase 2: field-value-type propagation through HM bounds. Body's
    -- recorded C_ARITH ops are re-emitted at each call site with operand TVs
    -- mapped to the per-call fresh instances, so caller field types flow
    -- into body operations.
