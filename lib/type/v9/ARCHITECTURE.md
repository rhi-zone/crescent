...
| **Annotation seam** | `annot/init.lua` | the `--:`/`--::` type-grammar STRING -> a v9-owned tree (NOT a lift of legacy `ann.lua`). Parses: atoms (integer -> number; literals UP-approximated to base atoms), `unknown`/`never`, unions, records (optional `x?:` / `readonly` / open `...`), function types (named params, multi-return, `...T` rest, `x is T` predicates as boolean results, `-> (T, U, ...)` result-OPEN tuples), `--:: Name = T` aliases (recursive ones unroll ONCE and cut to unknown at the knot — the clip discipline; expansion is MEMOIZED per parse so mutually-recursive alias graphs stay linear — the At is a DAG — with a hard expansion BUDGET as the honest `alias-budget` backstop, never a hang), `--:: declare name = T` global declarations (classified for the globals seam), index signatures `{ [string]: T }` / `{ [number]: T }` / the `T[]` sugar as REAL index-bounded records (key types beyond string/number are the `index-signature-key` bucket), and the CALL-SITE INSTANTIATION intrinsics `$Elem<N>` / `$Values<N>` / `$Keys<N>` / `$Arg<N>` (a declared fn's result position computed per call from argument N — how pairs/ipairs carry element types without generics). Everything else is a NAMED per-feature bucket (`unsupported:annotation-{generic, intrinsic, typeof, intersection, complement, index-signature-key, meta-slot, cdata, any, unknown-name, alias-budget, ...}`); `--:: require "mod"` is the TYPE-ONLY import (classified with its literal module name — lowering merges the dep's exported alias env under the file's own aliases), and runtime `require(...)` resolves through the stdlib declaration's `$Require<1>` instantiation (module summaries via the injected session); a require without a literal module name, an unresolvable module, or a session-less check stays the honest `unsupported:cross-module` boundary. |
...
| **Runner** | `check.lua` | `check_source`/`check_file` (caps-first) -> policy-stamped, position-sorted diags + the file's MODULE SUMMARY (summary.lua's At-data form). THE POWER DIAL: named policy rules (`op-mismatch` / `call-non-function` / `call-mismatch` / `field-write-mismatch` / `annotation-mismatch` / `cast-mismatch` / `force-cast` = error; `missing-field` / `use-before-narrow` = warn; `new-field-on-write` = off — the ONE named open-record concession; `unannotated-module-boundary` = off — the visibility dial over pure-inference summaries crossing the module boundary; per-bucket `unsupported:*`; "off" suppresses) — strictness is owner-decidable data, in one place. `check.session` is the CROSS-MODULE seam: demand-driven summaries over injected caps (modules.lua resolves dot->slash with the init.lua fallback through the ONE read_file cap), cached per path, cycles cut by an in-progress set into `unsupported:cross-module-cycle` — never a hang. |
| **Smoke** | `smoke.lua` | tool entry: whole-`lib/` totality run + diagnostic histogram (the coverage roadmap). The lower pass is deliberately DEP-LESS (totality needs no summaries); the full pass runs through ONE shared cross-module session — each file solved at most once per run. July 2026, with cross-module summaries: 1,566 files, ZERO crashes, full solve ~110s (~23s before this increment — the added cost is the DOM-shaped import environments, see the slice narrative; peak LuaJIT heap ~19GB on the heaviest importer). |
...
With CROSS-MODULE SUMMARIES (July 2026) the totals moved 422,506 ->
136,113 at ~110s, zero crashes (1,566 files; was ~23s — the cost note
below). `require` now yields the dep module's SUMMARY — its solved return
type exported as At DATA (summary.lua: val_to_at, memoized on Val
identity; the importer's at_val re-mints param cells in its OWN graph, so
cells never cross graphs) — through a demand-driven session
(check.session: per-path cache, ONE shared session per smoke run, cycles
cut by an in-progress set into `unsupported:cross-module-cycle`, 33). The
wiring is DECLARATION-driven: stdlib declares `require = (module: string)
-> $Require<1>` and lowering resolves the inst from the literal argument
through the injected deps seam — no name is keyed (pinned by a test
wiring a non-require name through the same declaration form; the old
`callee.name == "require"` lowering branch is DELETED). `--:: require
"mod"` imports the dep's exported alias env (own aliases shadow;
transitively closed). The two boundary buckets collapsed: cross-module
3,209 -> 207 (dynamic names + genuinely unresolvable modules — LuaJIT
builtins like `ffi`), annotation-unknown-name 3,461 -> 2,449. The typed
module surface moved everything downstream: use-before-narrow 376,464 ->
62,890 (required module values are no longer unknown), and the conversion
is REAL findings surface — op-mismatch 6,095 -> 26,621 and call-mismatch
4,355 -> 10,301 (sampled: dominated by `nil | T` module-function results
used unguarded — the same honest border class the index discipline
surfaced — plus the known conditional-tier-field class: `M._tier` written
under branches joins away, so summaries miss it), missing-field 6,507,
call-non-function 1,728, unconstrained-param 10,559.
`unannotated-module-boundary` (default off) is the visibility dial over
PURE-INFERENCE summaries crossing the boundary. COST, stated: ~110s (was
~23s) and ~19GB peak LuaJIT heap on the heaviest importers — the
DOM-shaped `lib.js_types` import environment (lib/web/html, the
reactive_dom trees). Landing the increment surfaced a LATTICE-OP
pathology: join/meet/equal walked two distinct DAGs per PATH
(exponential — the smoke hung on lib/web/html), fixed in the increment's
own leq-memo pattern (pairwise memos + EXACT absorption dedup so the
engine's no-change firing returns the operand identity + per-file reset;
lattice.lua states why LuaJIT's non-ephemeron weak tables cannot express
the cache). The principled cost fix is Val INTERNING (hash-consing) —
recorded in TODO as substrate. Engine untouched (EIGHTH consecutive data
point).
