...
T.describe("v9 lattice — structural open records", function()
    ...
    T.it("has_atom admission (the atom-metatable `__index` mechanism's probe)", function()
        T.eq(L.has_atom(L.single("string"), "string"), true, "string alone admits string")
        T.eq(L.has_atom(L.of({ "string", "nil" }), "string"), true, "string|nil admits string")
        T.eq(L.has_atom(L.record_of({}), "string"), false, "records admit no atom")
        T.eq(L.has_atom(L.single("number"), "string"), false, "number does not admit string")
    end)
