...
T.describe("v9 annot — `--::` declaration classification", function()
    ...
    T.it("`require \"mod\"` is the type-only import (modname extracted); other forms are annotation-<kw>", function()
        local kind, mod = annot.classify_decl("require \"lib.foo\"")
        T.eq(kind, "require", "require kind")
        T.eq(mod, "lib.foo", "module name")
        local k2, m2 = annot.classify_decl("require('lib.bar')")
        T.eq(k2, "require", "parenthesized/single-quote form")
        T.eq(m2, "lib.bar", "module name")
        -- a require line without a literal module stays the honest boundary.
        local k3, f3 = annot.classify_decl("require somevar")
        T.eq(k3, "feature", "feature")
        T.eq(f3, "cross-module", "cross-module")
        local _, f4 = annot.classify_decl("augment string { upper: () -> string }")
        T.eq(f4, "annotation-augment", "augment")
    end)
