...
-- ── sub-phase K6c: NODE_BINARY_EXPR / NODE_UNARY_EXPR / NODE_CAST_EXPR ───

local function bin(op, lhs, rhs)
	return { tag = defs.NODE_BINARY_EXPR, op = op, lhs = lhs, rhs = rhs }
end
local function unary(op, operand)
	return { tag = defs.NODE_UNARY_EXPR, op = op, operand = operand }
end
local function cast(expr, annotation, force)
	return { tag = defs.NODE_CAST_EXPR, expr = expr,
		annotation = annotation, force = force or false }
end
local function lit_int_k(n)
	return { tag = defs.NODE_LITERAL, lit_kind = defs.LIT_INTEGER, value = n }
end
local function lit_num(n)
	return { tag = defs.NODE_LITERAL, lit_kind = defs.LIT_NUMBER, value = n }
end
local function lit_str_k(s)
	return { tag = defs.NODE_LITERAL, lit_kind = defs.LIT_STRING, value = s }
end
local function lit_bool_k(b)
	return { tag = defs.NODE_LITERAL, lit_kind = defs.LIT_BOOLEAN, value = b }
end

T.describe("walker sub-phase K6c: NODE_BINARY_EXPR arithmetic", function()
	T.it("integer + integer → integer", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("+", lit_int_k(1), lit_int_k(2)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("integer + number → number", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("+", lit_int_k(1), lit_num(2.0)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("number * number → number", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("*", lit_num(1.5), lit_num(2.0)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("integer / integer → number (always)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("/", lit_int_k(6), lit_int_k(2)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("integer ^ integer → number (always)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("^", lit_int_k(2), lit_int_k(3)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("integer % integer → integer", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("%", lit_int_k(7), lit_int_k(3)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("integer + string rejects with E_OP_TYPE", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("+", lit_int_k(1), lit_str_k("x")), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_OP_TYPE)
	end)
end)

T.describe("walker sub-phase K6c: NODE_BINARY_EXPR comparison", function()
	T.it("integer < integer → boolean", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("<", lit_int_k(1), lit_int_k(2)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.boolean)
	end)
	T.it("string <= string → boolean", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("<=", lit_str_k("a"), lit_str_k("b")), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.boolean)
	end)
	T.it("integer < string rejects", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("<", lit_int_k(1), lit_str_k("x")), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
...
T.describe("walker sub-phase K6c: NODE_BINARY_EXPR equality", function()
	T.it("integer == string → boolean (any operands)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("==", lit_int_k(1), lit_str_k("x")), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.boolean)
...
T.describe("walker sub-phase K6c: NODE_BINARY_EXPR concat", function()
	T.it("string .. string → string", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("..", lit_str_k("a"), lit_str_k("b")), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.string_)
	end)
	T.it("number .. string → string", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("..", lit_num(1.0), lit_str_k("x")), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.string_)
	end)
	T.it("boolean .. string rejects", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("..", lit_bool_k(true), lit_str_k("x")), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_OP_TYPE)
	end)
...
T.describe("walker sub-phase K6c: NODE_BINARY_EXPR logical", function()
	T.it("integer and string → union(integer, string)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("and", lit_int_k(1), lit_str_k("x")), E.new(), s)
		T.eq(err, nil)
		T.ok(ty ~= nil)
		T.eq(ty.tag, "union")
	end)
	T.it("integer or string → union(integer, string)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(bin("or", lit_int_k(1), lit_str_k("x")), E.new(), s)
		T.eq(err, nil)
		T.ok(ty ~= nil)
		T.eq(ty.tag, "union")
	end)
...
T.describe("walker sub-phase K6c: NODE_UNARY_EXPR", function()
	T.it("unary -integer → integer", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("-", lit_int_k(3)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("unary -number → number", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("-", lit_num(3.14)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("unary -string rejects", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("-", lit_str_k("x")), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_OP_TYPE)
	end)
	T.it("not <anything> → boolean", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("not", lit_int_k(1)), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.boolean)
	end)
	T.it("#string → integer", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("#", lit_str_k("hello")), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("#table → integer", function()
		local s = V.new_solver()
		local env = E.bind(E.new(), "t", V.rec({}, true, nil))
		local ty, _e, err = W.walk_synth(unary("#", id("t")), env, s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("#integer rejects", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(unary("#", lit_int_k(3)), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
...
T.describe("walker sub-phase K6c: NODE_CAST_EXPR", function()
	T.it("checked cast passes on subtype (integer cast to number)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(cast(lit_int_k(3), V.number, false), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.number)
	end)
	T.it("checked cast fails on non-subtype (string cast to integer)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(cast(lit_str_k("x"), V.integer, false), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_CAST)
	end)
	T.it("force cast passes on overlap (number cast to integer)", function()
		local s = V.new_solver()
		-- number and integer overlap (integer <: number), so the force cast
		-- succeeds with target = integer.
		local ty, _e, err = W.walk_synth(cast(lit_num(3.0), V.integer, true), E.new(), s)
		T.eq(err, nil)
		T.eq(ty, V.integer)
	end)
	T.it("force cast fails on disjoint (string cast to integer)", function()
		local s = V.new_solver()
		local ty, _e, err = W.walk_synth(cast(lit_str_k("x"), V.integer, true), E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_CAST)
	end)
	T.it("cast without resolved annotation rejects with E_CAST", function()
		local s = V.new_solver()
		local node = { tag = defs.NODE_CAST_EXPR, expr = lit_int_k(1),
			annotation = nil, annotation_id = 7, force = false }
		local ty, _e, err = W.walk_synth(node, E.new(), s)
		T.eq(ty, nil)
		T.ok(err ~= nil)
		T.eq(err.code, D.E_CAST)
	end)
