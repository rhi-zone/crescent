...
-- Walk `ty` checking that every μ in it is contractive. `guard_depths` is the
-- set of binder depths (counting outward from the current point) that ARE
-- guarded by a constructor between here and their binder. We track the current
-- binder depth and, for each binder, whether a guard has been crossed.
--
-- Simpler framing: recurse with `unguarded` = the number of enclosing μ binders
-- that have NOT yet been guarded since their binder (the innermost `unguarded`
-- binders). A constructor (`rec`/`indexer`/`fn`) resets `unguarded` to 0 for its
-- children (everything under it is guarded w.r.t. all currently-open binders).
-- Entering a `mu` pushes a new binder: it becomes unguarded (unguarded+1). A
-- `tyvar(idx)` is non-contractive iff `idx <= unguarded` (it refers to a binder
-- that has not been guarded on this path).

local check_contractive --[[: (Ty, integer) -> boolean ]]

--: (Ty, integer) -> boolean
check_contractive = function(ty, unguarded)
	local k = ty.kind
	if k == "tyvar" then
		local idx = ty.idx or 1
	...
	if k == "mu" then
		return check_contractive(ty.body or G.never(), unguarded + 1)
	end
	-- Guarding constructors: children are guarded w.r.t. all open binders.
	if k == "rec" or k == "rec_with_indexer" then
		local fields = ty.fields or {}
		for i = 1, #fields do
			if not check_contractive(fields[i].ty, 0) then return false end
		end
		if k == "rec_with_indexer" then
			if not check_contractive(ty.key or G.never(), 0) then return false end
			if not check_contractive(ty.val or G.never(), 0) then return false end
		end
		return true
	end
	if k == "indexer" then
	...
	if k == "fn" then
		...
		local pva = p.vararg --[[: Ty | nil ]]
		local rva = r.vararg --[[: Ty | nil ]]
		for i = 1, #p.fixed do if not check_contractive(p.fixed[i], 0) then return false end end
		if pva ~= nil and not check_contractive(pva, 0) then return false end
		for i = 1, #r.fixed do if not check_contractive(r.fixed[i], 0) then return false end end
		if rva ~= nil and not check_contractive(rva, 0) then return false end
		return true
	end
	...
	if k == "union" or k == "inter" then
		local ms = ty.members or {}
		for i = 1, #ms do
			if not check_contractive(ms[i], unguarded) then return false end
		end
		return true
	end
