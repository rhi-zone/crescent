-- lib/fractal/ffi_ir_rust_c_abi.lua — the Rust-implementing-a-C-ABI projector,
...
-- C-ABI consumer — `ffi_ir_python_ctypes.lua`'s ctypes bindings,
-- `ffi_ir_ruby_ffi.lua`'s `ffi`-gem bindings, `ffi_ir_csharp_pinvoke.lua`'s
...
--     parameter/return, reusing type_ref_rust_serde.lua's primitive/struct type
...
-- type_ref_rust_wasm_bindgen.lua uses for kinds it cannot realize on its own
-- target's ABI, and the mirror image of `ffi_ir_ruby_ffi.lua`/
-- `ffi_ir_python_ctypes.lua`, where all three non-`copy` disciplines legitimately
...
-- type_ref_rust_serde.lua rather than shared, matching the TS source's own
...
local rust_serde = require("lib.fractal.type_ref_rust_serde")
...
-- compile. Duplicated from type_ref_rust_serde.lua per the file header's
...
-- ffi_ir_ruby_ffi.lua's identically-shaped helper.
...
-- type_ref.lua's own `ordered_keys` makes. The emitted SET of declarations is
...
--     delegating straight to type_ref_rust_serde.lua's
...
function M.to_rust_c_abi_type(ref)
	...
	-- `never`" (worked around in `ffi_ir_wit.lua` by reading the discriminant
	-- through an open structural cast, and in `ffi_ir_python_ctypes.lua` by this
...
-- type_ref_rust_wasm_bindgen.lua uses for the same reason.
...
-- requires (see `OwnershipDiscipline`'s `opaque-handle` case in ffi_ir.lua,
...
-- for every resource, and the one `ffi_ir_ruby_ffi.lua`/
-- `ffi_ir_python_ctypes.lua` bind against by the same `<resource>_free` name).
