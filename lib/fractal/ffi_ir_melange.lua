...
--
-- (See `OwnershipDiscipline` in ffi_ir.lua. Melange's ownership story is NOT
...
-- same conversion `type_ref.lua`'s `resolve_ref` applies to its own source's
...
-- property order `type_ref.lua`'s own `ordered_keys` establishes, and for the
...
-- `lib/fractal/` owns a shared case-conversion vocabulary, and inventing one
...
function melange_type(ref, ctx, name_hint)
	...
	if shape.kind == "ref" then
		-- Convention (see `resource_ref`'s own doc comment in ffi_ir.lua): a
...
-- module-naming rule) wrapping an abstract `type t` (per ffi_ir.lua's own doc
