-- lib/fractal/http_value.lua — the request/response VALUE model the HTTP
...
--
--   - A streaming response carries a `lib/fractal/stream.lua` Stream in place
...
-- into real bytes on a real socket is `lib/fractal/http_adapter.lua`'s job and
...
-- The deferred producer. `producer` is a lib/fractal/stream.lua Stream whose
...
function M.json_response(value, init)
	...
	if text == nil then
		return nil, "fractal.http_value: response body is not JSON-serializable: " .. tostring(err)
	end
...
-- A streaming response. `producer` is a lib/fractal/stream.lua Stream emitting
...
function M.stream_response(producer, init)
	if not stream.is_stream(producer) then
		error("fractal.http_value: stream_response requires a lib/fractal/stream Stream")
	end
	local status = (init and init.status) or 200
	return {
		status  = status,
...
function M.request_url(req, override_query)
	local host = req.host
	if host == nil then
		return nil, "fractal.http_value: cannot build an absolute URL — the request has no host (no Host field and no configured server host name)"
	end
