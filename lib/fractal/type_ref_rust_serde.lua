-- lib/fractal/type_ref_rust_serde.lua — the Rust/serde projector for the
...
--     deterministic stand-in `type_ref.lua`'s `ordered_keys` uses, for the
...
-- recover, exactly as `type_ref.lua`'s `ordered_keys` does for traversal.
...
local handlers = {
	...
	-- fields (see type_ref.lua's `instance` comment) — referenced by
