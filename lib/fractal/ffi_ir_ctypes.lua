...
-- `ffi_ir_ruby_ffi.lua` — both are consumer-side loaders for a C-ABI library —
...
function M.to_ctypes_shape(ref)
	...
	if kind == "ref" then
		...
		-- malformed shape the same way type_ref.lua's `resolve_ref` does for
		-- exactly this field.
...
function M.to_ctypes_type(ref)
	...
	-- `ffi_ir_wit.lua` by reading the discriminant through an open structural
...
-- along — same shape as `ffi_ir_ruby_ffi.lua`'s equivalent builders.
...
function M.to_ctypes(ref, name, library_path)
	...
	if kind == "method" then
		...
		-- one `ffi_ir_ruby_ffi.lua` uses at the identical spot.
