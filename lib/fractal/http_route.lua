-- lib/fractal/http_route.lua — the HTTP ROUTE TREE: its type, its constructor,
...
--
-- The route tree is a SEPARATE type from the API tree (`Node`, lib/fractal/
...
-- encoders are route.ts's OTHER half and live in lib/fractal/http_run.lua.
...
-- lib/fractal/http_meta.lua. `read_http_meta` below is this module's own,
...
local http_value = require("lib.fractal.http_value")

local M = {}

-- ── Types ────────────────────────────────────────────────────────────────

-- The open metadata bag, same shape as lib/fractal/init.lua's `Meta`. Declared
...
-- convention. Same shape as lib/fractal/input.lua's `ParamSource`; declared
...
local function merge_routes(target, incoming, path)
	...
	for i = 1, #incoming_keys do
		...
		if target_methods[method] ~= nil then
			error("fractal.http_route: apply_move_to: conflicting route — " .. method .. " "
...
function M.source_coverage_message(problems)
	--: { [integer]: string }
	local lines = {}
	lines[1] = "fractal.http_route: HTTP route source coverage: " .. tostring(#problems) .. " problem(s)"
	for i = 1, #problems do
		local p = problems[i]
