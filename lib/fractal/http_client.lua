-- lib/fractal/http_client.lua — the runtime HTTP client, ported from
...
--
-- See:
--   lib/fractal/http_value.lua           — the request/response value model
--   lib/fractal/http_client_extension.lua — the extension protocol
--   lib/fractal/stream.lua               — what a streaming body carries
...
local function query_values(key, v)
	if as_list(v) then
		...
		for i = 1, #v do
			...
			if type(item) == "table" then
				return nil, "fractal.http_client: query param '" .. key .. "' has a table element, which has no query encoding"
			end
			out[i] = tostring(item)
		end
		return out, nil
	...
	if type(v) == "table" then
		return nil, "fractal.http_client: query param '" .. key .. "' is a table, which has no query encoding"
	end
...
local function build_request(origin, verb, path, slugs, input, cancel)
	...
	if text == nil then
		return nil, "fractal.http_client: request body is not JSON-serializable: " .. tostring(encode_err)
	end
	return {
		method  = verb,
		path    = full_path,
...
local function timeout_message(verb, path, ms)
	return "fractal.http_client: request timed out after " .. tostring(ms) .. "ms: " .. verb .. " " .. path
end

--: (verb: string, path: string, reason: string) -> string
local function abort_message(verb, path, reason)
	return "fractal.http_client: request aborted (" .. reason .. "): " .. verb .. " " .. path
end
...
local function timeout_promise(timer, ms, verb, path)
	...
	if not is_promise(sleeping) then
		return nil, "fractal.http_client: opts.timer must return a promise"
	end
...
local function default_decode(res)
	...
	if err ~= nil then
		return nil, "fractal.http_client: response is not valid JSON: " .. tostring(err)
	end
	return decoded, nil
end
...
local function send(site, req, timeout_ms, token)
	...
	if timeout_ms ~= nil then
		...
		if timer == nil then
			...
			-- construction.
			return async.rejected("fractal.http_client: a timeout was requested but no opts.timer cap was injected")
		end
	...
	if not is_promise(raced) then
		return async.rejected("fractal.http_client: async.race did not return a promise")
	end
...
local function perform(site, input, call_opts)
	...
	if not as_response(settled) then
		error("fractal.http_client: the transport did not return an HTTP response value", 0)
	end
...
function M.create_client_from_route(route, opts)
	if opts.transport == nil then
		error("fractal.http_client: opts.transport is required — there is no default transport (see the module doc on caps)")
	end
	if opts.timeout ~= nil and opts.timer == nil then
		error("fractal.http_client: opts.timeout requires opts.timer — a timeout with no clock would never fire")
	end
