-- lib/fractal/ffi_ir_gleam_external.lua — the Gleam `@external` projector,
...
-- Reuses `type_ref_gleam_native.lua`'s `gleam_type_from_type_ref` for every
...
-- established in `ffi_ir.lua`): `@external`'s second and third arguments name
-- information ffi-ir's `FfiRef`/`FfiShape` schema has no field for (the
-- schema was deliberately kept target-agnostic — see ffi_ir.lua's file
...
local gleam_native = require("lib.fractal.type_ref_gleam_native")
...
--
-- Local rather than shared with `type_ref_codegen.lua`'s identically-behaving
...
-- it. (`type_ref_gleam_native.lua`'s own `doc_comment` does split, because ITS
-- source does.)
