-- lib/fractal/ffi_ir_rescript_external.lua — the ReScript `external`-declaration
...
--    same stand-in, for the same reason, that `type_ref.lua`'s own
...
local rescript = require("lib.fractal.type_ref_rescript_native")
...
-- the CONSUMER's scope, where type_ref.lua's `TypeRef`/`Meta` are not bound.
...
-- and minimal repro in ffi_ir.lua's own copy of this comment, and the TODO.md
...
-- formulation `type_ref_rescript_native.lua`'s own `sanitize_label` uses — is
...
-- (`sanitize_label` in type_ref_rescript_native.lua passes with the natural
