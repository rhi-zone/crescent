-- lib/fractal/jsonrpc_server_test.lua
-- Tests for lib/fractal/jsonrpc_server.lua — the dispatch core and both
-- transports.
--
...
local function build_tree()
  return fractal.api({
    ...
    echo = fractal.op(function(input) return input end),
    ...
    boom = fractal.op(function(_input) error("handler exploded") end),
    nothing = fractal.op(function(_input) return nil end),
    failing = fractal.op(function(_input) return result.err({ kind = "notFound", message = "no such book" }) end),
    wrapped = fractal.op(function(_input) return result.ok("unwrapped") end),
    counted = fractal.op(function(_input)
    ...
    streamed_error = fractal.op(function(_input)
      return stream.from(function(emit)
...
T.describe("lib.fractal.jsonrpc_server", function()
  ...
  T.describe("HTTP transport — params", function()
    ...
    T.it("honours a leaf's sourceMap override", function()
      local tree = fractal.api({
        whoami = fractal.op(function(input) return record(input).who end,
  ...
  T.describe("HTTP transport — notifications (§4.1)", function()
    ...
    T.it("still runs the handler", function()
      ...
      local tree = fractal.api({
        touch = fractal.op(function(_input) seen["count"] = seen.count + 1; return nil end),
      }, nil)
      local handler = server.http_handler_from_tree(tree, nil)
  ...
  T.describe("WebSocket transport — streaming", function()
    ...
    T.it("defaults a progress effect with no total to a unit total", function()
      local tree = fractal.api({
        vague = fractal.op(function(_input)
          return stream.from(function(emit)
    ...
    T.it("treats an untagged emission as a chunk", function()
      local tree = fractal.api({
        bare = fractal.op(function(_input)
          return stream.from(function(emit)
