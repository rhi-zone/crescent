-- lib/fractal/direct_test.lua
-- Tests for lib/fractal/direct.lua (the zero-protocol-overhead projection).
...
local direct  = require("lib.fractal.direct")
...
T.describe("lib.fractal.direct", function()

  T.describe("leaf shape", function()

    T.it("a bare leaf projects to a function", function()
      local api = direct.create_direct_api(fractal.op(function(input) return input end))
      T.eq(type(api), "function")
    end)

    T.it("a leaf callable returns a promise", function()
      local api = direct.create_direct_api(fractal.op(function(_) return 1 end))
    ...
    T.it("the handler's return value is the resolved value", function()
      local api = direct.create_direct_api(fractal.op(function(input) return input.n * 2 end))
    ...
    T.it("a handler raising an error rejects the promise", function()
      local api = direct.create_direct_api(fractal.op(function(_) error("boom") end))
    ...
    T.it("a handler returning a promise is awaited, not double-wrapped", function()
      local api = direct.create_direct_api(fractal.op(function(_) return async.resolved("inner") end))
      local v, e = call(api, nil)
      T.eq(e, nil)
      T.eq(v, "inner")
    end)
  ...
  T.describe("branch shape", function()
    ...
    T.it("a branch projects to a table keyed by child name", function()
      local api = direct.create_direct_api(fractal.api({
        list = fractal.op(function(_) return "listed" end),
      }))
      T.eq(type(api), "table")
    ...
    T.it("nests arbitrarily deep", function()
      local api = direct.create_direct_api(fractal.api({
        books = fractal.api({
          reviews = fractal.api({
            list = fractal.op(function(_) return "deep" end),
          }),
        }),
      }))
  ...
  T.describe("fallback and slug binding", function()
    ...
    T.it("a fallback projects to a function under its own name", function()
      local subtree = fractal.api({ read = fractal.op(function(_) return "read" end) })
      local api = direct.create_direct_api(
        fractal.api({}, { fallback = { name = "bookId", subtree = subtree } }))
    ...
    T.it("the slug value is seeded into a descendant leaf's input", function()
      local subtree = fractal.api({ read = fractal.op(function(input) return input.bookId end) })
      local api = direct.create_direct_api(
        fractal.api({}, { fallback = { name = "bookId", subtree = subtree } }))
    ...
    T.it("caller fields and seeded slugs merge", function()
      local subtree = fractal.api({
        read = fractal.op(function(input) return input.bookId .. "/" .. input.fields end),
      })
      local api = direct.create_direct_api(
        fractal.api({}, { fallback = { name = "bookId", subtree = subtree } }))
    ...
    T.it("nested fallbacks accumulate every slug", function()
      local inner = fractal.api({
        read = fractal.op(function(input) return input.bookId .. ":" .. input.reviewId end),
      })
      local subtree = fractal.api({}, { fallback = { name = "reviewId", subtree = inner } })
      local api = direct.create_direct_api(
        fractal.api({}, { fallback = { name = "bookId", subtree = subtree } }))
    ...
    T.it("a non-table input is passed through rather than merged into", function()
      local subtree = fractal.api({ read = fractal.op(function(input) return input end) })
      local api = direct.create_direct_api(
        fractal.api({}, { fallback = { name = "bookId", subtree = subtree } }))
    ...
    T.it("a leaf under no fallback receives the caller's input unchanged", function()
      local given = { a = 1 }
      local api = direct.create_direct_api(
        fractal.api({ read = fractal.op(function(input) return input end) }))
    ...
    T.it("a branch carrying both children and a fallback exposes both", function()
      local subtree = fractal.api({ read = fractal.op(function(_) return "sub" end) })
      local api = direct.create_direct_api(fractal.api(
        { list = fractal.op(function(_) return "list" end) },
  ...
  T.describe("the source tree is not mutated", function()
    ...
    T.it("projecting leaves the tree untouched", function()
      local leaf = fractal.op(function(_) return 1 end)
