-- lib/fractal/cli_projector.lua — the CLI projection of a Node tree, ported
...
--
-- A handler returning a `lib/fractal/stream.lua` stream is drained
...
--
-- See:
--   lib/fractal/init.lua   — Node, op/api, is_array
--   lib/fractal/tags.lua   — resolve_tags
--   lib/fractal/input.lua  — assemble (store-based input resolution)
--   lib/fractal/stream.lua — the streaming convention this drains
--   lib/fractal/page.lua   — the pagination shape `--all-pages` walks
...
local input_mod   = require("lib.fractal.input")
local page        = require("lib.fractal.page")
local result_mod  = require("lib.fractal.result")
...
local function coerce_field(field, value, schema)
	if schema.type == "array" then
		...
		if type(value) == "table" and fractal.is_array(value --[[: { [unknown]: unknown }]]) then
...
function M.bash_completion_script(root, schemas, program_name)
	...
	push(lines, "# Generated by lib/fractal/cli_projector.lua (static).")
...
function M.generate_completions(shell, root, schemas, program_name)
	...
	error("fractal.cli_projector: unknown shell '" .. tostring(shell) .. "'")
end
...
local run_cli_async = async.async(function(root, argv, caps, opts)
	...
	if tags.streaming == true or parsed.jsonl then
		...
		elseif type(res) == "table" and fractal.is_array(res --[[: { [unknown]: unknown }]]) then
...
function M.run_cli(root, argv, caps, opts)
	if type(caps) ~= "table" then
		error("fractal.cli_projector: run_cli requires a caps table (stdout_write, stderr_write, confirm, env)")
	end
	if type(caps.stdout_write) ~= "function" then
		error("fractal.cli_projector: run_cli requires the caps.stdout_write cap")
	end
	if type(caps.stderr_write) ~= "function" then
		error("fractal.cli_projector: run_cli requires the caps.stderr_write cap")
	end
	if type(caps.confirm) ~= "function" then
		error("fractal.cli_projector: run_cli requires the caps.confirm cap")
	end
	if type(caps.env) ~= "table" then
		error("fractal.cli_projector: run_cli requires the caps.env cap")
	end
