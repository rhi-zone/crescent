-- lib/fractal/jsonrpc_server.lua — the two transport adapters and the dispatch
...
--
-- STREAMING. A handler returning a `lib/fractal/stream` Stream is drained
...
local input   = require("lib.fractal.input")
...
--
-- Cross-module named types are not yet supported for lib/fractal's own
...
-- A decoded JSON array — a batch (§6). Uses `lib/fractal`'s `is_array`, under
...
local function id_of_malformed(raw)
	...
	if fractal.is_array(raw) then return wire.NULL end
	local id = raw.id
...
local function run_dispatch(dispatch, req, id, is_notification, opts)
	...
	if as_record(raw_params) and not fractal.is_array(raw_params) then
