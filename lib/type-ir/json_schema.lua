-- lib/type-ir/json_schema.lua — project a TypeRef to JSON Schema
-- draft 2020-12. Ported from fractal's packages/type-ir/src/json-schema.ts.
--
-- The input model is `lib/type-ir/init.lua` (shape constructors, the kind
...
-- json_schema_07.lua and draft-04 in json_schema_04.lua. The
...
-- init.lua. The emitted SET of properties/definitions is identical either
...
-- types rather than assuming `string`. init.lua's `EnumShape` declares
...
local passthrough_keys = {
	...
	-- json_schema_04.lua for the boolean-modifier form draft-04 uses).
	"exclusiveMinimum",
	"exclusiveMaximum",
	"minLength",
	"maxLength",
...
-- `const` is always actually set (see init.lua's `types.literal`).
--: (project: JsonSchemaProject, shape: TypeShape, meta: Meta) -> JsonSchema
