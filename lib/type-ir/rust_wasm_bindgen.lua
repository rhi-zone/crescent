-- lib/type-ir/rust_wasm_bindgen.lua — Rust codegen targeting
...
-- problem and same resolution as `init.lua`'s `ordered_keys` (see its
...
-- `init.lua`'s `ordered_keys` for the same decision made once already.
...
-- a wrapper carrying a concrete return type, while `rust_serde.lua`
