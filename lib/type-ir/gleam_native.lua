-- lib/type-ir/gleam_native.lua — the Gleam (https://gleam.run/)
...
--
-- This module does NOT require `lib/type-ir/kinds_common.lua`, and
...
--     the same deterministic stand-in `init.lua` adopted for
...
-- and the stdlib declaration agree. (`lib/type-ir/rust_wasm_bindgen.lua`
...
-- `init.lua`'s `ordered_keys` for the same reasoning applied to
-- traversal. Duplicated rather than shared because that one is module-local
-- to `init.lua`; the two are independent uses of the same convention, not
...
local handlers = {
	...
	--
	-- `method` has no entry of its own: `init.lua` registers `method`'s
