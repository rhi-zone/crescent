...
| 3 | **10-button input row mixing three affordance types.** Send/Continue/Impersonate (action), Export (export), Settings/Card Lorebook/My Lorebooks/Card Editor/Regex/Group (navigation into editors) — all rendered identically. ~5× over Miller's 7. | S1, S3, T1, T3 | Split into semantic groups: primary action (Send), composition (Continue, Impersonate), navigation (moved off the input row entirely). Blocked on #4 (navigation buttons relocate into card surface). |
| 4 | **Card Editor and Card Lorebook as separate top-level buttons**, both editing the same object (the loaded card). | S5, G3, G6 | One tabbed card surface (Identity, Lorebook, Greetings, Regex) replacing the separate editor/lorebook/regex panels. One entry point (card header Edit button). Resolves #3, #7, #8 as a bundle. |
| 5 | ~~**`btn-card-edit` labeled "Card Editor"**: creation-only affordance issue.~~ Moot since "New Card" now exists as a sibling. Residual: Card Editor button in toolbar is now redundant with card header Edit. Clean up when #3 lands (toolbar redesign removes it). | O3, T2 | Resolved by #1 (New Card added as sibling). Cleanup deferred to #3. |
...
| 7 | **Author's Note bar is permanently present.** Data-entry affordance for card runtime context competing for top-level attention. | S1, T4 | Collapse into card surface as a field on Identity tab. Part of #4 (tabbed card surface). |
| 8 | **Regex Scripts as top-level sibling to card.** Card-scoped state treated as a distinct top-level object. | S1, G3 | Regex moves into card surface as a tab. Part of #4 (tabbed card surface). |
