...
4. `docs/typechecker-v7-coherence-audit.md` — whether the current design is a
   consistent whole, plus blocking semantic seams.
5. `docs/typechecker-v7-kernel-semantics.md` — current semantic kernel,
   judgments, and certificate obligations.
6. `docs/typechecker-v7-semantic-mining.md` — rules for importing semantics from
...
8. `docs/typechecker-v7-consolidation-audit.md` — source hierarchy, conflicts,
   and lineage status.
...
1. **Place/binder model.** Decide how facts and postconditions name stable
   places after `unknown` refinement.
...
3. **Primitive capabilities.** Decide whether primitive cutouts such as
   `$SetMetatable` are first-class `Type` constructors or external capability
   metadata.
4. **Contextual control effects.** Specify at least `throws(E)` and
...
5. **Module/declaration environments.** Define `ModuleEnv`/`DeclEnv` before
   admitting `$Require`, `--:: require`, `$GlobalScope`, or module exports.
6. **Setmetatable fork.** Decide whether `set_metatable` seals construction or
   only fixes metatable state. `__index` chain walking depends on this.
7. **Match/type-level computation.** Specify match evaluation, captures,
   suspension/rejection, and field descriptors before admitting `$EachField` or
   deleting helper intrinsics.
8. **Operator/metamethod substrate.** Define operator application through
