...
- [x] **P4** `[gen-pass]` `--:: module "name": T` directive only records ctx.module_name; top-level bindings are not exported as the module's type — lib/type/static-v5/constrain.lua directive-processing loop (module branch) — closed (export side) next-commit (per-field CSub emitted post-walk; missing fields surfaced as gen errors)
