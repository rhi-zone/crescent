...
- [x] **R2** `[substrate]` T-CRowExtend-Bind mutates `rec.fields` in place; aliased records visible across constraints; monotone but not idempotent under park/wake; "record bound exactly once before any CRowExtend fires" invariant unstated and unverified — lib/type/static-v5/op_sem.lua:1187-1196, audit 2026-05-27 — **verified safe-by-construction 2026-05-27 (read-only analysis, no code change)**: records are created fresh per table literal (constrain.lua:1675); field addition is monotone (add-only; Bind precondition forbids overwrite; repeat keys route to Lookup→CEq); parked constraints store the uvar and deref fresh on wake, so they never act on a stale snapshot; spec doc (typechecker-v5-operational-semantics.md) notates σ[F ∪ {key}] as intended in-place extension. The park/wake hazard does not manifest. In-place mutation is the intended monotone-field-extension mechanism.
...
- [x] **R4** `[gen-pass]` `resolve_callee_eager` has no depth bound or cycle detection; unbounded field-access chain walk; recursive records → infinite loop — lib/type/static-v5/constrain.lua:864-924, audit 2026-05-27 — closed 90222966 (depth cap at 8 steps; visited-set cycle detection via `tostring(ty)` record identity)
- [x] **R5** `[gen-pass]` `resolve_aliases_impl` has no cycle detection or memoization; cyclic aliases (A=B, B=A) infinite-loop — lib/type/static-v5/constrain.lua:189-294, audit 2026-05-27 — closed 90222966 (depth cap at 32; visited-set cycle detection; returns type unexpanded on exceed)
- [x] **cross-file-cast** `[typechecker-bug]` `--[[: { [integer]: string } ]]` inline cast inside a test closure caused 11 cross-file type errors when 3 files were checked together via `bin/cr check <a> <b> <c>`. Workaround at commit 90222966 was to remove the annotation; root cause not investigated. Surfaced by R4/R5/Y2 fix agent, 2026-05-27 — closed d8a70d33 (R7+R8: per-session AnnState eliminated module-level mutable `_next_rowvar`/`effect_arities`; multi-file error count now equals sum of single-file counts with or without the cast; verified 2026-05-27 on 3-file invocation `error_format_test.lua constrain_test.lua cli_e2e_test.lua`: 740 = 82+444+214 both ways).
...
- [x] **R6** `[architecture]` cli.lua M.main reaches `io.open`, `io.stdout`, `io.stderr`, `os.getenv` directly despite module header claiming "All I/O via injected caps" — lib/type/static-v5/cli.lua:254-280, audit 2026-05-27 — closed dfd60eff (M.main removed from cli.lua; real caps construction (io.open reader, io.stdout/stderr writers, os.getenv color detection) moved to bin/cr-check.lua; cli.lua contains zero io.*/os.* references; convention: global I/O permitted only in top-level entry scripts)
- [x] **R7** `[architecture]` ann.lua `_next_rowvar` is module-level mutable, never resets; two parses in the same process produce row-var ID collisions silently — lib/type/static-v5/ann.lua:225-232, audit 2026-05-27 — closed d8a70d33 (AnnState context object; `next_rowvar` and `effect_arities` moved into per-session state; all public API takes `state` as first param; callers create one state per parse session)
- [x] **R8** `[architecture]` ann.lua `effect_arities` is module-level shared state; "idempotent" comment ≠ isolation across parsers — lib/type/static-v5/ann.lua:57-64, audit 2026-05-27 — closed d8a70d33 (see R7; same commit)
...
- [x] **Y2** `[gen-pass]` `extract_yield_from_scope` returns `(unknown, fresh_uvar, unknown)` if scope stack is empty; top-level `coroutine.yield` silently typechecks against unknown — lib/type/static-v5/constrain.lua:627-668, audit 2026-05-27 — closed 90222966 (returns nil on empty effect_stack; caller emits F2 error "coroutine.yield called outside a function annotated with !yield")
- [x] **Y3** `[gen-pass]` `spread_last_expr` may treat arrow-with-uvar-return as single-value, diverging from gen_expr_multi's expectation — lib/type/static-v5/constrain.lua:1835-1909, audit 2026-05-27 — **verified benign 2026-05-27 (read-only analysis, no code change)**: single-value fallback for unknown-arity returns is correct Lua semantics; the solver refines via the fresh uvar introduced by the unknown-arity call; gen_expr_multi correctly handles the single-value approximation. Caveat: verdict from code-trace, not a runnable repro.
...
- [x] **Y7** `[cli]` `expand_dotted` has no edge-case guards; `"."`, `".foo"` could collide silently — lib/type/static-v5/cli.lua:56-82, audit 2026-05-27 — closed 63e3a859 (empty-segment keys error explicitly; prefix-vs-leaf guard omitted after investigation: stdlib intentionally passes both leaf record and dotted children; see cli_test.lua for 12 new unit tests). **Verified 2026-05-27**: `stdlib_types.lua` lines 173-186 set `d["io.write"]/["io.read"]/["io.lines"]/["io.open"]` then set `d["io"] = types_mod.record({write,read,lines,open})` — all four field values are identical objects. `expand_dotted` silently drops the pre-built leaf because `"io"` becomes a dotted prefix, synthesizes a structurally identical record from the dotted children, and overwrites. No code reads `d["io"]` before `expand_dotted` runs. Silent drop is safe: the pre-built record is a redundant convenience that lose nothing when replaced by the synthesized one. Verdict: **intended (redundant pre-build; safe)**. **(superseded: expand_dotted removed entirely, stdlib written as nested records direct — see closed flatten-then-expand entry)**
- [x] **Y8** `[error-ux]` `render_prose` silent fallback for unknown ErrorDetails variants; new variants ship to users with generic prose, no compile warning — lib/type/static-v5/error_format.lua:170, audit 2026-05-27 — closed 051a8820 (unhandled non-nil details now renders `[rule: <tag>] msg`; details==nil path unchanged; 3 new assertions in error_format_test.lua)
...
- [ ] **Y10** `[surface]` ann.lua parse_declaration missing `augment` and `unseal` directives that v4 has; silently misparses as type alias — lib/type/static-v5/ann.lua:691-792, audit 2026-05-27
  - **Silent-misparse bug CLOSED** (commit 5e3ec7e6): `augment` and `unseal` are now detected explicitly and produce `"augment declarations are not yet supported in v5 (tracked gap)"` / `"unseal declarations are not yet supported in v5 (tracked gap)"` errors instead of falling through to the type-alias case. Tests added in ann_test.lua (10 new assertions, suite 714 total).
  - **Feature gap remains OPEN**: implementing `augment` (merge fields into an existing type binding) and `unseal` (rebind opaque to inner type) in v5 semantics is a separate design task. v4 surface syntax: `--:: augment Name { field: T }` / `--:: unseal Name`.
- [x] **Y11** `[surface]` `scan_number_lit` returns 0 on `tonumber` failure; defensive degradation hides scanner bugs — lib/type/static-v5/ann.lua:197-211, audit 2026-05-27 — closed faa9d33d (raises error() on tonumber failure; error path unreachable from correct scanner input; value is removing misleading fallback)
...
- [x] **Y8** `[error-ux]` `render_prose` silent fallback for unknown ErrorDetails variants — closed 051a8820 (unhandled non-nil details renders `[rule: <tag>] msg`; details==nil path unchanged; 3 new assertions in error_format_test.lua)
