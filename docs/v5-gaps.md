...
- [ ] **cross-file-cast** `[typechecker-bug]` `--[[: { [integer]: string } ]]` inline cast inside a test closure caused 11 cross-file type errors when 3 files were checked together via `bin/cr check <a> <b> <c>`. Workaround at commit 90222966 was to remove the annotation; root cause not investigated. Surfaced by R4/R5/Y2 fix agent, 2026-05-27.
