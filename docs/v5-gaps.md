...
- [x] **P6** `[gen-pass]` Arithmetic operators return number unconditionally; integer refinement deferred — lib/type/static-v5/constrain.lua (gen-pass #4 residual from 56af6204) — closed TBD (integer-valued $LitNum promoted to $LitInt in LIT_NUMBER handler; +/-/* of two integer-typed operands returns integer; /^%// always number)
