...
- [ ] **P5** `[gen-pass]` Return-last-spread not implemented: `return f(), 2` where `f()` returns multiple values — f's extra returns beyond field "1" are discarded rather than expanded into the caller's positional list — lib/type/static-v5/constrain.lua NODE_RETURN_STMT (rl>1 path)
...
- [x] **P6** `[gen-pass]` Type alias / require / module directives parsed but not scope-injected (`declare_var`, `declare_effect` stubs) — lib/type/static-v5/constrain.lua:41 — closed TBD (declare_var/declare_effect already wired; type_alias fully wired with eager body expansion + param substitution; module records ctx.module_name; require/template no-op by v4 parity)
...
- [ ] **P4** `[gen-pass]` `--:: module "name": T` directive only records ctx.module_name; top-level bindings are not exported as the module's type; $Require resolution for the module not wired — lib/type/static-v5/constrain.lua directive-processing loop (module branch)
