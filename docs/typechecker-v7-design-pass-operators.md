...
External declaration environments may bind runtime raw-operation names to these
capabilities when the target exposes them. For example, a Lua 5.4-like
declaration set may bind `rawlen`; a LuaJIT 5.1 declaration set should not.
Aliases of capability values preserve capability behavior. The checker must not
inspect the callee's source name.
