...
| Raw operations (`rawget`, `rawset`, `rawequal`, `rawlen`) | `docs/typechecker-v4-stdlib-design.md` stdlib scope | Direction chosen: raw lookup/assignment primitives bypass metamethods but not invalidation; LuaJIT default binds rawget/rawset/rawequal but not rawlen. | Raw ops bypass metamethods and interact with table identity/indexers. | Treating them as ordinary field ops may incorrectly include metatable behavior. | Core/stdlib extension: raw table observation/write primitives over identity/record state; `$RawLen` is target-dependent. |
