...
## Web / UI
`lib/web/` framework (middleware, routing, cookies, CSRF, static, html builder, reactive_dom). `lib/glassmorphism/` opt-in design tokens + components + theme toggle (CSS/JS, no Lua). Router, canvas (PPM/PGM/BMP), color toolchain, TUI on top of ANSI, layout, pretty_print, barcode, qrencode (wip).
