...
This means `to_dnf` only distributes *algebraic connectives that are structurally present* in the type graph. Named types and unevaluated match types reach `match_pattern` as-is.
...
**DNF is not the right algorithm.** DNF is exponential even for structurally-present connectives: `(A₁|B₁) & (A₂|B₂) & ... & (Aₖ|Bₖ)` materializes 2ᵏ terms — O(∏ union sizes). The coinductive approach processes the same type with O(∑ union sizes) work, because it looks up each field across intersection members independently rather than enumerating all combinations. `(A|B).x` and `(C|D).x` are computed separately and intersected lazily — 2+2 lookups, not 2×2. DNF forces you to materialize the cross-product before touching the pattern; coinductive matching is demand-driven and only computes what the pattern asks for.

The correct algorithm is **coinductive field-merging structural matching**:
