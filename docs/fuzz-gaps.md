...
- `never | T == T` — `try_unify(make_union([never, T]), T)` and reverse
- `never & T == never` — `try_unify(make_inter([never, T]), never)`
- `T | never == T | never <: T` — degenerate union
Currently untested. `never` is generated by `arb_type` but never appears in these forms.

### A2: readonly field semantics
- `{ readonly x: T } <: { readonly x: T }` — reflexivity (covered by optional flag tests but not readonly)
- `{ x: T } <: { readonly x: T }` — mutable satisfies readonly in read position
- `{ readonly x: T } </: { x: T }` — readonly does NOT satisfy mutable in write position
  (write position check is in constrain.lua, not unify.lua — may require a different test form)
