...
- `lib/type/` — runtime schema typechecker (separate from static).
  ...
  - `lib/type/static-v4/` — greenfield typechecker rewrite, foundation (Phases 4a–4g). Type representation + simple-sub-style subtyping; equi-recursive μ types; indexed access `T[K]`; complement `~T` with DNF-based emptiness; match types as bidirectional pattern destructors; rank-N polymorphism via deep-skolemization (Peyton-Jones et al. 2007) with MLstruct `#F` skolem tags and transitive-closure escape check; effects as a small named enumeration on function arrows (covariant); content-addressed cache primitives (binary serialize/deserialize, SHA-256 hash, manifest with cri-hash + per-file deps, cache_lookup/store over an injected `io_caps` capability — no global `os`/`io` reach). No AST walker, no CLI yet. See `docs/typechecker-rewrite-design.md` and the directory README. Status: wip.
  - `lib/type/static-v4/walker/` — AST walker bridging Lua AST nodes to the v4 core. Sub-phases A–H complete: functional env record + narrowing overlay; two-mode dispatch shell; per-node handlers for literals, identifiers, varargs, calls, functions, returns, control-flow + narrowing, statements, match, indexed access, module pattern, FFI cdef integration, and effect plumbing (error/pcall/xpcall intrinsics, with coroutine.yield riding the standard binding-driven call-effect-accumulation path). See `docs/typechecker-ast-walker-design.md`. Status: wip (A–H of A–J).
  - `lib/type/static-v4/driver/` — driver for the v4 pipeline (parse → decode → walk → cache). K2 (`decoder.lua`): eager arena → POJO AST decoder translating the legacy parser's `{ nodes, lists, pool, root }` arena output into the plain-Lua-table node shape the walker consumes (resolves intern IDs to strings, follows list-pool references into materialised child arrays, maps integer OP_* codes to walker string ops, preserves arena identity via per-decode cache). K1 (`driver.lua`): end-to-end `drive(source_path, opts)` — reads source via injected I/O caps, hashes (sha256), parses via injected parse cap, decodes via K2, builds an env seeded with stdlib bindings (via `inject_stdlib`) + io_caps + cache_dir + inherited require_chain, walks the chunk's statements sequentially through `walker.walk_synth` (driver owns chunk semantics — no NODE_CHUNK handler registered on the walker, preserving walker_test's "unsupported-tag stub" repro), collects diagnostics, and on a clean walk calls `V.cache_store` with the packed `(exports, aliases)` artifact and the per-require dep map. Recursive cache-miss walks wired via two module-level hooks on `walker/require_resolve.lua` (`_set_driver_hooks(drive_recursive_fn, deps_sink)`) — module-state rather than env-state because WalkerEnv's `clone()` does not carry runtime-injected fields. Cycle detection through the existing `env.require_chain` placeholder mechanism. See `docs/typechecker-v4-driver-design.md` §1, §6. Status: wip (K1 + K2 of K1–K6).
  - `lib/type/static-v4/stdlib_types_v4.lua` — K3 MVP stdlib type declarations for v4 (Option B per driver design §3): `{ bindings, aliases, parametric_aliases, intrinsics }` record consumed by the driver via `inject_stdlib`. Hand-built `V.fn` / `V.rec` / `V.forall` / `V.inter` / `V.union` values for `error`/`pcall`/`xpcall`/`require`/`type`/`tostring`/`tonumber`/`assert`/`select`/`next`/`pairs`/`ipairs`/`setmetatable`/`getmetatable`/`raw*`/`unpack`/`print`/`_VERSION`/`_G` and the `string`/`table`/`math`/`coroutine`/`bit`/`ffi`/`jit`/`package` tables. `io`/`os`/`debug`/`dofile`/`loadfile`/`loadstring`/`load`/`newproxy`/`collectgarbage`/`gcinfo` are deliberately excluded (CLAUDE.md "Caps-first, everywhere") pending a future `stdlib_types_v4_caps.lua`. Parametric aliases (`Arr`, `Ptr`, `Keys`, `Values`, `MetaOf`, `Open`, `Closed`, `PairsReturn`, `IpairsReturn`, `PcallReturn`, `Ctype`) are stored as Lua functions because v4 lacks a type-level lambda / deferred-match constructor. Spread function results, `select` tail-typing, and `$PatternReturn`/`$FindReturn` precision degrade to `unknown` per design §7. See `docs/typechecker-v4-stdlib-design.md`. Status: wip (K3 of K1–K6).
  - `lib/type/static-v4/cli.lua` + `cli_compare.lua` — K5 CLI surface for the v4 pipeline. `cli.lua` (K5): `bin/cr check --v4 [--summary] [--cache-dir=PATH] <file>...` — argv parsing, real io_caps built from `io`/`os` at the legitimate CLI boundary, stdlib injection, driver invocation, K4 default/summary rendering, exit code = 1 iff any error-severity diagnostic or driver-itself failure. `cli_compare.lua` (K5b): `bin/cr check --compare <file>...` — invokes legacy via subprocess (black-box, parses `file:line:col: severity:` lines from stdout with ANSI stripped) and v4 via the K1 driver in-process, computes a canonical-position multiset diff (key = `file:line:col:severity`; messages NOT compared, since taxonomies don't align), and emits per-file verdicts `agree` / `DIVERGE on decision` / `DIVERGE on details`. Exit code reflects agreement (0/1/2) not correctness — agreement is the test. Bin-side dispatch in `bin/cr-check.lua` selects `--compare` (priority), `--v4`, or legacy. Status: wip (K5 + K5b of K1–K6).
  - `lib/type/search/` — Hoogle-style type search over the registry. Status: wip.
  - `lib/type/analysis/` — agnostic static-analysis substrate (first mechanization of the agnostic direction; see `docs/agnostic-static-analysis-design.md` and the map's "Agnostic Static Analysis" section). `init.lua`: the language-agnostic substrate — Id, Artifact, Observation, Claim, Evidence, Dependency, TrustBoundary, AnalysisState, CheckRequest/CheckResult, SemanticsRegistry, and a worklist checker that finds an order-independent acceptance order over arbitrarily-ordered evidence, detects dependency cycles (terminating with a diagnostic, never looping), records dependencies as first-class records, and classifies claims accepted/rejected/unknown (evidence outcomes, not hosted truth; unknown is never consumed as proof). The substrate knows nothing about props/terms/types/binders/substitution: claim predicates and args are opaque `ArgValue` data, compared structurally for identity (no name-keyed special cases). Hosted-checker verdicts are surfaced as a visible trusted boundary (`unverified_checker_trust`) per the design's hosted-checker trust obligation. `prop.lua` (`prop.logic.min`) and `lambda.lua` (`lambda.untyped.min`) are two independent hosted semantics that import the substrate (never the reverse): propositional `holds` claims with assumption/trusted_axiom/and_intro/and_elim/modus_ponens/contradiction (contradiction is hosted policy, not explosion); untyped-lambda well_formed/alpha_eq/free_in/not_free_in/steps_to with capture-avoiding outermost call-by-name beta whose capture-avoidance side condition is a checkable cross-evidence dependency on a separately-produced `free_in` claim. Pure Lua. Tests (`*_test.lua`, 72 assertions) are the spec: every worked example from both validation docs plus claim-identity, scheduling-order-independence, cycle-termination, unknown-propagation, and adversarial probes. Status: wip (propositional + lambda rungs; cyclic-fixpoint and STLC rungs next).
...
- `lib/pdf/` — PDF object model, cross-reference table (traditional + streams), stream filter decoding (FlateDecode + PNG predictors), object serialization/incremental-update writing, and AcroForm field extraction/filling. `object.lua` parses the 8 PDF object types + indirect objects; `xref.lua` builds the offset table (follows /Prev and /XRefStm chains); `filter.lua` decodes stream data; `init.lua` composes them into `string_to_document`/`resolve`/`document_root`/`stream_to_bytes`; `write.lua` serializes objects back to PDF syntax and appends incremental updates (new objects + xref + trailer, chained via /Prev); `form.lua` extracts AcroForm fields (`document_to_fields`: name, type, value, default value, flags, page, rect — one record per widget) and fills them (`fill_fields`: sets /V, /AS for checkboxes/radio buttons, /NeedAppearances on the AcroForm dict) via `write.lua`'s incremental update. Content stream (text/graphics operators) parsing and font handling are a separate, parallel effort (not in `form.lua`'s scope: filling never regenerates an appearance stream); objects packed in an Object Stream (xref entry type 2) are represented but not resolved. Roadmap-v2 §2a. Status: initial.
...
- `lib/safe_regex/` — JS-regex catastrophic-backtracking validator, hologram-shaped algorithm ("no quantifier on a subexpression containing an unbounded quantifier"); pure Lua. Companion `lib/js_safe_regex/` ships the JS impl. Status: stable.
- `lib/js_safe_regex/` — JS impl of the safe_regex validator (ESM `safe_regex.js`), used by the browser realm bootstrap's RegExp / String.match / matchAll / search wrappers; `init.lua` is type-only. Parity-tested against `lib/safe_regex/` (static corpus + runtime verdicts via bun). Status: stable.
- `lib/js_realm_sandbox/` — allow-list realm lockdown for pack browser realms (ESM `sandbox.js`); `installLockdown(opts)` captures intrinsic originals, installs bounded-method wrappers (repeat/padStart/padEnd/join/flat/flatMap/Array(n)/Array.from/Array.of/fromCharCode/fromCodePoint/JSON.parse/JSON.stringify against SIZE_CAP), wraps `RegExp` and `String.prototype.{match,matchAll,search}` via `lib/js_safe_regex`, neutralises constructor reflection paths (Function/AsyncFunction/Generator/AsyncGenerator/`.__proto__.constructor.constructor`), strips Error.stack and RegExp legacy statics, prunes/freezes globalThis. Implements docs/platform_isolation.md §3 "Allow-list specification". `init.lua` is type-only. Tests run the full escape-test corpus in fresh `node:vm` contexts via bun; `parity_test.lua` asserts every spec corpus entry is referenced in `sandbox.test.js`. Status: stable.
- `lib/js_pack_validator/` — author-side hygiene validator for pack JS (ESM `validator.js`); walks the acorn AST against the pack-JS subset in docs/platform_isolation.md §3 "Pack JS subset (draft)" -- rejects class syntax, generators, `with`, tagged templates, dynamic `import()`, dynamic bracket member access, non-allow-listed `new` targets, and banned identifier references (`eval`, `Function`, `Reflect`, `Proxy`, `WeakRef`, `FinalizationRegistry`, `SharedArrayBuffer`, ...); integrates with `lib/js_safe_regex` for regex literal checks. Runs in author dev/CI via bun -- NOT a daemon dependency (per spec rejection of bundling JS parsers in the daemon). Vendors `acorn` at `dep/acorn/`. `init.lua` is type-only. `parity_test.lua` asserts every spec subset rule is exercised in `validator.test.js`. Status: initial.
- `lib/js_cap_bridge/` — postMessage capability bridge between pack realm and host stub (ESM `bridge.js`); `createRealmBridge({capNames, postMessage, registerMessageHandler})` produces non-constructable async cap shells (concise-method syntax) installed onto `globalThis.__cap__` by the sandbox bootstrap, each serialising calls to `{id, kind:"call", cap, args}` frames and returning promises resolved on host response; `createHostBridge({grantedCaps, capImpls, audit, postMessage})` validates the cap against the user's grants, invokes the impl via `Promise.resolve().then(() => impl(...args))` (foreign-thenable hazard avoidance), and audit-logs every attempt (`denied` / `missing` / `throw` / `ok`). Args are never implicitly coerced; envelope validation responds with `malformed` when id is recoverable, drops otherwise. Pure protocol implementation -- transport and message-handler registration are injected so the same code runs in `node:vm` test contexts and real iframes. Implements docs/platform_isolation.md §4 "Capability bridge protocol". `init.lua` is type-only. `parity_test.lua` asserts every protocol rule is referenced in `bridge.test.js` and optionally drives `bun bridge.test.js`. Status: initial.
- `lib/js_caps/` — day-zero browser cap implementations (ESM `index.js`); per-cap files (`text_encode.js`, `text_decode.js`, `compress.js`, `decompress.js`, `console_log.js`, `web_crypto_random.js`, `set_timeout.js`, `web_crypto_subtle.js`, `clipboard_write.js`, `fetch_api.js`, `kv.js`, `ui.js`) each export a pure wrap (or, for `fetch_api`, a `makeFetchApi` factory; for the kv_* family, a `makeKvCaps` factory; for the UI caps `toast`/`dialog`/`navigate`, a `makeUiCaps` factory) of the underlying host API with structural arg validation; `index.js` aggregates the bare-function caps into `dayZeroCaps: Map<string, function>` ready to drop into `lib/js_pack_host#mountPack` as `opts.capImpls`, exports factory caps (`makeFetchApi`, `makeKvCaps`, `makeUiCaps`) separately so the host page can construct them per manifest entry / per host-app configuration, and exports `buildDayZeroCapImpls({ fetchConfig?, kvConfig?, uiPrimitives? })` to compose the full Map in one call. Implements docs/browser_caps.md §5 "Day-zero exposed cap surface" -- shipped 17/17 (6 trivial pure-wrap caps + `set_timeout`, cancellable via the cap-bridge AbortSignal extension per docs/platform_isolation.md §4, + `web_crypto_subtle`, op-discriminated SubtleCrypto wrap with `encrypt`/`decrypt`/`sign`/`verify`/`digest`/`generateKey`/`deriveKey`/`deriveBits`/`importKey`/`exportKey`/`wrapKey`/`unwrapKey`; CryptoKey crosses the bridge via structured clone, + `clipboard_write`, `navigator.clipboard.writeText` wrap with 1 MiB size cap; NotAllowedError propagates as a `throw` envelope to the pack realm, + `fetch_api` factory `makeFetchApi({ allowed_origins })`, attenuated browser-side fetch with origin allowlist bound at host-side instantiation per docs/browser_caps.md §4.1.1; validates URL/method/headers/responseFormat, shapes response into structured-clone-safe `{status, statusText, headers, body}`, body is ArrayBuffer by default or JSON/text per `responseFormat`, AbortSignal propagates via the bridge's signal-marker replacement, + the `kv_read`/`kv_write`/`kv_delete`/`kv_keys` family via `makeKvCaps({ pack_id })`, IndexedDB-backed per-pack key-value store (one database `pack_<pack_id>` with a single `kv` object store; structured-clone values round-trip Uint8Array / Map / Set / Date directly; per-pack partitioning by database name; keys are non-empty strings up to 1024 chars; the four caps share a backend and are returned together by the factory, + the `toast`/`dialog`/`navigate` family via `makeUiCaps({ renderToast, renderDialog, requestNavigate })` per docs/browser_caps.md §4.4.10/§4.10.3/§4.10.4; pure routing layers over host-app-supplied UI primitives -- caps validate structured input (toast: message ≤512 chars, level ∈ info/warning/error, duration ≤30s; dialog: message ≤1024 chars, 1-8 non-empty button strings, host return must match an input button; navigate: path ≤2048 chars, host-app-defined semantics) and forward to the injected primitive). `init.lua` is type-only. `caps.test.js` covers happy path / validation failure / boundary per cap plus aggregator shape (kv_* tested against a tiny in-memory IndexedDB mock because bun's `node:vm` does not expose `indexedDB`; UI caps tested against a fake `uiPrimitives` recording calls); `parity_test.lua` asserts every shipped cap kind is referenced in `caps.test.js` and optionally drives `bun caps.test.js`. Status: initial.
- `lib/js_pack_host/` — pack iframe pipeline, both halves. Inside-iframe: ESM `bootstrap.js`; `runBootstrap({capNames, postMessage, registerMessageHandler, sizeCap?})` builds the realm-side bridge via `lib/js_cap_bridge#createRealmBridge`, then installs the lockdown via `lib/js_realm_sandbox#installLockdown` with the bridge's cap shells, returning `{caps, dispatch}` so pack source loaded LATER by the outer host page reaches caps through `globalThis.__cap__`. Sequencing is fixed: bridge first (captures vanilla intrinsics in its closures), lockdown second (seals realm and installs `__cap__`), pack source third (orchestrated by the outer host page). Outside-iframe: ESM `host.js`; `mountPack({container, packBootstrapUrl, capNames, capImpls, audit, onError?, origin?})` creates a cross-origin sandboxed iframe (`sandbox="allow-scripts"` only, `referrerpolicy="no-referrer"`), wires the host-side cap bridge (`createHostBridge`) to the iframe's window via `postMessage`/`addEventListener("message")`, validates `capNames ⊆ capImpls.keys()` at mount, enforces both origin (when `opts.origin` set) and source (`event.source === iframe.contentWindow`, unconditional) on inbound messages, and returns `{iframe, unmount, audit}` with audit entries for mount/unmount alongside the bridge's per-cap audit entries. Implements docs/platform_isolation.md §3 "Realm primitive" + "Bootstrap script" + §4 "Capability bridge protocol". `init.lua` is type-only (declares `run_bootstrap` and `mount_pack`). Tests: `bootstrap.test.js` pairs `runBootstrap` (inside `node:vm`) with `createHostBridge` (outside) via callback-routed postMessage; `host.test.js` pairs `mountPack` (outside, with a fake DOM exposing only the surface mountPack touches) with the inlined bootstrap inside a vm context, covering cap-validation rejection, mount audit, end-to-end cap round-trip, origin and source enforcement, and unmount listener+iframe cleanup. `parity_test.lua` asserts every bootstrap contract rule is referenced in `bootstrap.test.js` and optionally drives `bun bootstrap.test.js`. Real-browser smoke for `host.js` (real iframes, real postMessage) is a manual follow-up. Status: initial.
...
- `lib/platform/manifest_caps.lua` — shared cap-declaration helpers: `merge_cap_declarations` (daemon caps) with shorthand expansion ("required" / "optional") and top-level-plus-per-entrypoint merge. Extracted from `cli.lua` so the merge logic can be tested without loading the CLI's auto-main side effects. Status: initial.
- `lib/platform/projection_pipeline/` — pure function `(source_path, globals_files) -> ts_string | (nil, err)` that typechecks a Lua projection against an injected prelude stack and transpiles it via `lib/lua2ts` with `harden = true`, `module = "esm"`. Slice 1: no caching/daemon/mtime. Status: wip.
...
- `lib/os_isolation/` — OS-level process/thread isolation + forced interruption, distinct from `lib/sandbox`'s in-process capability restriction (see that module's header for the split rationale). Three independent isolation implementations: `fork_direct` (direct `fork()`, no `exec()`, same LuaJIT image via COW — requires a single-threaded caller, checked best-effort via `/proc/self/status` on Linux and refused, not silently risked, when violated), `fork_supervisor` (a single-threaded supervisor process, `supervisor_main.lua`, forks children on the caller's behalf — safe regardless of caller threading, only Lua source + JSON args cross the boundary), `thread` (separate `lua_State` per OS thread via a pure-FFI `pthread_create` + FFI-callback pattern, no vendored C — carries one open safety question, see module header, plus an observed-but-undiagnosed CPU-contention-sensitive timing caveat under heavy concurrent spawn load). Three independent interruption mechanisms: `interrupt_kill` (SIGKILL, process-granularity only), `interrupt_ptrace` (PTRACE_SEIZE/INTERRUPT suspend-resume, Linux only, non-destructive — confirmed working against a real child pid, confirmed EPERM against a thread's tid in this repo's own sandbox despite `ptrace(2)` documenting that case as always-allowed), `interrupt_cooperative` (opt-in caller-instrumented loop checks, not a hostile-script defense). Parity-tested across the three isolation implementations (`os_isolation_parity_test.lua`). Implements docs/genre-battery/sandboxing.md's "Decided direction". Status: initial.
...
- `lib/null/` — the shared null sentinel: one table, compared by identity, for data models that must distinguish an explicit null from an absent value. Used by `lib/format/json/{pure,ffi}` and `lib/type-ir`. Status: stable.
