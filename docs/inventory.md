...
- `lib/js_pack_host/` — iframe-side bootstrap that stitches the realm-side cap bridge and the realm lockdown into one entry point (ESM `bootstrap.js`); `runBootstrap({capNames, postMessage, registerMessageHandler, sizeCap?})` builds the bridge via `lib/js_cap_bridge#createRealmBridge`, then installs the lockdown via `lib/js_realm_sandbox#installLockdown` with the bridge's cap shells, then returns `{caps, dispatch}` so pack source loaded LATER by the outer host page can reach caps through `globalThis.__cap__`. Sequencing is fixed: bridge first (captures vanilla intrinsics in its closures), lockdown second (seals realm and installs `__cap__`), pack source third (orchestrated by the outer host page -- separate library, not included here). Implements the inside-iframe half of docs/platform_isolation.md §3 "Bootstrap script" / "Bootstrap order". `init.lua` is type-only. Tests pair `runBootstrap` (inside `node:vm`) with `createHostBridge` (outside) via callback-routed postMessage, covering paired-bridge end-to-end, cap subset enforcement, lockdown effective post-bootstrap, envelope shape, and non-constructable cap shells. `parity_test.lua` asserts every bootstrap contract rule is referenced in `bootstrap.test.js` and optionally drives `bun bootstrap.test.js`. Status: initial; outside-iframe host page is a forthcoming separate library.
