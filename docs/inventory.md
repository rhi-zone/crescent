...
- `lib/platform/platform_types.lua` — core platform type declarations (Manifest, CapDecl, BrowserCapDecl, EntryDef, AppRecord, TarEntry). Declarations only. Status: stable.
- `lib/platform/manifest_caps.lua` — shared cap-declaration helpers: `merge_cap_declarations` (daemon caps) and `merge_browser_cap_declarations` (browser caps, see docs/browser_caps.md §3) plus `validate_browser_caps` rejecting unknown day-zero kinds. Same shorthand expansion ("required" / "optional") and same top-level-plus-per-entrypoint merge for both namespaces. Status: initial.
