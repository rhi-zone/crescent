...
- `lib/type/` — runtime schema typechecker (separate from static).
  ...
  - `lib/type/static-v4/walker/` — AST walker bridging Lua AST nodes to the v4 core. Sub-phase A: functional environment record (`env.lua`) with copy-on-edit semantics and a narrowing overlay; two-mode dispatch shell (`walker.lua`, SYNTHESIZE / CHECK) keyed on `node.tag` with a register-handler extension mechanism. No per-node handlers yet — all dispatches stub to a clean "not yet implemented" error in sub-phase A. See `docs/typechecker-ast-walker-design.md`. Status: wip (A of A–J).
