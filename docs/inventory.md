...
- `lib/glassmorphism/` — opt-in glassmorphic design system (translucent panels, layered 3D borders, blue primary). Pure CSS tokens + components plus `theme.js` (dark/light/system toggle persisted to localStorage). No Lua. Used by ccv2 and library platform apps via symlinks under their `static/` dirs. Status: stable.
