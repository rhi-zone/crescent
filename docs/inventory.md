...
- `lib/type/` — runtime schema typechecker (separate from static).
  ...
  - `lib/type/static-v4/` — greenfield typechecker rewrite, foundation (Phases 4a–4d). Type representation + simple-sub-style subtyping algorithm; equi-recursive μ types; indexed access `T[K]`; complement `~T` with DNF-based emptiness checking and the MLstruct negation rewrite that supersedes the 4a union-on-RHS / intersection-on-LHS rejection; match types as bidirectional pattern destructors with `_` desugared to `~(union of arms)`, disjointness-checked at construction, forward and backward sharing one arm-matching primitive. No AST walker, no CLI, no quantifiers yet. See `docs/typechecker-rewrite-design.md` and the directory README. Status: wip.
