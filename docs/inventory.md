...
- `lib/js_caps/` — day-zero browser cap implementations (ESM `index.js`); per-cap files (`text_encode.js`, `text_decode.js`, `compress.js`, `decompress.js`, `console_log.js`, `web_crypto_random.js`) each export a pure wrap of the underlying host API with structural arg validation; `index.js` aggregates them into `dayZeroCaps: Map<string, function>` ready to drop into `lib/js_pack_host#mountPack` as `opts.capImpls`. Implements docs/browser_caps.md §5 "Day-zero exposed cap surface" -- this commit ships 6 of 17; the remaining 11 (fetch_api, web_crypto_subtle, kv_*, navigate, dialog, toast, clipboard_write, set_timeout) involve design decisions and land in subsequent commits. `set_timeout` is blocked on the cap-bridge AbortSignal cancellation extension (docs/platform_isolation.md §4); the previously-shipped `set_timeout`/`clear_timeout` pair was reverted as a half-measure (no cancellation handle). `init.lua` is type-only. `caps.test.js` covers happy path / validation failure / boundary per cap plus aggregator shape; `parity_test.lua` asserts every shipped cap kind is referenced in `caps.test.js` and optionally drives `bun caps.test.js`. Status: initial.
