...
- `lib/js_caps/` — day-zero browser cap implementations (ESM `index.js`); per-cap files (`text_encode.js`, `text_decode.js`, `compress.js`, `decompress.js`, `console_log.js`, `web_crypto_random.js`, `set_timeout.js`, `web_crypto_subtle.js`) each export a pure wrap of the underlying host API with structural arg validation; `index.js` aggregates them into `dayZeroCaps: Map<string, function>` ready to drop into `lib/js_pack_host#mountPack` as `opts.capImpls`. Implements docs/browser_caps.md §5 "Day-zero exposed cap surface" -- this commit ships 8 of 17 (6 trivial pure-wrap caps + `set_timeout`, cancellable via the cap-bridge AbortSignal extension per docs/platform_isolation.md §4, + `web_crypto_subtle`, op-discriminated SubtleCrypto wrap with `encrypt`/`decrypt`/`sign`/`verify`/`digest`/`generateKey`/`deriveKey`/`deriveBits`/`importKey`/`exportKey`/`wrapKey`/`unwrapKey`; CryptoKey crosses the bridge via structured clone). The remaining caps (fetch_api, kv_*, navigate, dialog, toast, clipboard_write) involve design decisions and land in subsequent commits. `init.lua` is type-only. `caps.test.js` covers happy path / validation failure / boundary per cap plus aggregator shape; `parity_test.lua` asserts every shipped cap kind is referenced in `caps.test.js` and optionally drives `bun caps.test.js`. Status: initial.
