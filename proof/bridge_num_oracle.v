(* Scratch oracle: number-atom membership verdicts from the PROVEN model.
   ...
   LuaJIT 5.1 model (corrected): ONE number value [VNum (NumRep)]. [VInt] is
   [VNum NRint] — an integer-valued double; [VFloat] is [VNum NRfrac] —
   ...
   [VFloat] is a non-integer double, NOT the same as [VInt]. *)
...
Compute (memb (BAtom ANum) (VInt)).    (* expect true  *)
Compute (memb (BAtom ANum) (VFloat)).  (* expect true  *)
Compute (memb (BAtom ANum) (VInt)).    (* expect true  *)
Compute (memb (BAtom ANum) (VStr)).    (* expect false *)
...
Compute (memb (BAtom AFloat) (VInt)).    (* expect true  — int IS a float on 5.1 *)
Compute (memb (BAtom AFloat) (VFloat)).  (* expect true  *)
Compute (memb (BAtom AFloat) (VInt)).    (* expect true  *)
Compute (memb (BAtom AFloat) (VStr)).    (* expect false *)

(* AInt — only integer-valued ([NRint], i.e. [VInt _]) numbers; [VFloat _]
   ([NRfrac _], a genuinely non-integer double) does NOT inhabit it. *)
Compute (memb (BAtom AInt) (VInt)).    (* expect true  *)
Compute (memb (BAtom AInt) (VInt)).    (* expect true  *)
Compute (memb (BAtom AInt) (VFloat)).  (* expect false — VFloat is a NON-integer double *)
Compute (memb (BAtom AInt) (VFloat)).  (* expect false *)
Compute (memb (BAtom AInt) (VStr)).    (* expect false *)
