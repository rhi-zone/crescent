...
**Generic function bodies are checked at definition time via skolem variables** (`skolemize_fn` in `constrain.lua`). Skolem vars have `FLAG_SKOLEM` set and cannot be bound by the solver, preventing self-loops. This is the default for all generic (`<T>`) functions. Exception: `--:: template` opts out — template function bodies are NOT checked at definition time; the body is skipped entirely and only checked at call sites when the concrete type `M` is known. Use `--:: template` for functions parametric over an interpreter object where definition-time checking is impossible.
