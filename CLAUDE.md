...
**Do the correct thing fully.** When the right approach is clear, implement it. Amount of work, scope, and convenience are never reasons to ship a known-wrong version. "Pragmatic but slightly unsound," "good enough for now," "we'll add X later," "out of scope," "no half-measures with a TODO," and **"minimal change"** are rejection categories, not design options — they all describe the same failure mode: choosing an incorrect solution and letting it accumulate. Half-measures and gradual migrations are context poisoning: every future session sees the wrong state, builds on it, and the error compounds. If a design needs X to be correct, ship with X or don't ship. The only legitimate reason to omit something is that it genuinely does not apply yet (no caller, no user, no data) — and that reason must survive scrutiny if challenged.

**"Minimal change" is a banned justification.** Smallness is not correctness. The question is "is this the *correct* change," not "is this a small change." A minimal fix that doesn't address the root cause poisons future sessions: the broken state stays in the repo, looks intentional, and next session's reasoning builds on the wrong premise. If the correct fix is large, ship the large fix or stop and surface the scope to the user — do not ship a small fix that leaves the underlying defect in place and call it pragmatic.

**Ad-hoc conditions are strictly forbidden.** A "fix" that adds a per-case carve-out, a kind-specific peek into other constraints, a "special-case this one situation" branch, or any other instance of "make X work *here* without solving why X is needed at all" is rejected outright. Such fixes set a bad precedent (future sessions pattern-match the carve-out as the right shape) and poison context (the repo accumulates wrongly-shaped state that every subsequent reasoning step builds on). If the principled fix requires reworking the architecture, that *is* the work — not a future commitment to delegate away. **"Our architecture is wrong" is a valid answer; act on it.**

**Temporary measures are context poisoning.** A "provisional" fix, "stopgap," "documented as temporary," or "to be replaced in phase N" implementation in committed code creates the exact failure mode the no-ad-hoc rule targets: future sessions read the provisional code, treat its shape as the intended design, build on it, and the provisional becomes permanent by accretion. The TODO comment is not a mitigation; the wrong-shape implementation IS the poison. When the correct fix isn't available yet (e.g., depends on a later phase or a feature not yet built), the right move is to **not implement the partial fix** — leave the case unsolved, fail loudly, or block on the dependency. Provisional implementations are rejected by the same standard as ad-hoc carve-outs.
...
**No ambient globals by default.** Crescent typechecks under the assumption that NO global names are ambient — every name a Lua file references must be declared (as a local, via `require`, or as an explicit stdlib declaration). The Lua standard library (`require`, `tostring`, `pairs`, `error`, `pcall`, etc.) gets its types from explicit `--:: declare ...` lines in stdlib type files, not from a built-in ambient table. Consequences: the typechecker must reject undeclared name references rather than silently widening to `unknown`; libraries must not reach for `_G` or assume language primitives are ambient; type-level intrinsics like `$Require<T>` exist specifically to give the explicit stdlib declarations enough power to type their return values without requiring full dependent types. Do not assume these intrinsics are redundant with value-level expressions — value-level expressions only have types because the intrinsics provide them (e.g. `typeof require(T)` decays to `$Require<T>` via the stdlib declaration, not the reverse).
