...
# escape_file()/escape_stdin() convention subagent-context-start.sh and
# subagent-context-refresh.sh both use).
#
# Key-matching (states 0-1 below) is a raw-text rolling-window search over
# the un-decoded JSON bytes -- no "am I inside a string" gating on the match
# itself. This looks unsafe (what if some earlier field's string VALUE
# happens to contain the literal text "<field>":?) but isn't: valid JSON
# always backslash-escapes a literal quote character appearing inside a
