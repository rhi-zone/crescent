...
- [ ] **lib/ljsocket/init.lua resists fixes (25 errors)** — every focused attempt regressed. Errors are mostly `$Opaque & (...unknown) -> _` overload mismatches in WSA/Windows FFI plus `meta.*` socket methods where `self` is `unknown` and naive annotation cascades through `socket.X(...)` mod-function calls. Speculative direction: declare a `Socket` newtype/alias used consistently across all `meta.*` methods AND coordinate with `mod.create`'s setmetatable return type. Too invasive for a per-file pass.
...
- [ ] **lib/xgboost, lib/stream, lib/hamt: recursive type aliases need method signatures** — each ~18 errors. The recursive Tree/Stream/Trie node types aren't fully declared, so accessor expressions on child nodes fall back to `unknown`. Pattern: declare `--:: TreeNode = { left: TreeNode | nil, right: TreeNode | nil, ... }` with all methods, then per-method `--:` annotations. Same approach worked on commits clearing similar files (treap, multimap, kdtree, etc.).
...
- [ ] **Add precise opaque-object type declarations for 9 libraries** — `lib/cron` (Expr), `lib/fsm` (Machine, Instance), `lib/glob` (Matcher), `lib/graph` (Graph), `lib/lru_ttl` (Cache), `lib/pagination` (Paginator, LazyPaginator), `lib/rate_limiter` (TokenBucket, LeakyBucket, FixedWindow, SlidingWindowLog, SlidingWindowCounter, ConcurrentLimiter, MultiLimiter), `lib/ratelimit` (TokenBucket, SlidingWindow, FixedWindow, LeakyBucket, KeyedLimiter), `lib/regex/pure` (Regex). These libraries reference their primary opaque type but never declare it, leaving 1-9 `undefined type` errors per file. Adding either a precise structural decl (matching `setmetatable({...}, Mt)` body) or an open `{ ... }` placeholder UNCOVERS pre-existing latent body errors that were previously masked because the typechecker bailed when the return annotation referenced an undefined type. Each library needs the proper structural type declared AND the resulting cascade of body errors fixed in the same pass. Filed during the bulk "import undefined type names" sweep — these were the only files where a permissive `{ ... }` decl still introduced regressions, so they were left untouched. See `TODO.md` "Phase D3 cleanup" for the closely-related `unknown <: any` cleanup and the soundness-vs-permissiveness tradeoff.
...
- [ ] **type/static: pcall return tuple binding propagates input fn type to slot 2** — `local ok, lib = pcall(ffi.load, name)` binds `lib` to the type of `ffi.load` (`(string, boolean | nil) -> unknown`) instead of to its return type. Workaround: re-annotate as `local raw = lib --: unknown` before casting. Reproduces in `lib/sqlite/init.lua` and `lib/hash/sha256/init.lua` (Phase C). The PcallReturn intrinsic from session 26 is supposed to type these as `(true, ...R) | (false, string)` — investigate why the second-slot binding is taking the function type instead.

- [ ] **type/static: `ffi.new("Byte[?]", n)` VLA overload not modelled** — `bin/cr check lib/compress/system.lua` and `lib/hash/sha256/init.lua` both report `no matching overload for ($Opaque(_), _ | nil) -> _ & (_, ? | nil) -> ?` at every VLA `ffi.new` call site. The `[?]` size-parameter overload of `ffi.new` is not in the stdlib type. Fix: add a `(string, integer) -> unknown` (or proper VLA-aware) overload to ffi.new in `stdlib_types.lua`.
