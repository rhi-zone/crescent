...
- [ ] **v5-source-pipeline-integration** — parser + gen-pass wiring for `&` (intersection) and `!Name<Args>` (effect) syntax. Prerequisite before G8/G12 are user-observable. Estimated 2-3 cycles. Substrate promotion from `experiments/` to `static-v5/` is a sub-cycle prerequisite.
