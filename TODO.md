...
- [ ] **ccv2 card self-containment migration** — writable `self_write` cap (see `docs/platform-design.md`), migrate kv → PNG for card state per `docs/card-app-design.md` "State storage and self-containment". Includes rename of global `world_info` → `user_lorebooks[]` array with active toggle + "My Lorebooks" UI section.
