...
- [ ] **De-specialcase builtins** — `require`, `pcall`/`xpcall`, `pairs`/`ipairs`, `type()`, `assert`, `error`, `select`, `string.find`, `io.open`, `string.byte`, `string.match`/`gmatch`/`gsub` are hardcoded in constrain.lua. Spec needed per builtin: what type system features are required to express it in stdlib.d.lua, which features are missing (e.g. `pcall` needs TAG_SPREAD + variadic capture; `pairs`/`ipairs` needs typed iterator return; `require` needs `$Require<T>`), and the order of removal (remove special case only after the feature that replaces it is implemented and tested).
...
- [ ] **Typechecker: table-valued dispatch key (GAP-HKT3)** — `fa[TC]` where `TC` is a module table used as a dispatch key produces `T_UNKNOWN` (open-table miss). The checker does not track table identity as a lookup key — it only resolves literal strings and opaque keys. So `fa[Mappable].map(...)` is untyped throughout. Fix: apply the typeclass dispatch key pattern (item above) — add `MappableKey: $Opaque` declaration to each typeclass module, change dispatch from `fa[Mappable]` to `fa[Mappable.key]`. The `$Opaque` / `FLAG_OPAQUE_KEY` mechanism already exists in the typechecker. **Not yet done** — `lib/fp/mappable`, `lib/fp/applicable`, etc. still use raw module-table keys. Until this is applied, `lib/fp/` dispatch is invisible to the typechecker and wrong-arg errors are silently missed.
