...
- [ ] **ccv2 `static/app.js` modularisation** — the frontend is ~2400 lines in a single file. First pass landed: `api.js` (HTTP helper) and `persona.js` (persona panel feature) extracted as ES modules; `index.html` already loads as `<script type="module">`. Remaining feature areas still inline in `app.js`: lorebook + linked-lorebooks panel, my-lorebooks overlay, settings overlay + readSettings/populateSettings, session panel, card editor (identity/greetings tabs), regex scripts panel, group chat panel, token counter, streaming send (`sendStreaming`), message list event delegation. Extract one feature area per pass; each gets its own `<name>.js` exporting a single `init(deps)` returning `{ reload? }` or nothing. Dependencies that cross modules (e.g. `showError`, `setBusy`, `reloadMessages`) should be injected through `init` opts, not imported from `app.js` (would cause circular imports).
