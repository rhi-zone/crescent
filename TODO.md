...
- [ ] **`array.lua`/`map.lua` likely have the same unmerged-split gap `text.lua`'s `merge_structs` fix (entry above, bug 4) just closed, unverified.** Their own `find_pos` (array.lua) and delete-time split call sites (`struct_store.get_item_clean_start`/`get_item_clean_end`) don't register the split-off tail into `txn.merge_structs`, so a split that's the ONLY structural change for a client in a transaction (e.g. a redundant range operation landing `count` inside an existing run) would be silently skipped by `transaction.lua`'s merge sweep, same as the text.lua bug this was found chasing. Not confirmed wire-visible for array/map — their fuzz mode (`parity_fuzz_test.lua`) still uses two-mode (`bytes/content`) for delete/overwrite sequences specifically because this is unverified, unlike text (now always byte-checked). Closing this: (1) add a fuzz probe forcing array/map to byte-mode the way text.lua's fix was validated, to confirm the gap is real and wire-visible there too; (2) if confirmed, register split-off tails into `txn.merge_structs` at array.lua's/map.lua's own split call sites (mirroring text.lua's `find_pos`/`M.delete`/`M.format` fix exactly); (3) once verified clean, drop array/map's two-mode fuzz split the same way text's was dropped.
