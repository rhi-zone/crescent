...
- [ ] **Enum inference** — Infer enum types automatically from all-literal tables. A table where every named field is a literal of the same base kind (all `LIT_INTEGER` or all `LIT_STRING`) is an enum. New type kind `TAG_ENUM_MEMBER` with `(enum_name_id, member_name_id, value)`. Display: `EnumName.MemberName` instead of the raw value. Unification: `EnumName.A <: integer` (or string). Narrowing: `x == Status.OK` narrows correctly. Table-literal form first; incremental (`E.X = 1; E.Y = 2`) deferred. Needed before structured diagnostics (so error codes display as `ErrorCode.ARITH_TYPE` not `1`).
