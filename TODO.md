...
- [ ] **Redundant force casts (2114 of them) are autofixable** — `--[[:! T]]` where actual is already `T`. The cast is strippable. Blocked on autofix design above. Manually stripping is mechanical but the autofix should land first.

- [ ] **Autofix for redundant/widening casts** — Config system landed (severity, allow patterns, `pkg.lua` rules table). Still missing: (1) autofix command — the user pushed back on `bin/cr fix`, `bin/cr check --fix`, and `bin/cr lint --fix` framings; the naming/integration question is unresolved. The conversation also rejected a JSON-roundtrip approach (the typechecker already has the spans, should fix in-process). (2) Widening cast classification — currently only `REDUNDANT_CAST` (identical types, strip) and `FORCE_CAST` (everything else) exist; widening (`S <: T`, convert `--[[:! T]]` to `--[[: T]]`) is not yet distinguished from narrowing or unrelated. The user also expressed narrowing should be warning, unrelated should be error. (3) Inline suppression (`--nocheck: rule_name`) not implemented. (4) Rule groups/categories not implemented. Open design question: should autofix mutate source from inside `bin/cr check` or be a separate invocation? User has strong opinions here that weren't fully captured.
