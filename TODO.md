...
- [ ] **Daemon `POST /api/new-card` violates platform invariant** — `lib/platform/CLAUDE.md` says "no format knowledge in the platform", but the daemon's new-card endpoint hardcodes `BLANK_CHARA_JSON` (CCv2-specific format knowledge) and implicitly assumes `runtime_files` = ccv2. Correct architecture: an app declares a `new` entrypoint — a separate script in the tarball that runs whatever creation flow the app wants (form, wizard, anything). Daemon's library-level "New Card" would launch `entry=new` of whichever card app the user picks. The platform stays format-agnostic; the app script handles its own format. The current daemon `/api/new-card` is a temporary hack that should be replaced once the `new` entrypoint convention is designed and the launch-with-entrypoint path exists in the platform.
