...
- [ ] **`?` optional field shorthand breaks alias usability** — when a named type alias `Foo = { ..., bar: T? }` contains an optional field using the `?` suffix shorthand, the alias body gets a `TAG_UNION` wrapping for the nil-able field, but the resulting type is incorrectly treated as unusable when `Foo` is referenced by *another* alias (e.g. `--:: declare x = Foo` reports "undefined type `Foo`"). The issue is that `T?` in struct position expands to `T | nil` which causes the alias registration to fail silently. Workaround: replace `T?` with `T | nil` explicitly in all struct field declarations. Affected files: `lib/formats/ccv2/ccv2_types.lua` (fixed by replacing all `?` with `| nil`). Root cause is in how the annotation parser or alias resolver handles `T?` at the field level within `--::` multiline struct declarations.
