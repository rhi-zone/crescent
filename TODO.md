...
- [ ] **NEW GAP found while stress-testing the above (forcing `parity_fuzz_test.lua`'s "content" mode to also assert raw bytes, as a one-off check — not shipped as a permanent test change): real yjs's `YText.prototype.delete` doesn't just delete the covered ContentString/ContentType/ContentEmbed items (`src/types/YText.js`'s `deleteText`, confirmed against `node_modules/yjs/dist/yjs.cjs`) — it also runs `cleanupFormattingGap`/`cleanupYTextAfterTransaction`, a YText-specific pass that removes or merges now-redundant `ContentFormat` open/close marker pairs left orphaned by the delete (e.g. `insert("J"); format(0,1,{italic:true}); delete(0,1)` deletes the format markers too, collapsing to 3 fully-deleted items; this port currently leaves the two format markers live and undeleted, since `text.lua`'s `M.delete` only ever deletes countable items, matching `AbstractType.js`'s generic `typeListDelete` rather than `YText.js`'s own `deleteText` + formatting-gap cleanup).** This is a distinct feature from "item compaction" above (a YText-only attribute-run normalization, not a generic transaction-cleanup pass) — out of scope for the compaction task that found it, not fixed here. Not currently caught by any test: `parity_fuzz_test.lua`'s "content" mode compares `text.to_string` only (plain string, no formatting), which is identical either way; only a raw byte comparison exposes it. Closing this requires porting `cleanupFormattingGap`/`cleanupYTextAfterTransaction` from `src/types/YText.js` into `lib/y_crdt/text.lua`.
