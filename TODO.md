...
- [ ] **ccv2 input toolbar redesign** — currently ~10 buttons (Send, Continue, Impersonate, Export, Settings, Lorebook, World Info, Card Edit, Regex, Group) which is too many. Design: primary actions prominent, secondary actions nearby, everything else behind a toolbar row or overflow menu. Apply the "button count is a budget" principle from `docs/ui-design.md`.
...
- [ ] **Define what "crescent-format card" concretely means.** The ccv2 reproduction story depends on import converting existing CCv2 cards to crescent format, but "crescent format" is currently undefined. Decide: marker iTXt chunk? Platform runtime embedded in the PNG? Normalization of `character_book` entries through our schema? Some combination? Must be decided before the import-conversion work.
- [ ] **ccv2 import-time conversion + library integration.** Once "crescent-format" is defined, the ccv2 import pipeline (1) writes those extensions into the incoming PNG and (2) adds the PNG to the library (currently the app's own `/api/import/*` endpoints only parse and return data — they don't install). Mass passive conversion of the existing CCv2 inventory is the dominant reproduction vector per `docs/ccv2-reproduction-audit.md`; this implements it.
- [ ] **ccv2 reproduction-audit items (10 UX failures).** Tracked as a living list in `docs/ccv2-reproduction-audit.md`. Ranked by leverage; #1 (no "New Card" entry point) and #2 (barren card header) are highest impact. Each item cites the violated rule IDs from `docs/ui-design.md`. Implement in order.
