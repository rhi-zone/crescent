...
- [ ] **Soundness gap 10: annotation parser silently drops trailing tokens after the parsed type.** `lex.lua` captures everything to EOL as the annotation content; `ann.lua:1153–1155` parses one type and returns without checking the scanner reached end-of-content. So `local y --: integer = "string literal"`, `local y --: integer ! ! garbage`, `local y --: integer = undefined_ident` all typecheck with 0 diagnostics. This is the load-bearing footgun for Gap 9 — `local y --: T = x` *looks like* annotated assignment, parses cleanly, and silently becomes a no-initializer declaration. Fix: in `ann.lua` after `parse_type(s)` for `ANN_TYPE` (also `ANN_TYPE_ARGS` and the type tail of `ANN_DECL` / `declare` / `newtype`), assert the scanner has consumed all non-whitespace content; emit a diagnostic otherwise. See `docs/soundness-audit.md` Gap 10.
