...
### backlog
- [ ] **Object narrowing via field access** — `if foo.x then aaa(foo)` should narrow `foo` itself so `aaa(foo)` typechecks. Currently `field_presence` narrowing removes nil from `foo.x`'s type but leaves `foo`'s type unchanged. Needed: when the narrowing target is a field access `obj.field`, bind `obj` in the narrowed scope to a tightened type (new scope binding, not mutation — branch-join handles post-block merging as usual). For union types: truthy branch keeps members where `field` is present/non-nil, PLUS any open-table members (open tables cannot be excluded since they may have any field); falsy branch keeps members where `field` is absent or nil-typed. For plain tables: rebind `obj` with `field` tightened to non-nil (new table type in scope).
- [ ] **Type system completeness audit** — no type tag has been audited for completeness. For every tag (TAG_UNION, TAG_INTERSECTION, TAG_FUNCTION, TAG_TABLE, TAG_NOMINAL, TAG_MATCH_TYPE, TAG_FORALL, TAG_LITERAL, TAG_ROWVAR, TAG_CDATA, TAG_SPREAD, TAG_NAMED, TAG_TYPE_CALL, TAG_INTRINSIC), enumerate all operations it must support and verify each is correctly implemented and tested: field access, indexing, call, unification (covariant/contravariant), try_unify, narrowing, display, serialization (cri_write/read), instantiate/generalize, resolve_annotation_type. Known gaps surfaced so far: intersection field lookup missing, try_unify doesn't handle TAG_INTERSECTION LHS, intersection subtyping unsound. These are symptoms — do the full audit. Write a spec table (tag × operation matrix) before fixing individual gaps.
