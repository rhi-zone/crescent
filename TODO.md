...
- [ ] **ccv2 `static/app.js` modularisation** — the frontend was ~2400 lines in a single file; now ~2070. Extracted so far: `api.js` (HTTP helper), `persona.js` (persona panel), `group.js` (group chat overlay), `regex.js` (regex scripts tab). `index.html` already loads as `<script type="module">`. Remaining feature areas still inline in `app.js`: lorebook + linked-lorebooks panel, my-lorebooks overlay, settings overlay + readSettings/populateSettings, session panel, card editor (identity/greetings tabs + tab switching), Author's Note, chat export, token counter, streaming send (`sendStreaming`), message list event delegation (edit/delete/swipe), focus-trap helpers (`trapFocus`/`releaseFocus`/`setupFocusTrap`), `escAttr`, New Card button. Extract one feature area per pass; each gets its own `<name>.js` exporting a single `init(deps)` returning `{ reload? }` or nothing. Dependencies that cross modules (e.g. `showError`, `setBusy`, `reloadMessages`, `trapFocus`, `escAttr`) should be injected through `init` opts, not imported from `app.js` (would cause circular imports).
