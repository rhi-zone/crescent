...
- [ ] **Typechecker bug: unannotated function params infer to `any`** — User flagged in current session (`d4565916`) as a bug: "unannotated params infer to any — this is a bug". Per crescent's type-system rules, `any` does not exist as an inference result; unannotated params must be `unknown`. The current behaviour cascades: callers passing through `any`-inferred params silently launder values that should fail to typecheck, and downstream `REDUNDANT_CAST` classification gets false positives because `unknown <: T` collapses to true under the broken inference. Probable site: `lib/type/static/constrain.lua` around line 1542 (no-annotation branch creates fresh `TAG_VAR` that gets `any`-bound by later constraints). Blast radius is unknown — fixing may surface hundreds of legitimate errors in code currently riding on `any`. (Source: current session.) (see docs/typechecker-param-semantics.md)

- [ ] **Typechecker bug: force casts act as inference sources via external constraints** — User flagged in current session: "force casts MUST NEVER BE INFERENCE SOURCES. otherwise why the fuck are they force casts". Investigation in current session (verified by reading every line of `M.try_unify` at `lib/type/static/unify.lua:851-1102` and `M.types_overlap` at `lib/type/static/unify.lua:1132-1232`) confirmed the cast itself is non-destructive — it doesn't call `bind_var`, doesn't mutate `ta`/`tb`. The actual mechanism: `local s_ = s --[[:! string]]` creates a fresh `TAG_VAR` for `s_`; later usages (`f(1)`, `s + 1`) bind that var, and because the cast made `s_` aliased to `s` in the constraint graph, `s` gets bound transitively. Result: force casts effectively constrain their source. Hundreds-to-thousands of force-cast sites depend on this incorrect behaviour. Fix: ensure force casts produce a fresh, non-aliased binding for the result so external constraints on the result cannot flow back to the source. Related to the unannotated-param bug above — both stem from inference variables being created where they shouldn't be. (Source: current session.) (see docs/typechecker-param-semantics.md)
...
- [ ] **REDUNDANT_CAST classifier conflates *unifiability* with *assignability*** — Current session: `solve.lua:solve_overlap` (~line 2515) uses `try_unify(actual, expected)` to decide whether a force cast is redundant. `try_unify(unknown, T)` returns true for any concrete T because `unknown` unifies with anything. But `unknown` is NOT assignable to a concrete type without explicit narrowing — so a force cast from `unknown` to T is genuinely required, not redundant. Concrete failing pattern from session: `local tv = type(v); if tv == "number" then local vn = v --[[:! number]] end` — alias narrowing does not propagate to `v`. Fix: replace `try_unify` with `is_subtype` (or an equivalent assignability check) in the classifier. The current `unknown <: T = true` collapse is incompatible with the typechecker's own `unknown` semantics. **The existing TODO item ("Redundant force casts ... classifier is unsound") captures this at a high level; this second-pass entry pins the exact line and the exact fix.** (Source: current session.) (see docs/typechecker-param-semantics.md)
...
- [ ] **ccv2 `static/app.js` modularisation** — the frontend was ~2400 lines in a single file; now ~445. Extracted so far: `api.js` (HTTP helper), `persona.js` (persona panel), `group.js` (group chat overlay), `regex.js` (regex scripts tab), `settings.js` (generation settings + connection test overlay), `sessions.js` (chats sidebar), `my-lorebooks.js` (user-side lorebook library overlay), `card-editor.js` (card editor overlay framework + identity/greetings tabs + Author's Note save), `lorebook-entry.js` (shared entry-row UI: `createEntryEl` + `escAttr`/`escHtml`, imported by both card-side and user-side lorebooks), `card-lorebook.js` (card-side lorebook tab: character_book entries + linked lorebooks), `messages.js` (message-list rendering + edit/delete/swipe + sibling cache + speaker palette), `send.js` (send/continue/impersonate + SSE streaming + busy state + textarea keyboard). `index.html` already loads as `<script type="module">`. Remaining feature areas still inline in `app.js`: chat export, token counter, focus-trap helpers (`trapFocus`/`releaseFocus`/`setupFocusTrap`), New Card button, keyboard shortcuts dispatch. Extract one feature area per pass; each gets its own `<name>.js` exporting a single `init(deps)` returning `{ reload? }` or nothing. Dependencies that cross modules (e.g. `showError`, `setBusy`, `reloadMessages`, `trapFocus`) should be injected through `init` opts, not imported from `app.js` (would cause circular imports).
- [ ] **Frictionless new card — cross-origin problem** — ccv2 app runs on its own subdomain (`app-<id>.host`); it cannot call the daemon's `POST /api/apps` directly (different origin). So "New Card" in the ccv2 app falls back to a PNG download instead of auto-importing. Fix: new `create_instance` cap — ccv2 backend calls `caps.create_instance(png_bytes)` → daemon cap impl runs the install pipeline → returns launch_url → frontend redirects. Fits the cap model, app stays self-contained, daemon stays format-agnostic. Needs cap design before implementing.
...
- [ ] **messages: sibling cache is keyed by current message id, not by the originating swipe-set id** — when the user swipes prev/next, `el.dataset.id` is updated to the chosen sibling's id, but the cache entry remains keyed under the original id. The next swipe on the same element does `ensureSiblings(el.dataset.id)` against the new id and refetches `/api/swipes`. Fix: key the cache by a stable identifier (the swipe-set id, or the message's first sibling), not by the active id. Failing test: `messages.test.js` "sibling cache: second swipe in either direction does not refetch /api/swipes".
