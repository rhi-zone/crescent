...
- [ ] **ccv2 input toolbar redesign** — tabbed surface landed. Now remove: btn-lorebook, btn-card-edit, btn-regex (all redundant with card header Edit), btn-export (chat export → move to card header). Keep: Send, Continue, Impersonate, Settings, My Lorebooks, Group. Result: 6 buttons, under Miller's 7. In flight.
- [ ] **Frictionless new card — cross-origin problem** — ccv2 app runs on its own subdomain (`app-<id>.host`); it cannot call the daemon's `/api/import-card/upload` directly (different origin). So "New Card" in the ccv2 app falls back to a PNG download instead of auto-importing. Fix options: (a) daemon adds CORS `Allow-Origin` for app subdomains + ccv2 app backend passes daemon origin to frontend via `/api/card` response; (b) new `create_instance` cap: ccv2 backend calls `caps.create_instance(png_bytes)` → daemon cap impl runs import pipeline → returns launch_url, frontend redirects. Option (b) is cleaner (no CORS config, fits cap model, app stays self-contained). Needs cap design before implementing.
