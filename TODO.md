...
- [ ] **Pre-commit: enforce 0 errors AND 0 warnings** — The pre-commit hook currently only checks errors (no-new-errors vs HEAD). Warnings should be treated the same way: no new warnings introduced by a commit. Requires: (1) `bin/cr check` to support `--exit-on-warnings` or similar flag that exits non-zero when warnings exist; (2) pre-commit hook updated to use it with the same staged-vs-HEAD comparison logic. This enforces the invariant that the codebase never accumulates technical debt silently. Block on getting warning count to 0 first.

- [ ] **Linting config system + `bin/cr fix`** — Model after normalize's config format (steal it). In `pkg.lua`, a `rules` table under `typecheck` where each rule can have `{ severity = "error"|"warning"|"info"|"off", allow = {"glob/**"}, options = {...} }`. Rule groups/categories so you can toggle families together. Inline suppression via `--nocheck: rule_name` comment. `bin/cr fix` as a unified autofix command across typechecker and lint: redundant force casts → strip, widening force casts → convert to checked cast, emmylua annotations → convert to `--:` style. This is the prerequisite for pre-commit 0-warnings enforcement (can't enforce what you can't configure). Design doc at `docs/lint-config-design.md` before implementing.
