...
- [ ] **Typechecker substrate gap (surfaced building S1):** a union of string literals (`"a" | "b" | ...`) is NOT assignable to itself in *function-return* position — it widens to `string` and the return check rejects it (reflexive `X: string is not assignable to X`). It works fine as a record-field value and as a local annotation. Minimal repro: `--:: K = "a" | "b"` + `--: () -> K  local function f() return "a" end`. Workaround in `lib/sem/value.lua`: `kind_of` declares return `string`, not the closed `ValKind` union (forfeits exhaustiveness checking on kinds). Fix the return-position literal-union assignability in the checker, then restore `ValKind` returns.
