...
- [ ] **`nil` passes as subtype of any type in function arity mismatch** — when a function has fewer params than expected (or vice versa), the missing param falls back to `T_NIL`. `unify(T_NIL, string)` then passes silently, so a 2-param function assigns to a 1-param slot without error. Root cause: `T_NIL <: T` passes for all T in unify.lua — nil is not the bottom type but is being treated like one. Fix: only `nil <: nil | T` should pass; `nil <: string` is a real type error. Reported 2026-04-19.
