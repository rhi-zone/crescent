...
- [ ] **Frame-parser surface for live_table / event_stream demos** — `server.lua` `pump_stream` (around lines 351-368) currently wraps each shell line as `{message=...}`, which is the right shape for `log_stream` but wrong for `live_table` (`{op, row?, key?}`) and `event_stream` (`{time, kind, body}`). Until the backend can produce the right frame shape, demo aliases for these two primitives produce frames the projections silently ignore. One hypothesis (not yet decided): a new alias spec field `output.frame_format = "jsonl"` makes `pump_stream` `pcall(json.decode, line)` and forward the parsed object as the frame, with an error event on parse failure. Pack author writes a shell command emitting JSONL (`while true; do echo '{"op":"upsert","row":{...}}'; sleep 1; done`) and gets a real demo. Other shapes worth considering: a structured cap surface that yields frames natively, or built-in parsers (`output.parser = "ps" | "journalctl"`). Decide before adding demo aliases — adding them with the wrong frame shape is worse than not adding them.
