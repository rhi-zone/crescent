...
- [ ] **`check_string`/`check_file` with `opts.globals_files`: a globals file with >= 19 leading plain `--` comment lines before its first `--::` annotation silently disables argument-type checking for everything the file declares.** Root-caused while investigating the `lib/platform/apps/system_dashboard/projections/projection_types_test.lua` failure ("rejects wrong-typed prop argument" — `dom.span(42, {})` produces zero errors when it should reject `42` as not assignable to the `Props` record). Confirmed **content-independent**: a globals file consisting of N throwaway `-- dummy line k` comments followed by a `declare dom = { span: (props: Props|nil, ...) -> Element }` block reproduces it purely by leading-comment-line count — 18 leading lines checks correctly (1 error), 19–29 lines silently passes (0 errors), 30 lines checks correctly again, 31+ lines silently passes again. Non-monotonic in line count, so it is not a simple length/timeout guard — looks like an off-by-something in a fixed-size buffer or line-tracking accumulator that only bites within specific leading-comment-line-count bands. Confirmed it's specific to *leading* comment lines before the first annotation (trailing dummy comments after all declarations do not trigger it) and is unrelated to which `check_string`/`check_file` disk-cache path is used (reproduced fresh each run with distinct file content, so it isn't a stale-cache artifact). **This is currently live in production**: the real `lib/platform/apps/system_dashboard/projections/projection_types.lua` has 36 leading comment lines before its first `--::` line, so today `dom.*` prop-argument typechecking is silently a no-op for every projection author. Root cause not yet found — needs a read of `docs/type-system.md` + `docs/typechecker-v2.md` (per `lib/type/static/CLAUDE.md`) and a walk of the `--::` annotation/comment scanner in `lib/type/static/` (likely `ann.lua` or `check.lua`'s doc-comment extraction) before attempting a fix; not something to patch by trimming the file's real documentation to dodge the parser defect.
