...
- [ ] **A value narrowed from `string.find`'s `$FindReturn<P>`-derived return does not satisfy `string.byte`'s overloaded (intersection) signature, even though it satisfies a plain single-signature function and arithmetic.** Repro in `lib/pdf/object.lua` (`parse_dictionary_or_stream`, the `endstream`-scanning fallback for streams with an unresolvable indirect `/Length`): `local found = string.find(s, "endstream", pos, true); if found == nil then return nil, "..." end; local raw_end = found` — using `raw_end` (or even `found` directly, no arithmetic needed to reproduce) as an argument to `string.byte(s, raw_end)` fails overload resolution with "cannot pass `_` where `integer | nil` expected" / "cannot pass `_` where `integer` expected", i.e. both candidates of `byte`'s intersection type reject it. The same narrowed value passes cleanly to `string.sub` (single, non-overloaded signature) and to plain arithmetic (`raw_end - 1`) with zero errors. Root cause not investigated; likely `$FindReturn<P>`'s narrowed member type carries an internal placeholder (`_`) that isn't fully resolved to plain `integer`, and only an intersection-type overload resolver's stricter candidate-matching surfaces it — non-overloaded callers structurally accept `_` and let it through. Worked around with an explicit `--[[: integer]]` cast at the point `found` is bound to `raw_end` (a same-type coercion — `found` is already `integer | nil` narrowed to `integer`, not a widening cast — so it's a normal checked cast, not `--[[:! T]]`). Revert the cast when this resolves cleanly without it.
