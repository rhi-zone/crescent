...
- [ ] **Daemon `POST /api/new-card` violates platform invariant** — `lib/platform/CLAUDE.md` says "no format knowledge in the platform", but the daemon endpoint hardcodes `BLANK_CHARA_JSON` (CCv2-specific). The ccv2 app already has all the editing tools — "New Card" is just a blank starting point that the user then edits. The correct fix: the ccv2 tarball includes a `blank.png` entry (a minimal valid CCv2 card); the daemon's new-card endpoint reads that entry from the installed app's tarball instead of hardcoding the format. Platform stays agnostic; ccv2 owns its own blank template. Separate creation-paradigm apps (CYOA builders, template galleries, etc.) are orthogonal — they produce a PNG that gets imported normally.
