...
- [ ] **Eliminate remaining intrinsics via `match` arm patterns** — `$PcallReturn`, `$PairsReturn`, `$IpairsReturn`, `$Keys`, `$EachField` are intrinsics only because `match` doesn't yet support function-type arm patterns. Note: `$Keys<T>` IS `keyof T` — same thing, sigil-named. Once these patterns are implemented, all become user-definable in stdlib.d.lua with no new operators needed:
  ...
  - **[SPECCED]** `{ ...[K]: V }` all-fields pattern — spec at docs/all-fields-pattern-spec.md (2026-03-30). Matches any table (total). K = union of key types (string for named fields, indexer key type). V = union of widened value types. Eliminates `$Keys`, `$Values`, `$IpairsValues`. IpairsReturn: `match K { integer => ..., _ => never }` downstream filter. Implementation: new PAT_ALL_FIELDS in ann.lua, eval_all_fields in match.lua.
