...
- [ ] **Eliminate remaining intrinsics via `match` arm patterns** — `$PcallReturn`, `$PairsReturn`, `$IpairsReturn`, `$Keys`, `$EachField` are intrinsics only because `match` doesn't yet support function-type and indexer arm patterns. Note: `$Keys<T>` IS `keyof T` — same thing, sigil-named. Once these two `match` patterns are implemented, all become user-definable in stdlib.d.lua with no new operators needed:
  - `match T { (...) -> R => R }` — binds return type (eliminates `$PcallReturn`, `ReturnType`)
  - `match T { { [K]: V } => K }` — binds indexer key type (eliminates `$Keys`, `$PairsReturn`, `$IpairsReturn`). Known leak: when two `pairs()` loops are merged into one variable, the constraint chain back to the source table breaks and `$PairsReturn<T>` escapes the `for` scope — the inferred type is `$PairsReturn<Scope>` instead of `Scope`. Worked around in constrain.lua self-check with an explicit `--:` annotation. Reproduction: `ctx.scope = narrow_mod.apply_narrowed(ctx, else_neg)` where `else_neg` is built by merging two `pairs()` loops.
