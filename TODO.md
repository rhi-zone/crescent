...
- [ ] **Daemon `POST /api/new-card` violates platform invariant** — `lib/platform/CLAUDE.md` says "no format knowledge in the platform", but the daemon's new-card endpoint hardcodes `BLANK_CHARA_JSON` (CCv2-specific). This is a bootstrap convenience hack for the zero-cards state. The correct long-term answer is that card creation is just another app — a card creator app lives in the library alongside player apps, and the user launches it. No platform-level "new" convention needed; the platform stays agnostic. The daemon hack should eventually be removed in favor of an installable minimal card creator app.
