...
- [ ] **Eliminate remaining intrinsics via `match` arm patterns** — `$PcallReturn`, `$PairsReturn`, `$IpairsReturn`, `$Keys`, `$EachField` are intrinsics only because `match` doesn't yet support function-type arm patterns. Note: `$Keys<T>` IS `keyof T` — same thing, sigil-named. Once these patterns are implemented, all become user-definable in stdlib.d.lua with no new operators needed:
  - `match T { (...) -> R => R }` — binds return type (eliminates `$PcallReturn`, `ReturnType`)
  ...
  - `(A, ...T)` in type position — tuple prepend with spread typevar (needed for `$PcallReturn`'s `(true, ...ReturnType<F>)` form; may already be handled by TAG_SPREAD)
  After implementing these patterns, `$Require<T>` and `$Opaque<T>` remain as intrinsics (module system + nominal identity — not pure type computation). All others become prelude type aliases in `lib/type/static/prelude.lua`.
...
- [ ] **Parser stack overflow on deeply-nested types** — types with depth >~30 levels produce errors with nil message and col=0 (recursive descent hits Lua's call stack limit). Currently worked around in fuzz_test.lua by pre-checking and skipping ill-parsed types. Fix: either convert the recursive descent type parser to an iterative precedence-climbing parser, or add explicit depth tracking with a clean error at the limit. Currently silently swallows the error as a nil-message entry in err_ctx.errors.
- [ ] **fuzz_arb.lua sub_size halving reduces deep-type coverage** — changed from `sub_size = size - 1` to `sub_size = floor(size/2)` to prevent exponential type string blowup. This bounds effective type depth to ~6 levels. Pathological cases like deep intersection-of-unions or overloaded function chains may be under-tested. Consider a separate `arb_type_deep` that uses `size - 1` for algebra-level tests (fuzz_alg.lua) where no parsing is needed.
