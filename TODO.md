...
- [ ] **De-specialcase builtins** — `require` now has $Require<T> backing; still special-cased in constrain.lua pending stdlib.d.lua milestone declaration. Remaining: `pcall`/`xpcall` (needs TAG_SPREAD variadic capture), `pairs`/`ipairs` (needs typed iterator return), `type()`, `assert`, `error`, `select`, `string.find`/`io.open`/`string.byte` (now re-annotated with `-> ...()` spread — special cases can be removed), `string.match`/`gmatch`/`gsub`. Remove each special case only after its replacement is tested end-to-end.
