...
- [ ] **KNOWN DIFFERENCE, not yet closed: no "item compaction."** Two related-but-distinct yjs transaction-cleanup optimizations this port doesn't perform, both confirmed against `node_modules/yjs/dist/yjs.cjs`:
  1. Adjacent same-client Items with mergeable content get merged into one wire Item during cleanup (`tryToMergeWithLefts`) — already noted in `doc.lua`'s own header as a scope cut. Affects `parity_test.lua`'s `multiple_inserts` fixture (two sequential inserts stay two separate Lua Items instead of merging into one).
  2. A deleted-and-superseded Item's content gets GC-compacted to a bare `ContentDeleted` placeholder (dropping the actual value bytes) under yjs's default `gc: true`. Affects `delete_middle` (the deleted "lo wo" middle item keeps its string bytes instead of collapsing to a length-only placeholder) and `map_operations` (superseded/deleted map entries keep their value bytes).
  Both are marked `T.skip(...)` in `parity_test.lua` with content-equivalence checked instead of a raw byte comparison, rather than left as unexplained failures. Closing this requires implementing both compaction passes during/after `doc.transact` (or an explicit `M.compact`/`M.gc` step) — a real feature addition, not a bug fix, scoped out of the parity-test-infrastructure task that found it.
