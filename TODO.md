...
- [ ] **Design crescent-native socket library before rewriting ljsocket** — ljsocket is a vendored port (CapsAdmin/luajitsocket, imported Feb 2026), 1250 lines, no caps injection, 19 structural type errors. A rewrite is justified but requires a dedicated design session first: caps injection shape (does socket take epoll? or does caller wire it?), fd abstraction, async connect/accept pattern, flag enum style, error returns, platform abstraction. Do NOT use the style survey from the agent as ground truth — it was a quick scan, not a design decision. Write `docs/socket-design.md` before touching `lib/socket/`.
