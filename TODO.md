...
- [ ] **Initiative B — lua2ts + projection-Lua subset for pack-shipped projections** — partial.
  ...
  - **Still open**:
    - Pack-load pipeline that runs the typechecker → lua2ts → registers transpiled projection in the host JS registry.
    - Browser-side loader (host JS that consumes transpiled module output and `register()`s with `static/projections/registry.js`).
    - Host-side `Object.freeze` of built-in prototypes at startup (already exists in `static/harden.js` per line 21 above; verify it covers what harden mode assumes about the runtime).
    - Sync-only relaxation question — debounced renders, animations, tooltips. Punted; revisit when needed.
    - ShadowRealm / Worker isolation (alternative to lua2ts harden's static approach). Not needed for v1; revisit if `Object.freeze` startup proves insufficient.
    - `s:repeat(n)` is unparseable in Lua (`repeat` is a keyword) — projection prelude needs to expose JS's `String.prototype.repeat` under a different name (or rely on `s["repeat"](s, n)` syntax). Not blocking v1; affects what authors can write.
