...
- [ ] **Initiative B — lua2ts + projection-Lua subset for pack-shipped projections** — current registry shape (open set, runtime selection) anticipates pack-shipped projections; not yet wired. Architectural notes from design discussion: (1) projection-Lua is a *strict subset* of crescent Lua, sanitization is a property of the transpiler not a runtime gate; (2) banned: `_G`, `os`, `io`, `debug`, `package`, `loadstring`, `require`, `string.dump`, `getfenv`/`setfenv`, FFI; (3) emitted JS hardening: `t[expr]` compiles to `__safeGet(t, expr)` that throws on `__proto__`/`constructor`/`prototype`; no globals reachable from projection scope (`window`/`document`/`Function`/`eval`/timers/`fetch`/etc.); strict allowlists on tags/attrs/styles in the sealed DOM builder; `Object.create(null)` for user-data records; no closures capture mutable host state; sync-only (no timers, no async — open question whether to relax later). Open Qs: (a) typechecker projection-mode mechanism — path-based (files under `projections/` get a different prelude) vs import-graph based; (b) when transpile happens — at pack-load time, build step, on-the-fly?; (c) projection API surface (the `.d.lua` describing sealed builder + `ctx` + recursion). `lib/lua2ts/` already exists (see further down in TODO) — Initiative B is hardening + projection-API spec, not building from scratch.
...
- [ ] **Card app `send_event` return-check audit** — `send_event` became fallible (returns `(ok, err)`) when SSE landed. The 6 callers in `lib/platform/apps/charactercardv2/server.lua:1431-1475` were not updated. Latent inconsistency only — they all fire-and-close at end of stream so behavior is unchanged today, but a future change that streams mid-handler will silently swallow disconnects.
