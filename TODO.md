...
- [ ] **Soundness fix: occurs check** — `bind_var` (unify.lua) has no occurs check. Self-referential `'a ~ { next: 'a }` creates a cycle in the union-find structure; display/traversal may loop. See `docs/soundness-audit.md` Gap 4.
