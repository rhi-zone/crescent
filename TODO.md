...
- [ ] **`lib/web/js_types.lua` method convention** — all DOM methods are declared with self-parameter style `(Document, string) -> HTMLElement` (Lua method calling convention). Browser code uses dot notation (no explicit self), so every DOM call produces a "missing argument" type error. Fix: redeclare all DOM methods as function-style `(string) -> HTMLElement` etc. Affects `lib/web/html/init.lua` which currently workarounds with `--:: declare document = any`.
