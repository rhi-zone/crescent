...
- [ ] **Newtype IDs for type/intern/node IDs** — Type IDs, intern IDs, and node IDs are all `integer` at runtime but semantically distinct. Declare `newtype TypeId = integer`, `newtype InternId = integer`, `newtype NodeId = integer` etc. in ctx.d.lua. Annotate arena functions and helpers accordingly so swapped args are caught. Uses the existing `TAG_NOMINAL` mechanism — no new type system machinery needed.
