...
- [ ] **ccv2 import-time conversion + library integration.** The import pipeline (`lib/platform/import.lua`) already embeds the ccv2 runtime into a card PNG. New daemon endpoint `POST /api/import-card/upload` (113bc18) accepts raw PNG bytes. Still missing: drag-drop or file-picker UI on the library app that calls this endpoint, so users can import arbitrary cards without using the ST source adapter.
- [ ] **ccv2 tabbed card surface** — consolidate Card Editor, Card Lorebook, Author's Note, and Regex Scripts into a single tabbed panel (tabs: Identity, Lorebook, Greetings, Regex) reachable via the card header Edit button. Resolves reproduction-audit items #3 (toolbar overcrowding), #4 (duplicate entry points), #7 (Author's Note always visible), #8 (Regex as top-level sibling). Large frontend refactor; spec in `docs/ccv2-reproduction-audit.md` items #3-#4.
- [ ] **ccv2 input toolbar redesign** — blocked on tabbed card surface landing first (navigation buttons relocate into card surface). After that: Send stays, Continue/Impersonate stay as composition group, Export moves to card header, Settings/Lorebook/Regex/Group move to card surface or overflow. Apply S1, S3, T1, T3 from `docs/ui-design.md`.
...
- [ ] **service/cli.lua `pretty_print` nested tables** — `list-keys` outputs `providers=table: 0x...` instead of the array contents. `pretty_print` handles top-level arrays and key=value maps but not nested tables (a map whose values are arrays). Fix: recursively serialize value tables to JSON-like inline form, or just use `json.encode` for nested structures. Use `--json` as workaround in the meantime.
...
- [ ] **`lib/http/` x-suffix naming cleanup** — `serverx.lua`, `tablex.lua`, `staticx.lua`, `staticx_404.lua` use opaque `x` suffix. Actual distinctions: `serverx` = WebSocket-capable server (no TLS); `tablex` = table router with glob/wildcard capture (`req.globs`); `staticx` = full static server with dir listing + `getxattr` MIME types. Rename to communicate semantics: `server_ws.lua`, `table_glob.lua`, `static_full.lua`, `static_full_404.lua`. Update callers: `crescent_examples/ws_json.lua`, `router/fsx.lua`, and any others.
