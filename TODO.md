...
- [ ] **Soundness audit** — do this AFTER the type system is feature-complete. Known gaps: (1) union-of-functions call: arg that matches only *some* union members at try_unify level (due to TAG_VAR permissiveness) may slip through; (2) covariant/contravariant positions in generics not enforced; (3) recursive types. Note: (2b) try_unify TAG_INTERSECTION — FIXED (2026-03-15); (2c) intersection LHS merged-field subtyping — FIXED (2026-03-15).
...
- [ ] **LIT_NUMBER float fix** — `LIT_NUMBER` still uses per-file numval index in `data[1]`. Should store the double value directly across `data[1]`+`data[2]` (8 bytes, reinterpret via FFI cast). Makes float literals globally comparable; enables `x == 3.14` narrowing and consistent display.
...
- [ ] **Newtype IDs for type/intern/node IDs** — Type IDs, intern IDs, and node IDs are all `integer` at the Lua level but semantically distinct. Annotating them all as `integer` is wrong (they're not interchangeable). Need nominal newtypes: `TypeId`, `InternId`, `NodeId` etc. so annotations accurately capture which ID space is expected. Prerequisite: nominal type constructors in annotation language.
- [ ] **Explicit `any` warning** — Session 16 (2026-03-15): DONE. `resolve_annotation_type` emits a warning when `TAG_ANY` is encountered in an explicit annotation (`ctx._ann_warn_line` set at call sites in LOCAL_STMT, FUNC_EXPR, FUNC_DECL). infer.lua annotations fixed (55 → 0 warnings).
- [ ] **Structured diagnostics** — Replace all `report(ctx, line, col, string)` calls with `report(ctx, line, col, code, args)`. Integer error code (an enum) + typed args table per code. Central `format_diagnostic(entry)` renders the message from a template table. Enables: consistent formatting in one place, machine-readable codes, correct display once enum inference is done. All error message templates follow the agreed consistent style (backticks, natural English). Dependency: enum inference first.
