...
- [ ] **Phase C of unannotated-param-semantics plan: MISSING_PARAM_ANNOTATION autofix + modal inference + outlier detection** — Phase A (commit `c43bd439`) and Phase B (commit `a61c7cbb`) shipped. Phase C deferred because the current solver does not maintain per-call-site bindings: `solve_sub` destructively binds a param var to a single type, so post-pass we know only the "consensus" type the solver agreed on across callers, not the per-call-site distribution that the plan's modal-selection algorithm (≥80% threshold, strictly more frequent than alternatives, no autofix on tie) requires. Two implementation paths: **(a)** non-destructive variant of `solve_sub` for inferred params that records each (call_site_line, arg_tid) into a side table and lets the post-pass aggregate; **(b)** record (call_site_line, arg_tid) on `C_SUB` at constraint-emit time in `constrain.lua` and aggregate in the existing post-pass. Path (b) is less invasive but increases per-constraint memory; path (a) keeps the solver clean but changes the binding semantics for inferred-param C_SUBs (callers no longer compatibility-check via unify — would need to be reinstated separately). Also out of scope: `PARAM_INFERENCE_OUTLIER` warning at minority call sites; annotation-rendering helper (round-tripping a tid back to `--:`-parseable syntax); preceding-line byte-position computation for single-line vs multi-line function defs. Plan docs: original plan + commit messages on `c43bd439` and `a61c7cbb`. (Source: current session.) (see docs/typechecker-param-semantics.md)
