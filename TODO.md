...
- [ ] **Gap P1: dotted callee effect propagation broken** — `io.write(...)` has a uvar callee at gen-pass time; `!io` is never extracted. F2 enforcement does not fire for dotted stdlib calls. Only direct-bound names propagate effects. Highest-priority source pipeline fix.
- [ ] **Gap P2: pcall return type is flat `boolean | unknown`** — correct form is discriminated `(true, R...) | (false, E)`. Requires variadic generics (G17) first.
- [ ] **Gap P3: coroutine.create returns unparameterised `thread`** — full `Coroutine<Y,S,R>` parameterisation deferred; requires G17 or a specialised constraint family.
- [ ] **Gap P4: Arrow subtyping defaults uvar to CEq at S-Quiesce** — proper bounded-tvar tracking (G9) is deferred; may reject valid programs.
