...
- [ ] **Eliminate remaining intrinsics via `match` arm patterns** — `$PcallReturn`, `$PairsReturn`, `$IpairsReturn`, `$Keys`, `$EachField` are intrinsics only because `match` doesn't yet support function-type arm patterns. Note: `$Keys<T>` IS `keyof T` — same thing, sigil-named. Once these patterns are implemented, all become user-definable in stdlib.d.lua with no new operators needed:
  - **[DONE]** `match T { () -> R => R }` — function-type arm implemented (match.lua, 2026-03-29). Binds R to return type. `() -> R` with 0-param pattern matches ANY function regardless of param count (enables `ReturnType<F>`). For multi-return, R binds to a tuple. `PcallReturn<F>` can be expressed as `match F { () -> R => (true, R) | (false, string) }` for single-return F. Full $PcallReturn replacement for multi-return requires `(true, ...R)` spread-in-tuple-position (not yet supported in type syntax).
  - **[DONE]** `match T { { [K]: V } => K }` — indexer arm patterns implemented in match.lua (see type_complex_test.lua "match: indexer arm patterns"). `{ [K]: V }` in a match pattern binds K → indexer key type, V → indexer value type. Alias-param substitution happens before match evaluation, so T in result arms is already concrete — `PairsReturn<{ [string]: integer }>` CAN already be expressed as a match alias for indexer tables. Full intrinsic replacement requires named-field fallback arms (K=string, V=union of all field values): this needs a way to union all field value types in a result expression, which requires either `$EachField`-level iteration in a result, or a new "field values" pattern. Known leak for $PairsReturn: when two `pairs()` loops are merged into one variable, the constraint chain breaks and `$PairsReturn<T>` escapes scope. Workaround: explicit `--:` annotation.
  - **[DONE]** `(true, ...R)` spread-in-tuple-position — implemented (2026-03-30, env.lua TAG_TUPLE splice, constrain.lua unwrap_spread, narrow.lua propagate_multi_ret). `$PcallReturn` intrinsic deleted; `PcallReturn<F> = match F { () -> R => (true, ...R) | (false, string) }` in stdlib.d.lua.
  - **[DONE]** `$Values<T>` and `$IpairsValues<T>` intrinsics — added (2026-03-30, intrinsic.lua extract_values). `$PairsReturn`/`$IpairsReturn` intrinsics deleted; now match aliases in stdlib.d.lua using `$Values`/`$IpairsValues`.
  - **[DONE]** `{ ...[%K]: %V }` all-fields pattern + `%Name` capture sigil — implemented (3f67ea2, 2026-03-30). `%Name` in pattern position = TAG_CAPTURE (always matches, binds). Bare `Name` in pattern = TAG_NAMED (type lookup). PAT_ALL_FIELDS distributes per-field, unions results. `$Keys`/`$Values`/`$IpairsValues` intrinsics deleted; `Keys<T>`/`Values<T>` are now pure match aliases in stdlib.d.lua.
  After implementing the all-fields pattern, `$Require<T>`, `$Opaque<T>`, and `$FfiC` remain as permanent intrinsics. All others become prelude type aliases in stdlib.d.lua.
...
- [ ] **Narrowing doesn't apply to locals assigned from function call returns** — at narrowing time during constraint generation, locals assigned from function calls are still TAG_VAR (unsolved constraint variables). `types.subtract(TAG_VAR, T_NIL)` returns TAG_VAR unchanged. Workaround: add `--: T | nil` annotation to the receiving local so it gets a concrete type. Affects all `if not x then return end` patterns where `x` comes from a function call. **Fix direction (2026-04-01):** flow typing is not inference — they are separate concerns. Narrowing operates on resolved types, not constraint variables. The fix is a two-pass architecture: (1) constraint generation + solving resolves all TAG_VARs to concrete types, (2) a post-solve flow typing pass walks control flow and applies narrowings to the now-concrete types. No `C_SUBTRACT` constraint needed, no fixpoint, no interaction with the solver. Narrowing stays lexical/scoped — scope exit reverts to the pre-narrowing resolved type. Early-return patterns work because there is no join point. The two-pass approach is sound: constraints from the narrowed scope were generated against TAG_VAR (un-narrowed), which is more permissive — no false positives, just slightly less precision in the narrowed scope.
