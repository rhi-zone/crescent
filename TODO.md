...
- [ ] **Soundness fix: try_unify TAG_VAR** — `try_unify` returns true for any free TAG_VAR on either side (unify.lua:601-602). This lets wrong types slip through union/intersection overload dispatch. Fix requires distinguishing unconstrained TAG_VAR ("any") from one with a partial bound. See `docs/soundness-audit.md` Gap 1.
...
- [ ] Cross-file go-to-def for fields — `x.bar` where x is a required module: navigate to where `bar` is defined in the module. Requires storing field origin info (source file + line) in the type arena or CRI format.
