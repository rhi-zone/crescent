...
- [ ] **Daemon `POST /api/new-card` violates platform invariant** — `lib/platform/CLAUDE.md` says "no format knowledge in the platform", but the daemon's new-card endpoint hardcodes `BLANK_CHARA_JSON` (CCv2-specific format knowledge) and implicitly assumes `runtime_files` = ccv2. Correct architecture: apps declare a `new` entrypoint in their manifest. The library calls `POST /api/launch/:app_id?entry=new` on the daemon, which mints a launch token and redirects to the app's "new" entry (which runs the app's own creation UI/flow). This is strictly better than a static template: the app's creation flow can include naming, parameters, branching, etc. The app knows its own format; the daemon stays format-agnostic. Also unblocks cross-origin limitation (library dispatches through daemon launch, not directly to app subdomain).
