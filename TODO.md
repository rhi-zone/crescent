...
- [ ] **ccv2 `static/app.js` modularisation** — the frontend was ~2400 lines in a single file; now ~1330. Extracted so far: `api.js` (HTTP helper), `persona.js` (persona panel), `group.js` (group chat overlay), `regex.js` (regex scripts tab), `settings.js` (generation settings + connection test overlay), `sessions.js` (chats sidebar), `my-lorebooks.js` (user-side lorebook library overlay), `card-editor.js` (card editor overlay framework + identity/greetings tabs + Author's Note save). `index.html` already loads as `<script type="module">`. Remaining feature areas still inline in `app.js`: card-side lorebook + linked-lorebooks panel (and the shared `createEntryEl` helper currently injected into `my-lorebooks.js`), chat export, token counter, streaming send (`sendStreaming`), message list event delegation (edit/delete/swipe), focus-trap helpers (`trapFocus`/`releaseFocus`/`setupFocusTrap`), `escAttr`, New Card button. Extract one feature area per pass; each gets its own `<name>.js` exporting a single `init(deps)` returning `{ reload? }` or nothing. Dependencies that cross modules (e.g. `showError`, `setBusy`, `reloadMessages`, `trapFocus`, `escAttr`) should be injected through `init` opts, not imported from `app.js` (would cause circular imports). When the card-side lorebook is extracted, move `createEntryEl` into a shared module (e.g. `lorebook-entry.js`) and import it from both sides.
