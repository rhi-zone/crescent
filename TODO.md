...
- [ ] **`lib/web/reactive_dom/` typechecker annotations** — needs `--:: require "lib.web.js_types"` + `--:: declare document = Document` + proper element/attrs/handler types throughout. Currently uses implicit `document` global (typechecker error on `unknown identifier`). Pre-existing gap not caused by js_types fix.
