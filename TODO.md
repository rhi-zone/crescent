...
- [ ] **Deferred arg checking for free-TV params** — when `<F: (A,B)->R, A, B, R>(f: F, a: A, b: B)->R` is called, `a: A` and `b: B` are free TVs at argument-checking time and absorb any arg type (no error for wrong types). C_BOUND fires after the call and back-propagates A, B, R from F's concrete type, but by then argument constraints are already resolved. The `...P` vararg path avoids this by returning `false` (defer) when P is a free TV, re-running after C_BOUND resolves P. The named-param path needs the same defer logic: when a regular param TV is still free (FLAG_GENERIC + unbound), defer the arg unification until the TV is resolved. Fix: in solve_callable's param loop, if `et.tag == TAG_VAR` and the TV has FLAG_GENERIC, return false to defer. This requires tracking which free TVs are generic vs inferred-at-call-site to avoid infinite deferral. See type_soundness_test.lua "named-param variant" GAP comments.
