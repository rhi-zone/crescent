...
- [ ] **FFI fixed-size-array element typing (typechecker prerequisite for accessor cleanup).** `cdata_struct.array_field[index]` where the cdef declares `int32_t[N]` does not narrow to the element type — it stays as `{[integer]: integer}`. This is the root cause of hundreds of pre-existing errors in `lib/type/static/*.lua` (every direct `t.data[N]` access). Pinned by the C2 attempt (see `docs/typechecker-data-accessor-cleanup.md` "Status — BLOCKED" section). Minimal repro: `ffi.cdef[[ typedef struct { int32_t data[7]; } S; ]]; local function f(t --: S) return t.data[0] end` — should typecheck, currently errors with `cannot return {[integer]: integer}: cannot assign to integer`. Fix unblocks the typed-accessor cleanup AND removes ~500 pre-existing errors across the typechecker codebase. Likely small fix in `lib/type/static/cdef.lua` or `intrinsic.lua` (wherever FFI struct field access is constrained).
